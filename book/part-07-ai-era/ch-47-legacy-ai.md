...
|---|---|---|---|
| **Google Antigravity**(Gemini 3.1 Pro) | 200 萬 token | ★★★★★ 大型 codebase 整體理解 | 可一口氣吃下整個中型 monorepo,但 production 穩定性仍差。**用它做分析,不用它做生產修改**。 穩定性差在 Brownfield 修改場景的風險尤其高：多步驟推導（A 呼叫 B、B 的副作用影響 C）中途若模型輸出不一致，誤差會逐步累積，最終產出的程式碼可能帶有隱含邏輯 bug，而這類 bug 往往在並行測試階段才浮現。**分析階段**可以接受不穩定性，因為輸出只是文字報告，人工審閱可以過濾；**修改階段**容不下這個風險，程式碼直接進 codebase，出錯的代價是線上事故。 |
...
1. **理想化語言斷裂**：To-Be 設計引入全新的 Bounded Context 名稱與概念（例如把「放款合約」拆成「Product」和「Agreement」），但現有的業務使用者、訓練材料、客服腳本全都用舊語言。上線後使用者不知道「Agreement」對應到哪個操作流程，訓練成本爆表，最後只能回退。
2. **無法回滾的切換**：To-Be 模組直接取代舊模組，沒有並行期。一旦發現差異（往往是舊系統藏了十年的邊緣邏輯），既無法在新系統快速補齊，也無法在不影響線上的情況下切回舊系統，陷入「前進不了、後退不得」的僵局。
3. **Strangler Fig 中途卡死**：設計上假設「做完所有 BC 再上線」，但 Strangler Fig 要的是漸進交付。一旦遇到比預期更複雜的舊模組，整個遷移就卡在那裡無法繼續，最終被迫放棄或全量重寫。

這三種失敗的共通點是：**To-Be 的設計忽視了舊系統的語言習慣、缺乏中間緩衝態、缺乏回滾能力**。三條黃金規則就是針對這三個點的對策。
...
流程圖的步驟順序不是任意的，每個關鍵轉折都有因果理由：

- **Step 2（契約測試）為何必須在 Step 3（Agent 生成骨架）之前**：契約測試的目的是把舊模組的行為固化成可執行的規格——這是整個迭代的 source of truth。如果先讓 Agent 生成骨架，工程師容易不自覺地拿新骨架的行為來定義「應該是什麼」，而不是舊系統的實際行為。順序倒過來，就把「重寫即重新定義需求」的陷阱擋掉了。
- **並行運轉（Step 4）為何在生成骨架之後才啟動**：並行運轉需要兩側都能接收真實流量、各自計算、輸出可比結果。這要求 To-Be 模組至少有一個可運行的初版——骨架寫完、契約測試通過之後，才有並行的資格。太早開啟並行會讓比對結果充滿雜訊（To-Be 尚未完整），失去診斷意義。
- **Step 5 為何是決策點而非直接修復**：差異的根源決定了正確的處置。是「舊模組 bug」就代表 To-Be 寫了比舊系統更正確的版本，此時應該留下並寫 ADR 解釋決策，而不是盲目對齊舊版。是「業務歧義」代表程式碼層面無法決定哪邊對，強行修復只是把模糊搬進新系統，必須召集人類做決議。這三條分支的後續動作完全不同，Step 5 的分類是後面所有動作的前提。
