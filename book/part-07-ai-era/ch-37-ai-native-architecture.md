...
> **下游章節**:[Ch 38 Context-Driven Engineering](./ch-38-context-driven-engineering.md)、[Ch 39 RAG 與知識架構](./ch-39-rag-memory-tool.md)、[Ch 40 Multi-Agent 系統設計](./ch-40-multi-agent.md)
> **延伸補章**:[Ch 46 Agentic QA](./ch-46-agentic-qa.md)、[Ch 28 Compliance by Design](../part-05-quality/ch-28-compliance.md)

---

## 37.1 冷觀察 ⸺ 胸痛病人被 LLM 判成「非緊急」
...
ESI 是美國急診護理協會維護的 5 級分流標準 [^CIT-336]:**Level 1 是會死的(立即),Level 2 是會壞掉的(< 10 分鐘),Level 3–5 視資源消耗下降排序**。**這個標準的最大失誤成本是病人死亡 ⸺ 每差一級,醫療資源的投入時間窗口就差十分鐘到數小時;Level 1 / 2 的誤判代價是命,Level 4 / 5 誤判代價是病人多等四十分鐘。** 這個尺度在 ER 圈是常識,但在做這套系統的工程團隊裡沒人講過這句話。
...
## 37.2 真問題 ⸺ AI-Embedded / AI-Augmented / AI-Native 是三種不同定位
...
進一步看,這三種定位的本質差別是「**確定性層**」與「**非確定性層**」的邊界放在哪 ⸺ Embedded 是確定性層包住非確定性層,Augmented 是兩層平行,Native 是非確定性層包住確定性層。這條邊界的位置不同,決定了系統的錯誤模式根本不同。37.2.3 節會展開這個推論。

這裡需要補充一個重要的細節:「崩潰」描述的是**設計意圖**,不是物理上的不可能。NorthVale 的系統在事故後八週確實被改成 LLM 故障時走純 DMN 規則 + 人工分流 ⸺ 技術上的降級路徑從來都存在。問題是在設計之初**從沒有人問過「LLM 不在的時候,旅程怎麼走」**,也從沒有人把答案寫進架構文件、測試計畫或 circuit breaker 設定。這才是 AI-Native 的真正風險所在:不是旅程物理上不能沒有 LLM,而是架構師**從沒設計那條沒有 LLM 的路**。換句話說,更精確的判準應該是:**「系統設計當初,有沒有預想 LLM 故障時的降級路徑?」** 沒有 → AI-Native(設計缺口);有 → AI-Augmented。

NorthVale 的問題是:**他們宣稱做的是 AI-Augmented,實作出來的是 AI-Native(設計上沒有降級路徑),而業務領域(臨床決策、生命安全)其實只允許 AI-Embedded**。三層錯位,事故只是時間問題。

### 37.2.2 為什麼「AI-Native」不等於「多用 LLM」
...
### 37.2.3 真正在處理的是「確定性 vs 非確定性」的邊界放在哪
...
這個拓樸決定了**錯誤模式**也不同,推論如下:

在 **AI-Embedded** 系統中,LLM 是被規則調用的工具。LLM 故障時,外層的確定性規則仍然在運行,系統可以直接跳過 LLM 呼叫、走降級路徑,業務旅程受損最小 ⸺ **故障是局部的,降級成本低**。

在 **AI-Native** 系統中,Agent 是整條旅程的規劃者:它決定下一步做什麼、呼叫哪個工具、何時結束。一旦 Agent 規劃出錯,整條決策鏈從根部就走偏了 ⸺ **故障不是某個 step 沒回來,而是整條旅程選錯了方向**。這正是 NorthVale 案例的核心問題:LLM 不是算錯某個數值,而是把整個分流旅程帶進了「等候區」而不是「搶救室」。

因此:Embedded 系統的故障多半是「LLM 回不來/超時」,降級即可;Native 系統的故障是「Agent 規劃出錯誤的旅程」⸺ 不是某個 step 失敗,是**整條旅程選錯**。臨床決策、金融授權、能源調度這類「選錯比慢更糟」的領域,如果走 AI-Native,就必須在非確定性層外圍**強制套上確定性護欄**,否則就是 NorthVale 的故事。
...
## 37.3 決策框架 ⸺ 該選哪一種定位、AI-Native 怎麼分層

在進入各張細節表格之前,先看全景。下面這張決策樹是整個 37.3 的骨架 ⸺ 37.3.1 到 37.3.5 分別展開樹中每個分支的細節。建議先讀懂這張圖,再往下看各層展開。
...
這張圖的關鍵在 **Q1 那個問句**:「**把 LLM 拿掉,還能成立嗎?**」這個問題能逼出產品真正的定位。NorthVale 當初如果在 kickoff 會議上回答了這個問題,他們會發現 ER triage 的核心旅程「拿掉 LLM 走不下去」⸺ 那一刻他們就應該停下來重做設計,而不是上線等出事。Q2 / Q3 的「會死人/虧錢/違法」是第二道過濾 ⸺ 高風險領域走 Native 基本上是自尋麻煩。

以下 37.3.1 到 37.3.5 依序展開樹中各分支的細節。
...
### 37.3.3 治理三支柱:HITL、Circuit Breaker、Audit Trail
...
> **重要:三支柱的必要性首先取決於業務領域,其次才是 AI 定位。** 表格中「高風險領域必要性」欄標示的是跨定位的底線要求 ⸺ 一個 AI-Embedded 系統如果在醫療領域運作(例如 EMR 用 LLM 自動生成出院醫囑草稿),HITL 與 Audit Trail 同樣是必要的,不因為它只是「Embedded」就可以省略。判斷順序是:先問「這個業務是否會死人/虧錢/違法」⸺ 是的話,無論是哪種定位,三支柱都要上;不是的話,再依 AI 定位決定輕量 vs 完整治理。AI-Native 的治理要求較重,是因為 Native 系統在高風險領域的出現本身就是一個架構警訊(參見 37.3.1 的適用情境表),而不是因為「Native 比 Embedded 天生需要更多治理」。
...
這份契約在 Hexagonal 視角下,可以把「Agent + LLM」整體當成一個 **Adapter**(`Driven Adapter`),L4 Tool 是另一組 Adapter ⸺ Domain 核心(L4 deterministic tools 服務的真實業務邏輯)不需要知道自己背後的 AI 實作細節,只需要接收結構化的 input、產出結構化的 output。

這裡要區分兩個不同的概念:**實作抽象(Abstraction)** vs **風險盲視(Ignorance)**。Domain 不需要知道 LLM 的模型版本、token 計費或 prompt 細節 ⸺ 這是好的抽象。但 Domain **必須知道哪些決策路徑由 LLM 驅動**,以便做風險評估、設定 HITL 觸發點、以及履行合規義務。NorthVale 的心臟科主治醫師如果從一開始就知道「ER triage 的 ESI 判讀由 LLM 完成」,他可能會要求 ESI ≤ 2 的紅旗判讀不能由 LLM 單獨決定。臨床責任(與 HIPAA / EU AI Act 的審計要求)不允許「Domain 對 LLM 的存在毫不知情」。**「Domain 不需要知道 AI 的實作細節,但 Domain 必須知道哪些決策路徑由 LLM 驅動」 ⸺ 這才是一條判斷 AI-Native 設計做得好不好的正確準則**。

### 37.3.5 與 Modular Monolith / Hexagonal / Event-Driven 整合
...
回到本節開頭的決策樹:37.3.1 的情境表展開了 Q1 三個分支的細節邊界;37.3.2 的七層架構是「進入 Native 分支後」的內部結構;37.3.3 的治理三支柱是所有高風險分支(Q2 / Q3 答「是」)的必要配備;37.3.4 的契約是確定性層與非確定性層之間的介面協議;37.3.5 說明七層架構如何落進既有架構風格。

在跨 PM / SA / 法遵的三方會議上,推薦從 Q1 開始對齊:先回答「LLM 拿掉旅程能否成立」,再看業務領域風險,最後才討論元件選型。把 Q1 的答案白板上寫下來,一半的定位混亂問題就消失了。
...
## 37.5 交付清單 ⸺ AI-Native System Vision Card
...
| 層 | 元件名稱 | 是否需要? | Owner |
|---|---|---|---|
| L1 Foundation Model | GPT-4o-class 私有部署 + Claude Sonnet fallback | ✅ | AI 平台組 |
| L2 Inference / Serving | OpenShift 4.16 自管 | ✅ | SRE |
| L3 Memory | Epic FHIR R4 查詢(唯讀,每次 triage 即時拉) | ✅ | 整合組 |
| L4 Tool | DMN 紅旗規則表、ESI 寫入、paging API、HL7 ADT | ✅ | 急診系統組 |
| L5 Agent Orchestration | 無獨立 Orchestrator;LLM 單次推理即回傳 | 最小化 | AI 平台組 |
| L6 Experience(HITL UI) | 護理師工作站 confirm UI | ✅ | 前端組 |
| L7 Governance | LangSmith + FHIR AuditEvent + Circuit Breaker | ✅ | 病安長 + SRE |
...
四項中先挑一項做完就好,建議是最後那一項 ⸺ 把手上正在規劃的 AI 功能拉出來,補一張 Vision Card,逼自己回答 Litmus Test。本章留給你的就是「把 LLM 拿掉,旅程還能成立嗎」這條判斷線。下一章 [Ch 38 Context-Driven Engineering](./ch-38-context-driven-engineering.md) 會接著回答:**確定性那一層,要怎麼餵給 AI 才能讓它穩定產出**。

---

## Cross-References

- **下一章**:[Ch 38 Context-Driven Engineering](./ch-38-context-driven-engineering.md) ⸺ AI 協作下的脈絡工程
...
- **下游 RAG**:[Ch 39 RAG 與知識架構](./ch-39-rag-memory-tool.md) ⸺ L3 Memory 的展開
- **下游 Multi-Agent**:[Ch 40 Multi-Agent 系統設計](./ch-40-multi-agent.md) ⸺ L5 Agent Orchestration 的展開
- **延伸補章**:[Ch 46 Agentic QA](./ch-46-agentic-qa.md)、[Ch 28 Compliance by Design](../part-05-quality/ch-28-compliance.md)
