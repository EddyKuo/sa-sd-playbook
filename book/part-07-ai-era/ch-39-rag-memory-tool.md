...
實務上混合不是預設最優解——何時值得加第二、三軌有門檻判斷:若領域內法規條號 / 產品代碼類查詢佔 > 30%,優先加入 BM25;若結構化關聯查詢(「哪個 tier 適用哪條規則」)佔 > 20%,考慮加 KG。若兩個門檻都不到,單軌 vector 已足夠——此時強行三軌只增加維運複雜度,對 recall 的邊際收益不顯著。只有在這些門檻觸發後,三軌混合才能穩定帶來那 18~25 個百分點的增益。
...
| 層 | 內容 | 何時讀 / 寫 | 何時清 |
|---|---|---|---|
| **Short-term** | 當下 conversation 的 message list、scratchpad、tool call 中間結果 | 每個 turn 讀、session 結束前寫 | session 結束 |
...
> **注意**:turn 是單次對話往返(用戶說一句、Agent 回一句),session 是連續多個 turn 的容器。Short-term memory 在整個 session 內保留(跨 turn 可讀),session 結束即清除。HavenAxis 的 LangGraph `MemorySaver` 只做到了這一層,跨 session 什麼都沒記。
...
fact extraction 的輸出應是結構化 JSON,每條事實格式如下:
...
```json
{
  ...
  "value": "zh-TW",
  "confidence": 0.95,
  "extracted_from": "客戶說『我住在台灣，請用繁體』",
  "ttl_days": 365
}
```

一個 session 通常提煉成 5–10 條這樣的事實(`preferred_language`、`kyc_tier`、`last_rejected_doc_type`、`preferred_contact_channel` 等)。`confidence` 低於 0.7 的條目建議捨棄或標記待複核,避免低信心的推測污染後續 session。
...
# Composition:回傳的 case_id 與 next_action_for_agent 欄位設計來支援 Composition——
#             case_id 可直接餵給下一個 tool(如 mark_kyc_document_received)
TOOL_RESULT_OK = {
...
> **命中分散度影響的是成本**:集中命中時(常用 80% 的同一份),long context 每次都把同一份文件完整塞進 prompt,會重複消耗相同 token;相比之下 RAG 精選幾段反而沒有優勢。分散命中時(長尾查詢、每次命中不同 chunk),RAG 的精選檢索比 long context「全包」節省 50~80% 的 token 成本——這才是表格中「命中分散度」一列真正在衡量的代價差異。
...
>
> **如何系統性進行 CRUD → task-oriented 的拆分**:(1) 列出所有 endpoint;(2) 分組——把解決同一個「業務任務」的 endpoint 歸在一起(如 KYC 補件相關的 GET / PATCH / POST 都屬「KYC 文件管理」任務);(3) 為每組任務命名一個動詞 + 名詞限定的 LLM 友善 tool 名稱;(4) 驗收測試——給 50 個典型 query,LLM 選對率達 > 95% 才算完成拆分。
