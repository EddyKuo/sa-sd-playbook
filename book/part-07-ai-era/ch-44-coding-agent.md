...
> **前置閱讀**:[Ch 1 為什麼 SA/SD](../part-01-foundations/ch-01-why-sa-sd.md)、[Ch 21 Modular Monolith](../part-04-architecture/ch-21-modular-monolith.md)、[Ch 33 ADR 與架構知識管理](../part-06-engineering/ch-33-adr-architecture-knowledge.md)、[Ch 38 Context-Driven Engineering](./ch-38-context-driven-engineering.md)
> **下游章節**:[Ch 45 AI 時代的 SA 角色重構](./ch-45-ai-eval-drift-redteam.md)
> **延伸補章**:[Ch 47 遺留現代化](./ch-47-legacy-ai.md)

---

## 44.1 冷觀察 ⸺ 同一個 Cursor,在兩個 codebase 表現差三倍
...
這句話刺耳的不是「拆錯了」,是 ⸺ 工程團隊一直以為「服務小一點 = Agent 表現好一點」。實際上**剛好相反:Agent 的表現由「脈絡完整度」決定,而脈絡完整度跟服務切多細沒有單調關係**。Ch 1 的 PayLoop 在 180 天炸掉,炸的是「決策失憶」;Ch 38 的 AisleNova 在第六個月歸零,歸的是「沒有契約結構的脈絡」;CartMosaic 在第六週掉到 24%,掉的是「拆分時把脈絡留在原地」。同一條線在 2026 年反覆上演 ⸺ AI Coding Agent 的失敗,十次有八次是 codebase 沒準備好。

---

## 44.2 真問題 ⸺ Agent 的表現由 Codebase 決定,Codebase 是 Agent 的 Context
...
| **Codebase 重構成模組化形狀**(Ch 21) | 極高(2–5x 改善) | 高(1–6 個月) | Agent 投資前先做 |
| **CDE 三層脈絡寫好**(Ch 38) | 極高(配合上一項放大) | 中(1–3 個月) | 跟重構並行 |
...
### 44.2.2 Agent-friendly Codebase 的工程意義
...
2. **Skill 對應 ADR**(Ch 38 §38.3.4)⸺ Agent 推論時讀的是當前生效的決策
3. **Codebase 模組邊界清楚**(Ch 21)⸺ Agent 局部推理不必撈整個 repo
4. **Test 覆蓋足以驗證生成程式**(Ch 38、Ch 34)⸺ Agent 寫完能自我驗證
...
換句話說,Codebase 設計是 AI 時代的隱形基礎建設。它不是 RD 的個人偏好,**是 RD Team 對 AI Coding Agent 的最大投資項目**。Ch 21 的 Modular Monolith 不只是「人類維護的形狀」,在 2026 是「Agent 推理的形狀」⸺ 同一個工程紀律,2026 多了一類受益者。

---

## 44.3 決策框架 ⸺ 工作流四種、Agent-friendly 判準、SDLC 五切入點

下面這幾張表跟兩張 Mermaid,在現場相當好用。它們的共同前提是:**選工作流之前,先把 codebase 設計到 Agent-friendly**;工作流選對了,SDLC 整合才有著力點。

### 44.3.1 Coding Agent 工作流四種
...
### 44.3.2 Plan-Execute 與 Test-Driven Generation 兩種模式
...
### 44.3.3 Agent-friendly Codebase 的判準表

§44.2.2 的六個屬性,寫成可被 review 的判準表:

| 維度 | 不友善 | 可接受 | 友善 | 量測方式 |
|---|---|---|---|---|
| **CLAUDE.md 完整度** | 沒有 / 只有 README | 有 CLAUDE.md ≤ 200 字、寫了基本規則 | ≤ 800 字、含模組地圖、命令清單、紅線、Skill 索引 | PR 模板強制檢查存在;字數靠 lint |
| **Skill ↔ ADR** | Skill 不存在 / Skill 與 ADR 脫鉤 | Skill 存在但 Knowledge Sources 漂移 | 雙向連結 + CI fitness function 守(Ch 33) | `cde-skill-adr-consistency.sh` |
| **模組邊界** | 一個函式跨 7 個 domain(spaghetti) | bounded context 模糊但目錄分明 | Ch 21 modular monolith、ArchUnit / Konsist 守 | 反向依賴比例 < 5% |
...
### 44.3.6 Agent-friendly Codebase ↔ CDE ↔ ADR 三角

把這條鏈條跟 Ch 38、Ch 33 接起來,可以畫成一張總覽圖:
...
flowchart LR
    subgraph SD[SA/SD 產出]
        ...
        ADR[ADR<br/>Ch 33]
        C4[C4 Diagrams<br/>Ch 20]
    end

    subgraph CDE[CDE 三層 Ch 38]
        Const[CLAUDE.md<br/>Constitution]
        Role[Role<br/>.claude/agents/*.md]
        Skill[Skill<br/>.claude/skills/*]
    end

    subgraph CB[Agent-friendly Codebase Ch 21+本章]
        Mod[模組邊界]
        Naming[命名一致性]
        Test[測試覆蓋]
        Ctx[Context Size]
...
### 44.3.7 完整 CLAUDE.md 範例片段

下面這份是 CartMosaic 重整後的 `inventory-service` 倉庫根目錄的 CLAUDE.md。它符合 §44.3.3 的「友善」判準(800 字內、含模組地圖、命令清單、紅線、Skill 索引),可直接抄走改寫。
...
> ✅ **修正方向**:每個 repo 一份 CLAUDE.md,大小 ≤ 800 字,內容對齊 §44.3.7 的範例。重點不是寫得長,是寫得「Agent 能在 5 秒內找到該讀什麼」⸺ Module Map、Hard Rules、Common Commands、Skill 索引四項是最低劑量。CartMosaic 重整時花了一週把 9 個服務各補 CLAUDE.md,其中六份是直接複用同一個模板填空。**不要等「我們先寫個完美的」⸺ 先有一份 200 字的,比沒有好十倍**。
...
> ✅ **修正方向**:SWE-Bench 是 capability 上限的指標,不是「在你 codebase 上的表現預測」。要評估 Agent 在自己環境的真實表現,做一份「**內部 SWE-Bench**」⸺ 從過去三個月真實 PR 抽 30 個任務、把對應的 spec / test 拆出來、讓 Agent 跑一次、人工 grade。這個內部分數才是 ROI 預估的依據。另外:**人寫測試 + Agent 寫實作**才是 Test-Driven Generation 的本意(§44.3.2);**Agent 寫測試 + Agent 寫實作**等於沒測試,因為兩邊的盲點是同一個。把這條紀律寫進 CLAUDE.md 的 Hard Rule:「critical path 的測試必須由人類撰寫或 review,Agent 只能補邊角測試」。

---

## 44.5 交付清單 ⸺ 一頁式 Agent-Friendly Codebase Card

每個 repo 要評估「自己對 Agent 多友善」時,**第一份要產出的不是改 codebase,是體檢卡**。它是一頁 Markdown,把 §44.3.3 的六個維度與 owner 攤開,讓團隊知道現況、知道下一步。

把它存在 `docs/agent-friendly-card.md`,跟 System Charter(Ch 1)、CDE Setup Card(Ch 38)同層,**三張卡片合在一起就是「這個 repo 讓 Agent 進得來、做得對、傳得出去」的最小完整劑量**。
...
> 對齊 CDE Setup:`docs/cde-setup.md`(Ch 38 §38.5)
...
- [ ] 講清楚 AI Coding Agent 的成功不在「Agent 多強」,在「**Codebase 多 Agent-friendly**」⸺ 並能用 §44.2.1 的 ROI 表反駁「換更貴模型就好」這類提案
...
- [ ] 為手上的 repo 寫好一份 `docs/agent-friendly-card.md`,並把第一份 CLAUDE.md 補進去(對齊 §44.3.7 範例)

如果四項中先挑一項做完就好,建議是最後那一項 ⸺ 找你手上**最常被工程師罵「Cursor 在這個 repo 寫不好」**的那個 repo,先補一份 800 字 CLAUDE.md,下個 sprint 看 PR 通過率有沒有改善。Ch 38 給你 CDE 三層的契約結構,本章給你「在 coding 場景具體怎麼用」的工作流與判準;Ch 47 接著把這套邏輯擴展到「遺留 codebase 怎麼補做 Agent-friendly」⸺ 那是 2026 年大部分工程團隊真正會卡住的地方,本章先把基礎契約交到你手上。
...
- **回顧**:[Ch 38 CDE](./ch-38-context-driven-engineering.md) ⸺ 本章是 CDE 在 coding agent 場景的應用層
- **下一章**:[Ch 45 AI 時代的 SA 角色重構](./ch-45-ai-eval-drift-redteam.md) ⸺ Coding Agent 普及後 SA 的職責變化
- **延伸補章**:[Ch 47 遺留現代化](./ch-47-legacy-ai.md) ⸺ 既有 codebase 怎麼補做 Agent-friendly
