...
下面以一個「財務分析師 Agent」為例，看三家廠商如何實現同樣的四要素：
...
## 42.3 決策框架 ⸺ SA 如何設計 Agent 設定規格

### 42.3.1 系統提示工程的三段式結構

三家廠商的官方最佳實踐都指向同一個系統提示結構。原因是這三段正對應業務需求文件的三個根本問題：(1) 這個角色的責任邊界在哪（角色定位）；(2) 它被允許和禁止做什麼（行為規範）；(3) 它的產出格式是什麼（輸出規格）。這個框架是 SA 本來就應該設計的東西——只是現在需要整合進 Agent 設定文件：
...
### 42.3.2 最重要的架構選擇：[Manager Pattern](../annex-f-glossary.md#manager-pattern) vs. [Handoff Pattern](../annex-f-glossary.md#handoff-pattern)
...
當設計 Multi-Agent 協調時，最關鍵的一個決策點是：中央 Agent 應該保持控制權並整合結果，還是應該把對話所有權轉移給專家 Agent？這個選擇影響整個系統的容錯能力和延遲特性。
...
### 42.3.3 [Progressive Disclosure](../annex-f-glossary.md#progressive-disclosure)：三層載入策略
...
### 42.3.4 CLAUDE.md / 專案記憶：跨 Agent 的全域規格
...
### 42.3.5 SKILL.md 開放標準：跨平台的可重用能力
...
> **修正方向**：Caldwell 應該：(1) 從 Python 程式碼裡提取所有 `instructions` 字串，寫成獨立的 `agent.md` Markdown 文件；(2) 把術語定義集中到 `CLAUDE.md`；(3) 建立 PR 審查流程，每次改動都要有 SA 簽核。這樣每次需求變更都有可追蹤的 diff，§42.5.1 的審查清單才有文件可核對。
