...
事後查明的根因:模型供應商在上線後第 9 天悄悄推了一次 claude-sonnet-3-7 的 patch 版本更新,同時 pgvector 索引裡的一份「條款範本庫更新文件」被法務助理手動上傳——這份更新文件沒有通知工程團隊、沒有走版本控制、也沒有觸發任何 CI 流程,在向量索引層面是「靜默的」——兩個變化疊加後讓 Agent 在特定租戶的長對話情境下開始偏向舊版措辭。任何一個單一變化都不足以觸發問題,組合起來才會。
...
**這一層用傳統 QA 測**。寫單元測試、契約測試、整合測試。工具選擇可按測試對象直接對號入座:

| 測試對象 | 推薦工具 | 測試重點 |
|---|---|---|
| 外部 API 契約(Provider/Consumer) | **Pact** | 雙方 schema 相容性;上游改版前先跑 Pact |
| JSON / YAML schema 驗證 | **jsonschema** | 結構化輸出欄位型別與必填項 |
| OpenAPI spec 靜態分析 | **Spectral** | spec 本身的規範性(lint 層) |
| RAG 檢索器回傳格式 | **pytest + jsonschema** | 文件片段長度、metadata 欄位完整性 |
| 資料庫 Query 正確性 | **整合測試(pytest + testcontainers)** | SQL 語意與預期結果集 |

這部分跳過,後面所有問題都會被歸咎於「LLM 不準」,但實際上是工具回傳格式不穩。
...
| 偏誤 | 描述 | 診斷方法 | 解法 |
|---|---|---|---|
| **Position Bias** | Judge 系統性地偏向第一個 / 第二個答案 | 對同一對樣本分別評估 (A,B) 與 (B,A) 兩個順序;計算兩組評分的皮爾遜相關係數,若偏離 1.0 超過 0.1 則確認有位置偏誤 | 每次評估都隨機交換 A/B 順序,n 次取平均 |
| **Verbosity Bias** | Judge 偏好更長的答案 | 對長度差異顯著的樣本對(如 50 字 vs 300 字)計算 Judge 偏好率;若長答案勝出率超過 70% 即為異常 | 在 rubric 明確要求懲罰冗長 |
| **Self-preference** | Judge 偏好跟自己模型風格相似的答案 | 用兩個不同模型家族各跑一次 Judge,比較偏好分佈;若偏好一致則 bias 低,若分歧超過 15% 則懷疑 self-preference | 用不同模型家族交叉驗證(GPT 受測就用 Claude 當 Judge) |
| **Sycophancy** | Judge 容易被 prompt 中的暗示帶風向 | 在 Judge prompt 裡加入「以下答案來自頂尖專家」或「以下答案來自初學者」的暗示,比較前後評分差異;若超過 0.5 分即確認有 sycophancy | Judge prompt 中刻意不透露受測者身份與意圖 |
...
> **閾值推導**:2σ 對應約 95% 置信區間,意味著在正常波動範圍內,單日超標的機率只有約 5%。「連續三天」是為了排除單日噪聲——一天超標可能是流量異常或節假日效應,三天連續才代表系統性偏移。若需要更靈敏的告警(例如高合規場景),可降至 1.5σ + 連續兩天,但誤報率會顯著上升;若要降低噪聲(例如流量波動大的場景),可升至 2.5σ + 連續五天。這個閾值不是固定規則,是一個需要根據系統基準波動率和業務容忍度校準的起點。
...
| Pre-production | Manual + 自動化 | Red Team Suite(靜態紅隊 Playbook + 自動化雙 Agent 跑 2–4 小時;自動紅隊產生的對抗樣本須人工過濾後才進 Eval Set) | 任何 P0/P1 攻擊成功 |
...
## 46.5 交付清單 ⸺ 一頁式 Agentic QA Pack

每當工程師準備把 Agentic 系統推上生產,第一份要交出的不是架構圖或 API 文件,而是這份 QA Pack——它把 Eval Set、Judge Rubric、Red Team Playbook、Drift Dashboard 和 CI/CD 整合點一次打包,讓 QA 負責人與 Risk Lead 都能在同一份清單上對齊驗收標準。
...
> 版本:v0.1 | 撰寫日期:2026-01-15 | 擁有人:Risk Lead Mei
...
<!-- 為什麼這欄:六個月 accuracy 從 92.3% 漂到 71.4% 沒人發現,就是因為這條沒接。
     反饋指標(banker 不採納比率、升級 MLRO 比例)其實有在監看,但兩個問題讓信號消失了:
     (1) 升級比例的基準線設得過高(參照歷史高峰值而非正常均值),導致實際上升幅度沒超過閾值;
     (2) 這些反饋指標從未與品質指標(Judge 平均分)做聯合告警,各自獨立的微幅上升
         被工程師當成「使用者習慣變化」而非品質下滑的信號。
     接上正確的 Drift Dashboard 後,FN rate 任一週連續三天 > 5% 就會自動建 ticket。 -->
- 品質:Judge 平均分 / 各維度 / FN rate(基準 < 3%)
- 工具:KYC lookup 失敗率 / 平均工具呼叫深度
- 反饋:banker 「不採納建議」比率 / 案件升級至 MLRO 比例
- **三類指標做聯合告警**:任一類連續 3 日 > 2σ 自動 PagerDuty + 建 ticket 給 Risk Lead;基準線設為上線後首 30 天的均值而非歷史高峰
