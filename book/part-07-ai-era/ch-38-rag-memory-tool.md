...
# 第 35 章|RAG、Memory 與 Tool 設計
## ⸺ Agent 系統的三組記憶與三種能力

> **前置閱讀**:[Ch 14 API 設計](../part-03-design/ch-14-api-design.md)、[Ch 15 資料儲存設計](../part-03-design/ch-15-data-storage.md)、[Ch 33 AI-Native Architecture](./ch-33-ai-native-architecture.md)、[Ch 34 Context-Driven Engineering](./ch-34-context-driven-engineering.md)
> **下游章節**:[Ch 36 Agent 編排與多代理協作](./ch-36-multi-agent.md)、[Ch 38 AI-Era SA/SD 心法](./ch-38-ai-eval-drift-redteam.md)
> **延伸補章**:[補章 B Agentic QA](./chB-agentic-qa.md)、[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)

---

## 35.1 冷觀察 ⸺ 那個記不住客戶 KYC 進度的合規客服 RAG
...
## 35.2 真問題 ⸺ RAG / Memory / Tool 是三個獨立子系統

把 Agent 系統當成「LLM 加上幾個 RAG 與 function」是 2025 年很常見的腦圖。這個腦圖在 demo 階段沒問題,在生產階段會以三種不同的方式失敗。HavenAxis 撞到的是其中一種,另外兩種會在 §35.4 出現。
...
[Ch 33](./ch-33-ai-native-architecture.md) 把 AI-Native 的拓樸講完了,[Ch 34](./ch-34-context-driven-engineering.md) 把 Context-Driven Engineering 的工程實踐講完了。本章把鏡頭縮到 Agent 內部,看這三個子系統各自怎麼設計、怎麼決策、怎麼避免變成那種「答得很流暢但記不住客戶是誰」的健忘聊天機器人。

---

## 35.3 決策框架

### 35.3.1 RAG 三段式 ⸺ Retriever / Reranker / Generator
...
注意三件事:`tenant_id` 在每個 CTE 都要寫(這是 [補章 E](../part-05-quality/chE-compliance.md) §E.4 的硬規,vector index 共用必須靠 row-level filter 隔離);`LIMIT 50` 是經驗值,RRF 在 top-50 內最穩;最終取 top-20 給 reranker,reranker 才取最終 top-5 給 LLM。
...
### 35.3.6 2026 的 long context vs RAG
...
換句話說,2026 的 stack 不是「long context vs RAG」,是「**1M context 是 working memory,RAG 是 long-term knowledge**」⸺ 跟 §35.3.4 的 Memory 三層其實是同一個結構,只是換了視角。
...
> ✅ **修正方向**:寫操作 tool 預設兩段式:第一段 `prepare_transfer` 回傳一個 `confirmation_token` 與 dry-run 預覽;第二段 `execute_transfer(confirmation_token, user_signed_otp)` 才真的執行,且需要使用者額外簽核(OTP / passkey)。Agent 永遠拿不到「直接執行」的權限。這條規則對應 [Ch 25 Security by Design](../part-05-quality/ch-25-security-by-design.md) 與 [補章 E](../part-05-quality/chE-compliance.md) §E.6 的 human oversight 要求。

---

## 35.5 交付清單 ⸺ 一頁式 Agent System Card
...
> 風險等級:Low | Medium | High  ←(對齊 EU AI Act Annex III 與補章 E)
...
四項中先挑一項做的話,建議是最後那一項 ⸺ Card 寫完之後,前面四項的缺口會自然浮上來。下一步可以接 [Ch 36](./ch-36-multi-agent.md) 學多 Agent 協作,或先跳到 [補章 B Agentic QA](./chB-agentic-qa.md) 學怎麼把 Agent 的品質量化(RAGAS [^CIT-358]、TruLens、DeepEval),再回來把 [補章 E](../part-05-quality/chE-compliance.md) 的細粒度存取補上。本章留給你的就是那一頁 Card,把它寫出來,Agent 會少健忘八成。

---

## Cross-References

- **下一章**:[Ch 36 Agent 編排與多代理協作](./ch-36-multi-agent.md) ⸺ 多 Agent 之間的 context 傳遞與職責分工
- **AI 時代的 SA/SD 心法**:[Ch 38 AI-Era SA/SD](./ch-38-ai-eval-drift-redteam.md)
- **品質量化**:[補章 B Agentic QA](./chB-agentic-qa.md)
- **合規與隔離**:[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)
