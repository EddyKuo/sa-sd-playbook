...
# 第 36 章|Multi-Agent 系統設計
## ⸺ 不是更多 Agent 解決問題,是更窄的 Agent 解決問題

> **前置閱讀**:[Ch 33 AI-Native 架構](./ch-33-ai-native-architecture.md)、[Ch 34 Context-Driven Engineering](./ch-34-context-driven-engineering.md)、[Ch 35 RAG / Memory / Tool](./ch-35-rag-memory-tool.md)
> **下游章節**:[Ch 37 工程治理與責任邊界](./ch-37-coding-agent.md)、[Ch 38 AI 時代的成本與容量規劃](./ch-38-ai-eval-drift-redteam.md)
> **延伸補章**:[補章 D 共識、衝突與仲裁](./chD-multi-agent.md)

---

## 36.1 冷觀察 ⸺ 七個 Agent 上線當天,P95 從 8 秒變 47 秒
...
刺耳的不是「拆錯了」,是 ⸺ 這個團隊以為自己在做架構升級,實際上他們把一個邊界不清的 monolithic prompt,**換成七個邊界更不清的 Multi-Agent prompt**。Ch 33 的 NorthVale 是把 LLM 用錯地方;這一章的 Helmsworth 是把 Multi-Agent 拆錯時機。**錯位的不是工具,是判斷拆分時機的能力**。

---

## 36.2 真問題 ⸺ Multi-Agent 不是更多 Agent,是 Anthropic 的決定階梯
...
### 36.2.1 為什麼「拆成 Multi-Agent」常常變成把問題搬家
...
| **Context 不夠(缺知識來源)** | 加 RAG / Memory(Ch 35) | 拆成多個 Agent 不會憑空多出脈絡 |
...
### 36.2.3 Multi-Agent 是進入更高複雜度的決定,不是省事的捷徑
...
## 36.3 決策框架 ⸺ Anthropic 五模式、拆分判準、通訊協定

下面這幾張表跟兩張 Mermaid,在現場相當好用。它們的共同前提是:**Multi-Agent 不是預設選項,是排除其他選項後的最後一站**。

### 36.3.1 Anthropic 五種 Agentic 模式對照表

把 *Building Effective Agents* [^CIT-360] 的七種模式收斂成現場最常用的五種(Augmented LLM 是 Ch 33 的範圍,Autonomous 是補章 D 的範圍,本章聚焦中間五種):
...
| **CrewAI**[^CIT-363] | Crew + Role + Task | 混合 | 上手最快、Role 概念貼近 CDE Ch 34 | 對複雜編排表達力弱 |
...
### 36.3.6 與 CDE Role 的關係:Multi-Agent 是 Role 在 runtime 的具體實現

Ch 34 講過 CDE 三層的 Role 層 ⸺ Orchestrator / PM / SA / RD / DBA / QA / UI-UX 等。把 CDE Role 與 Multi-Agent 放在一起看,關係其實很直接:
...
### 36.3.8 一個最小範例:LangGraph 寫對的 Multi-Agent
...
### 36.3.9 2026 視角:Sub-agents 與 Long Context 的拉鋸
...
> ✅ **修正方向**:在 Sprint Planning 把 Anthropic 的決定階梯貼在白板上 ⸺ Augmented LLM → Workflow(四種)→ Multi-Agent。任何「拆 Multi-Agent」的提案,提案人必須先回答:「**你試過 Prompt Chaining 嗎?Routing 呢?Evaluator-Optimizer 呢?分別卡在哪一個指標?**」三個答案缺一,提案直接退回。把這條紀律寫進 ADR(Ch 30)的「Pre-Decision Checklist」段,每次 Multi-Agent 提案都要對照一次。
...
> ✅ **修正方向**:任何 Multi-Agent 系統(包括 Evaluator-Optimizer 迴圈)必有**三種 Budget Circuit Breaker**:(1)**呼叫次數上限**(每張工單全鏈路 LLM call ≤ N,例如 N=8);(2)**Wall-clock 上限**(全鏈路 ≤ T 秒,例如 T=15s);(3)**Token 上限**(全鏈路 input+output token ≤ K,例如 K=50,000)。任何一條超過,整個鏈路強制停止並降級到「人工接手」或「上一版可用結果」。把這三個閾值寫進 ADR、寫進 Constitution(Ch 34 §34.3),CI 加 fitness function 確保沒有 Agent 設計繞過 budget 檢查。**Budget 不是優化,是安全**。
...
> ✅ **修正方向**:Multi-Agent 的核心優勢**從來不是「多」,是「窄」**。每個 Agent 的 context 應該有明確界定:訂單 Agent 只看訂單上下文(不看物流軌跡)、物流 Agent 只看物流(不看付款明細)、退款 Agent 看訂單 + 付款但不看物流。對應 CDE Skill 三要素(Ch 34 §34.3.2)裡的 Knowledge Sources ⸺ 每個 Agent 有自己的 Knowledge Sources 白名單,**broadcast 是反模式,select 才是預設**。把這條紀律寫成 fitness function:任何 Agent 的 system prompt + tool input 加總 token 數 > 上限,CI 直接 fail。**Multi-Agent 設計失敗的最大徵兆,就是每個 Agent 看到的東西差不多 ⸺ 那等於沒拆**。

---

## 36.5 交付清單 ⸺ Multi-Agent System Card

每次評估「這個系統該不該走 Multi-Agent / 該怎麼拆」,**第一份要產出的不是 LangGraph 架構圖,是 Multi-Agent System Card**。它是一頁 Markdown,寫不滿一頁就是判準沒走完,寫超過一頁通常是把 Skill 細節塞進來了 ⸺ 那些屬於 CDE Skill 文件(Ch 34),不屬於這份 Card。

把它存在 `docs/architecture/multi-agent-system-card-{slug}.md`,跟 ADR 同 repo,跟 AI-Native Vision Card(Ch 33)同層。

````markdown
# Multi-Agent System Card — {子系統名稱}

> 版本:v0.1 | 撰寫日期:YYYY-MM-DD | 擁有人:{名字}
> 對齊:AI-Native Vision Card(Ch 33)、CDE Setup Card(Ch 34)、相關 ADR:{連結}
> 狀態:Draft | Reviewed | Approved

## 1. 模式選擇(勾一個,只能一個主模式)

- [ ] **Augmented LLM**(Ch 33 範圍,本卡不適用)
...
- [ ] **Multi-Agent: Autonomous**(補章 D 範圍,需專案級審查)
...
- [ ] 已評估「Long Context 單 Agent」是否可解(Ch 33 §33.3.5)
...
- 對應 Ch 33 §33.3.3 Audit Trail 三支柱:____
...
至於拆完之後 ⸺ 多個 Agent 之間怎麼**達成共識、處理衝突、設計仲裁機制**(投票 / 階層 / 法庭式裁決等),這已經超出本章一頁 Card 能容納的範圍,留給[補章 D 共識、衝突與仲裁](./chD-multi-agent.md)展開。下一章 [Ch 37 工程治理與責任邊界](./ch-37-coding-agent.md)會接著回答另一個更早出現的問題:**當 Subagent / Multi-Agent 出錯時,誰負責?**

---

## Cross-References

- **回顧**:[Ch 33 AI-Native 架構](./ch-33-ai-native-architecture.md) ⸺ L5 Agent Orchestration 是本章展開的母層
- **回顧**:[Ch 34 Context-Driven Engineering](./ch-34-context-driven-engineering.md) ⸺ Role 是本章 Multi-Agent 在 design-time 的契約
- **回顧**:[Ch 35 RAG / Memory / Tool](./ch-35-rag-memory-tool.md) ⸺ 單 Agent 加 RAG 常常勝過 Multi-Agent
- **下一章**:[Ch 37 工程治理與責任邊界](./ch-37-coding-agent.md) ⸺ Multi-Agent 出錯時的責任歸屬
- **下游**:[Ch 38 AI 時代的成本與容量規劃](./ch-38-ai-eval-drift-redteam.md) ⸺ Budget Circuit Breaker 的容量視角
- **延伸補章**:[補章 D 共識、衝突與仲裁](./chD-multi-agent.md) ⸺ 投票 / 階層 / 仲裁的具體機制
