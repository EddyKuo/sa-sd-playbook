...
- **L2** 在 tool 設計時做(對應 [Ch 38 §35.3.5](./ch-39-rag-memory-tool.md) 的 Tool 四原則 Least Privilege),寫操作預設 dry-run + HITL。
...
每個 AI 功能上線前,**第一份要產出的不是 eval 跑完的 Excel,是 AI Quality Card**。它一頁,寫不滿一頁就是還沒想清楚。把它存在 `docs/agents/{agent-name}-quality.md`,跟 [Ch 38 §35.5](./ch-39-rag-memory-tool.md) 的 Agent System Card 同 repo,跟程式碼同步演化。
