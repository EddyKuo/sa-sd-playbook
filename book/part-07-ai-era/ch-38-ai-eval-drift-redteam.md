...
# 第 38 章|AI 系統的 Eval、Drift 與 Red Team
## ⸺ 把不確定性變成可量測

> **前置閱讀**:[Ch 27 SRE / SLO / Chaos](../part-05-quality/ch-27-sre-slo-chaos.md)、[Ch 33 AI-Native 架構](./ch-33-ai-native-architecture.md)、[Ch 35 RAG / Memory / Tool](./ch-35-rag-memory-tool.md)、[Ch 37 Prompt 工程與成本治理](./ch-37-coding-agent.md)
> **下游章節**:[Ch 39 AI 時代 SA/SD 心法](../part-08-synthesis/ch-39-capstone.md)
> **延伸補章**:[補章 B Agentic QA](./chB-agentic-qa.md)、[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)

---

## 38.1 冷觀察 ⸺ 上線 92%、半年 71%、開發團隊一直不知道

我在 2026 年第二季看過一份事故報告。

虛構亞太數位銀行 **HavenAxis Bank**(`CASE-FIN-010`,沿用 [Ch 35](./ch-35-rag-memory-tool.md) 同一家銀行,但這次故事發生在另一條產品線),旗下虛構合規 AI 助手 **AMLNavigator**,2025 Q3 上線。這套助手的角色不是面對客戶,而是面對 banker:當第一線理專看到一筆「疑似可疑」交易時,在工作站按一個按鈕,LLM 在 12 秒內讀完客戶的 KYC 紀錄、過去 90 天交易、AML 規則庫,給出一段「**這筆交易應送 STR(Suspicious Transaction Report,可疑交易申報)還是放行**」的建議,並附上引用條號(`金管法 [114] 第 9 條`、`FATF Recommendation 10` 等)。
...
### 38.2.2 三軸並行:Eval、Drift、Red Team
...
把這三件事接到 [Ch 27](../part-05-quality/ch-27-sre-slo-chaos.md) 的 SLO / Error Budget 思路,你會發現它們其實是**同一套機制在 LLM 領域的對應物**:Eval 對應 SLI 的 baseline 量測,Drift 對應 SLI 的時間序列監控與 burn rate,Red Team 對應 Chaos Engineering(主動注入故障驗證恢復力)。差別在於,在 LLM 系統,你需要新的工具(eval set、LLM-as-Judge、KS test、PSI、prompt injection 庫)才能把這三件事跑起來。

換句話說,本章的核心命題不是「AI QA 是新東西」,是「**SRE 的方法論可以延伸到 AI,但你需要新的測量工具**」。下面 §38.3 把這些工具一個一個攤開來。

---

## 38.3 決策框架 ⸺ Eval Set 四維、Drift 四種、Red Team 三層、Judge 校準
...
LLM-as-Judge 的更深入處理(包括雙 Agent 對抗評估、自動 rubric 生成),會在 [補章 B Agentic QA](./chB-agentic-qa.md) 詳述。本章只強調一件事:**不校準的 judge,等於用一把沒校準的尺去量品質的退化 ⸺ 你會以為一切還好,直到監理機關來量**。

### 38.3.5 量化指標:Accuracy / Faithfulness / Toxicity / Cost / Latency
...
### 38.3.7 Drift 監測的統計與 embedding 方法
...
- **L2** 在 tool 設計時做(對應 [Ch 35 §35.3.5](./ch-35-rag-memory-tool.md) 的 Tool 四原則 Least Privilege),寫操作預設 dry-run + HITL。
...
> ✅ **修正方向**:**永遠維護一份 human-labeled calibration set**(50–100 筆,SME 標註),每次換 judge / 換 rubric / 換被評模型都跑這份校準集,要求 Cohen's Kappa ≥ 0.6 才放行。對高風險領域(crypto、PEP、醫療診斷)額外切片計算 Kappa ⸺ 整體 0.7 但 crypto 0.4 就不能用。Judge 偏誤的處理在 [補章 B Agentic QA](./chB-agentic-qa.md) 還會深入。
...
> ✅ **修正方向**:**Eval set 必須切片,且每片必須有獨立門檻**。Slice 維度至少包含:地區(SG / HK / TW / JP)、產品線(消金 / 法金 / 投資)、風險等級(low / medium / high)、語言(zh-TW / zh-CN / en / ja)。每片至少 50 題,每片獨立達標才算上線。整體達標、片段不達標 → 阻擋上線,並在覆盤會上強制討論這個落差(這條規則對應 [補章 E Compliance by Design](../part-05-quality/chE-compliance.md) 的 fairness assessment 要求)。

---

## 38.5 交付清單 ⸺ 一頁式 AI Quality Card

每個 AI 功能上線前,**第一份要產出的不是 eval 跑完的 Excel,是 AI Quality Card**。它一頁,寫不滿一頁就是還沒想清楚。把它存在 `docs/agents/{agent-name}-quality.md`,跟 [Ch 35 §35.5](./ch-35-rag-memory-tool.md) 的 Agent System Card 同 repo,跟程式碼同步演化。
...
下一章 [Ch 39 AI 時代 SA/SD 心法](../part-08-synthesis/ch-39-capstone.md) 會把整個 Part VII 收束成「2026 年做架構與分析的人,腦袋裡要先裝什麼」。如果你想先把本章 Eval / Judge / Red Team 的細節跑得更深,可以跳到 [補章 B Agentic QA](./chB-agentic-qa.md) ⸺ 那裡會展開「雙 Agent 對抗評估、自動 rubric 生成、自動紅隊 Agent」這些 2026 才慢慢成熟的做法。本章留給你的就是那一頁 Card,把它寫出來,事故會在它送達前就被攔下。

---

## Cross-References

- **下一章**:[Ch 39 AI 時代 SA/SD 心法](../part-08-synthesis/ch-39-capstone.md) ⸺ Part VII 收束
- **可靠度方法論的母體**:[Ch 27 SRE / SLO / Chaos](../part-05-quality/ch-27-sre-slo-chaos.md) ⸺ Eval / Drift / Red Team 是 SLO / Burn Rate / Chaos 在 LLM 領域的對應物
- **AI 系統架構**:[Ch 33 AI-Native Architecture](./ch-33-ai-native-architecture.md) ⸺ L7 Governance 的展開即本章
- **RAG / Tool 設計**:[Ch 35 RAG / Memory / Tool](./ch-35-rag-memory-tool.md) ⸺ Tool 四原則與 L2 Tool 紅隊的銜接
- **Prompt 與成本**:[Ch 37 Prompt 工程與成本治理](./ch-37-coding-agent.md) ⸺ Prompt drift 監測上游
- **延伸補章**:[補章 B Agentic QA](./chB-agentic-qa.md)、[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)
