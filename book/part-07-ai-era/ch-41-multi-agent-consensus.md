...
這裡有一個重要的銜接點需要明確：當 Bounded Negotiation 不收斂、觸發「升級到 Resolver Agent / Human」時，**這個升級動作本身不是由 Agent 自主決定的**，而是由分散式狀態機的確定性轉換表觸發的。具體而言，系統狀態從 `disputed` 確定性地轉換到 `arbiting`，由更強的 Resolver Agent 或人類接手。這條轉換路徑是預先設計在 transition table 裡的，不是 Agent 在協商失敗後自己「決定」要升級。這個設計區分確保了升級行為的可預測性：無論哪兩個 Agent 協商不收斂，系統都走同一條確定性路徑進入 arbiting 狀態，而不是各自按照不同邏輯發出不同的升級訊號。
