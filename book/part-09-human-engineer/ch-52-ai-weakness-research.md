...
這意味著什麼？一個月掃不出問題，也許只是工具量程外的風險還沒被觸發。六個月都掃不出問題，就開始暗示另一件事：威脅環境可能已經移動到這個工具的認知盲區。連續的「正常」不是安全的累積，它可能只是盲點的累積。把這兩個認知合在一起，得出一個工程結論：**不能用 AI 的掃描結果作為安全狀態的最終評估，而是要主動設計機制來發現 AI 的盲點**。
...
主動研究 AI 弱點，可以從三個層次進行，複雜度遞增。層次一是基本防線，每個組織只要在用 AI 輔助高風險任務，就應該做。層次二是在層次一發現風險後的專業化追蹤——把「知道有問題」轉化成「知道 AI 在哪些具體任務上以什麼方式失敗了」。層次三是針對最高風險任務的主動攻防，當等待失敗自然發生的代價太高時使用。這三個層次不是互斥的選項，而是遞進的防護深度：每一層都以前一層的輸出作為輸入。
...
層次一告訴你「什麼變了」——截止日後出現了哪些新攻擊手法、新法規、新技術模式，以及 AI 對這些變化的認知有多少落差。層次二則系統化地記錄「AI 在哪些具體任務上失敗了」。當層次一的校準檢查發現風險（例如：截止日後出現了新的攻擊類別），層次二就是追蹤這個風險真正實例化的地方——每一次 AI 給出錯誤輸出、被誤判、或被測試出盲點，都應在此留存。沒有層次二，層次一發現的「潛在風險」只會停在抽象層面，無法轉化成可操作的委派邊界調整。
...
層次二的局限在於它只能記錄已經被發現的失敗。但許多盲點在事故發生前永遠不會自然浮出——AI 的弱點往往是靜默的：它不會報錯，只會給出看起來合理但實際上落在盲區的輸出。等待這類失敗在真實任務中自然出現，代價可能是不可逆的。因此需要層次三：主動設計測試場景，在威脅真正來臨前找到盲點。層次三與層次二可以並行運作——每次 Red Team 的發現都會成為新的 Failure Catalog 條目，回饋到層次二；而層次二積累的失敗模式，又為下一次 Red Team 的選題提供方向。
...
每當工程師意識到 AI 安全審查已連續多月沒有新發現，第一份要產出的不是下一份掃描報告，而是一張能追蹤「AI 盲點是否在擴大」的季度校準 Checklist。把它存在 `docs/ai-ops/`，跟程式碼同 repo，跟 README 同層。
...
````markdown
## AI 弱點研究 — 季度校準 Checklist
> 版本:v0.1 | 撰寫日期:2026-03-30 | 擁有人:Kevin (CISO Office)
...
````markdown
## FC-007
> 版本:v0.1 | 撰寫日期:2026-03-22 | 擁有人:Kevin (CISO Office)
