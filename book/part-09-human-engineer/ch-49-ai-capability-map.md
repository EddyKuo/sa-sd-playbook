...
以「安全設計」評估為例：舊模型因為訓練資料截止較早，對某個存取控制設計的評估停留在「可識別已知攻擊模式」，給出「中可靠」的評定，團隊也據此決定 AI 起草、人工複查。但新模型的訓練資料納入了 2025 年下半年公開的新型側信道漏洞資料庫，對同一份設計的評估變得更嚴格——它發現舊答案對這類漏洞完全沒有防禦設計，重新給出「低可靠」的評定。這不是 AI 變差了，而是「評估標準的分母變大了」：舊模型不知道新威脅存在，所以沒有缺口；新模型知道了，所以評分更低。如果團隊沒有意識到模型版本切換，仍然用舊版本對應的「中可靠、AI 起草即可」來處理同類任務，就會在不知情的情況下低估風險。
...
把它存在 `docs/ai-delegation/`，跟程式碼同 repo，跟 README 同層。
...
````markdown
## AI 任務委派可靠性評估
> 版本:v1.0 | 撰寫日期:2026-02-09 | 擁有人:Tech Lead（VaultStack 計費組）
