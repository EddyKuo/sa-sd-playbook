...
AI 生成的程式碼假設「買方 `confirmed` → 更新訂單狀態為 `confirmed`」是一個原子操作，但 PortBridge 的業務邏輯要求這中間必須有賣方的庫存驗證步驟。

這個 gap 是有具體形狀的：Tech Lead 給 AI 的材料包括 OpenAPI spec（定義 `buyer_confirmed` webhook payload 格式）和一份兩頁的功能需求摘要（描述「買方按確認鍵後，訂單進入已確認狀態」）。這兩份文件都正確——但都沒有記載「已確認」對買賣雙方語義不同、以及賣方側需要庫存驗證這個中間步驟。

那條規則存在哪裡？在一份三年前寫的「跨境訂單業務流程說明」PDF 裡，URL 格式是 `confluence.portbridge.internal/pages/12847`，連結已經失效，原作者兩年前離職。現任 Tech Lead 知道「有這個邏輯」，但沒有意識到這件事從來沒有被納入任何 AI 能讀到的文件。

問題的本質不是 AI 讀錯了文件，而是**沒有人知道哪一份文件應該包含這條規則**。它應該在初始 spec 裡嗎？應該在 API 文件的 `buyer_confirmed` 欄位說明裡嗎？應該在 CLAUDE.md 的「常見陷阱」段落嗎？三個選項在覆盤前都沒有人明確決定過。
...
## 53.2 真問題 ⸺ AI 程式碼的 review 需要一個新的認識論
...
> **前提：業務規則清單本身必須先存在。** 這一步的有效性取決於你能否提供一份完整的業務規則清單。PortBridge 的問題正是這份清單不存在——規則分散在失效的 Confluence 連結、離職工程師的記憶、和口頭慣例裡。如果你的團隊目前沒有這份清單，在逆向驗證之前需要先完成一個「隱性規則挖掘」步驟：找 Tech Lead 或 Product Owner，逐一詢問：「這個功能裡，有哪些業務規則是刻意要實作但在 spec 裡不明顯的？」把答案寫下來，這份清單才是逆向驗證的起點，也是事後應補入 CLAUDE.md 或 API 文件的材料。

```
業務規則清單（手工整理，含隱性規則挖掘後補充的項目）：
1. 買方確認 → 觸發賣方庫存驗證 → 驗證通過才更新狀態
   （來源：Tech Lead 口頭確認，原文件在失效 Confluence URL）
2. 驗證失敗 → 訂單進入 pending_seller_review 狀態，不是失敗
   （來源：同上；spec 裡僅寫「驗證失敗需人工介入」，未指定狀態名）
3. 跨時區時，使用買方時間作為確認時間戳
   （來源：OpenAPI spec，buyer_confirmed payload 欄位說明）

在 AI 程式碼裡找：
1. ☐ 庫存驗證步驟在哪裡？→ 找不到（隱性規則，未進入 prompt context）
2. ☐ pending_seller_review 狀態在哪裡？→ 找不到（同上）
3. ☐ 時間戳邏輯在哪裡？→ 找到，在 line 87（有文件，AI 正確實作）
```
...
## 53.5 交付清單 ⸺ 一頁式三層 AI 程式碼審計清單

每次工程師第一次獨立帶含 AI 生成程式碼的 PR 進 review，第一份要交出的不是 review 意見，而是一張填好的三層審計清單——它迫使 reviewer 在開口之前先完成業務規則對照，而不是只看語法與測試通過率。把它存在 `docs/ai-review/`，跟程式碼同 repo，跟 README 同層。
...
### 53.5.1 範例：PortBridge 那個 PR 該被三層審計擋下來的版本

PortBridge（`CASE-ECM-010`）的 240 行 webhook 同步 PR 通過了 36 個測試、通過了傳統 Code Review，唯獨沒有通過第二層意圖符合性。下面是事故覆盤後 Tech Lead 從新流程裡長出的那份檢查表，回填到當時的 PR：

````markdown
## AI 程式碼 Review — 三層審計清單
> 版本:v0.1 | 撰寫日期:2026-02-08 | 擁有人:Tech Lead Wei

PR：#2841 跨時區訂單狀態同步 webhook   審計者：Tech Lead Wei
日期：2026-02-08（事後補簽）

### 第一層：技術正確性
☑ 錯誤路徑:webhook 5xx 與簽章失敗皆有處理（line 132–168）
☑ 邊界:時區、空 payload、重送（idempotency-key）已驗
☑ 安全:HMAC 驗章、白名單來源 IP

### 第二層：意圖符合性
<!-- 為什麼這欄:這層才是這個 PR 真正的事故來源;沒這層,測試 36 個全綠也擋不住「已確認」的雙語意。 -->
☐→☑ AI 隱性假設已詢問:Claude 回答「假設 buyer_confirmed → 直接更新訂單為 confirmed，
   未在 spec 看到中間驗證步驟」← **這就是 gap**
☐→☑ 業務規則對照（手工從三年前的跨境流程 PDF 整理）：
   - 規則 1:買方確認須觸發賣方庫存驗證 → 程式碼**找不到**，待補
   - 規則 2:驗證失敗應入 `pending_seller_review` → **找不到**，待補
   - 規則 3:跨時區用買方時間做時間戳 → 找到，line 87
☐→☑ 測試業務覆蓋率:36 個測試覆蓋技術路徑，**0 個對應規則 1/2**

### 第三層：架構符合性
<!-- 為什麼這欄:AI 在局部寫得乾淨,卻常在跨 BC 邊界處越線;
     沒這條,Order BC 與 Inventory BC 的耦合方向會被默默反轉。 -->
☐→☑ 依賴方向:webhook handler 直接寫 inventory 表 → **違反 BC 邊界**，須改走 InventoryService
☑ CLAUDE.md 禁止事項:無新增違規（既有規則未觸及這條業務語義）
☑ 命名:符合既有慣例

### 後續行動
☑ CLAUDE.md 新增「常見陷阱」:`buyer_confirmed` 與 `seller_confirmed` 為兩階段，
   中間必須過庫存驗證
☑ Failure Catalog 新增 FC-014:同類「跨系統同名狀態」未來一律列為意圖符合性高風險
☑ 該 PR 退回，補 18 個業務規則測試後重提
````
**意圖符合性那一格沒填,程式碼上線就是賭**;填了,gap 在合入前就會被命名成具體的兩條規則。三週後客服信箱裡的那個問題,提前在 review 階段付清。
