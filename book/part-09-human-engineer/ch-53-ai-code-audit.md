...
> **前提：業務規則清單本身必須先存在。** 這一步的有效性取決於你能否提供一份完整的業務規則清單。PortBridge 的問題正是這份清單不存在——規則分散在失效的 Confluence 連結、離職工程師的記憶、和口頭慣例裡。
>
> **如果業務規則清單目前不存在，請先做「隱性規則挖掘」**，再進行逆向驗證：
>
> 1. **與誰溝通**：PM（確認產品語義邊界）、Tech Lead（確認過去實作決策）、Product Owner（確認業務優先順序）。三者缺一則至少要有 Tech Lead，他最清楚哪些規則只存在於口頭慣例。
> 2. **問什麼問題**：準備以下問題清單逐一詢問：
>    - 「這個功能裡，有哪些刻意的實作細節但 spec 裡沒有明確寫？」
>    - 「上次有人問過類似問題嗎？當時怎麼解釋？」
>    - 「這個功能如果壞掉，客服最可能接到哪種投訴？那個情境的前置條件是什麼？」
> 3. **時間預期**：單功能的挖掘通常需要 30–60 分鐘訪談。隱性規則多的系統（跨部門業務流程、有歷史包袱的狀態機）可能需要兩輪。
> 4. **產出物格式與存檔**：訪談後整理成一份「業務規則補充清單」，格式參照上面的業務規則清單範例（規則描述 + 來源 + 對應 spec 位置或「口頭確認，待補文件」）。存放在 `docs/ai-review/business-rules-{feature}.md`，並同步評估是否需要補入 CLAUDE.md 的「常見陷阱」段落或 API 文件的欄位說明。
...
PortBridge（`CASE-ECM-010`）的 240 行 webhook 同步 PR 通過了 36 個測試、通過了傳統 Code Review，唯獨沒有通過第二層意圖符合性。下面是事故覆盤後 Tech Lead 按新流程回填當時的 PR 記錄，模擬**假如當時就有三層審計流程，應得到的審計結果**——這是一份「事後模擬回填」，讓讀者看清楚意圖符合性 gap 在哪個步驟應被攔截：
...
日期：2026-02-08（模擬回填：如果當時即時執行三層審計應得到的結果）
...
如果資源有限無法同時實施三層，建議優先在**第二層補足業務規則清單和逆向驗證**（先列出清單，再逐條找程式碼對應位置），理由是這個動作讓「意圖符合性」從抽象概念變成具體的可操作步驟，五分鐘就能有第一個輸出。但不應因此省略**第一層的高風險區檢查**——錯誤路徑、邊界條件、安全假設是 AI 生成程式碼最常見的靜默失敗點，不做這三項，即使業務規則清單完整，上線仍有技術風險。三層的優先順序是：先做第二層讓你知道「做的是不是對的事」，再做第一層讓你知道「對的事做得安不安全」，第三層則防止局部正確的程式碼破壞整體架構。
