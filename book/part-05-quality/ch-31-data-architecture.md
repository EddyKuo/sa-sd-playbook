...
## 31.1 冷觀察 ⸺ 把 Lakehouse 用成「更貴的 SFTP」
...
這句話指向的是「只買工具、不做組織改造」這個失敗路徑。但值得注意的是,反面也成立:**只做組織改造、完全不補儲存層基礎設施**,同樣會卡住。在實際觀察到的 Data Mesh 落地案例中,成功的組織通常不是「組織 100% 到位、再動工具」⸺ 它們是**組織改造與基礎設施投資同步推進**,只是節奏不同。

MedNexus 的案例有一個關鍵前提:**他們 Delta Lake 與 Unity Catalog 已經存在了三年,儲存層的問題不是沒有工具,是工具有了但沒人填 metadata、沒人對 schema 負責**。所以他們的正確路是「組織層先走、儲存層先不動」。這個結論是 MedNexus 情境下的對的答案,但不是通則。

更完整的判準如下:

- **已有 Lakehouse 基礎設施(有 Delta / Iceberg,有 catalog)**:治理欠缺是主因 → 組織層先走,補 Data Mesh 四原則,儲存層可暫不動
- **從零開始(CSV + SFTP,沒有任何平台投資)**:組織與基礎設施同步進行,但輕量起步 ⸺ 先把 domain data PO 指名、同步建一個最小可行的 ELT 管線(dbt + 一個 Iceberg catalog),不需要等組織 100% 到位才動工具
- **儲存層混亂(三套格式並存、沒有統一 catalog)**:先收斂儲存層,治理討論建立在穩定的基礎上才有意義

**「組織優先」是方向,不是序列。** 最常見的誤讀是把它變成「組織不到位、工具一行不能寫」⸺ 這樣做的團隊通常在第六個月遇到「組織層還沒到位、業務已經在燒」的壓力,然後在慌亂中一次性把所有工具全上,反而更混亂。

### 31.2.2 儲存層 = Warehouse / Lake / Lakehouse / Lakebase
...
### 31.2.3 存取層 = ETL / ELT / Reverse ETL / Streaming
...
### 31.2.4 三層問題的混淆,是 MedNexus 事故的根因
...
MedNexus 那場 retro 後,他們把這張表貼到牆上,第一個發現是:**組織層那一欄整欄是空的**。後來他們花了 14 個月做組織層(下文 §31.5 交付清單會還原他們的 Data Architecture Decision Card),儲存層幾乎沒動。這個選擇是有前提的:他們的儲存層(Delta + Unity Catalog)是能用的,三年的平台投資並沒有白費,缺的是使用那個基礎設施的治理機制。如果他們從零開始、連 catalog 都沒有,組織層與儲存層就需要同步起步。

### 31.3.2 Data Mesh 四原則 — 不是教條,是體檢清單

這張表有兩個欄位:第三年體檢時的狀態(開始補救前),以及 14 個月組織層工作之後的進度(第四年 Q2)。Mesh 不是一個你「做到」或「沒做到」的二元狀態,是一個漸進移動的光譜。

| 原則 | 它在處理什麼 | 落地檢核 | MedNexus 第三年體檢(補救前) | MedNexus 第四年 Q2(14 個月後) |
|---|---|---|---|---|
| **Domain Ownership** | 中央 team 撈不動所有 domain 的資料專業 | 每個 domain 有指定 data PO、有獨立 backlog | ✗ 全部由中央平台 team(16 人)消化 | △ 七家醫院各指 1 名 Data PO,名單已進 `owners.yaml`;backlog 各院自理,品質還不穩定 |
| **Data as a Product** | Domain 不只丟 raw 資料,要把它做成「能被外部消費的產品」 | 每份 product 有名字、版本、SLO、文件、發現性 | ✗ 七家醫院只丟 CSV,沒人寫文件 | △ 每院先發 1 個 `emr-discharge-summary` v0.1,SLO 有寫但還沒 enforce |
| **Self-Serve Platform** | 中央 team 提供「讓 domain 能自助發布資料產品」的能力 | Domain team 不需要中央 team 的人力就能 publish | ✗ 任何資料變動都要中央 team 改 PySpark | ✗ `dbt-template-emr` runbook 寫了,各院還在學,仍需中央人力協助 |
| **Federated Governance** | 跨 domain 的治理規則(隱私、品質、契約)機讀化、可執行 | Data Contract 進 CI、隱私規則自動稽核、Schema Registry | ✗ Unity Catalog 開了帳號沒人填 metadata | ✗ Data Contract YAML 草稿已寫,CI 鉤子預計 Q3 上線,尚未生效 |

**這個進度說明了什麼?** 14 個月之後,四項中有兩項從 ✗ 移到 △(進行中),兩項仍在 ✗。這不是失敗,這是 Data Mesh 的真實移動速度:組織習慣的改變比工具的部署慢一個量級。MedNexus 的 PM 在第四年 Q2 的回顧上說:「我們還沒有 Mesh,但我們現在知道每個欄位對不上時,應該打電話給誰。」⸺ 這就是這 14 個月的具體產出。

**現場常用的拇指法則**:**這四項至少要有兩項真實落地(✓ 或穩定 △),才能說「我們在做 Data Mesh」**。其中 Domain Ownership 是入口 ⸺ 沒有它,後面三項都沒地方扎根。

### 31.3.3 Lakehouse 三大開源格式對照(2026)
...
### 31.3.4 ETL / ELT / Reverse ETL / Streaming 四模式對照
...
注意:這張決策樹是「找最痛的層先下手」的引導,不是「必須等組織層 100% 完成才能動儲存層」的封鎖。實際落地通常是並行推進:組織層節奏較慢(六到十八個月)、儲存層改造在評估完畢後可以同步開始,但優先順序與資源分配需要明確區分。**「組織優先」意思是組織層的決定不能被工具決定取代,不是組織先做完才能碰工具**。
...
MedNexus 把這張卡釘在會議室白板上 14 個月。儲存層沒動一行,但七家醫院終於有 Data PO 在群組裡接「這欄是誰填的」這種訊息。四個 Data Mesh 原則裡,Domain Ownership 和 Data as a Product 從全失進到進行中;Self-Serve Platform 和 Federated Governance CI 仍在建設。這不是「Data Mesh 完成了」⸺ 這是「Data Mesh 開始了」。

**資料架構的成熟度不是看用了什麼工具,是看出問題時找不找得到人**。MedNexus 的案例同時說明了另一件事:找得到人,是 14 個月組織工作的起點,不是終點。
