...
STRIDE 在這裡仍然是基礎 ⸺ Spoofing(身份偽造)、Tampering(資料竄改)、Repudiation(否認)、Information Disclosure(資訊洩漏)、DoS、Elevation of Privilege 這六個維度,對 LLM 的 API 端點、工具呼叫介面、輸出 pipeline 同樣適用。**但 LLM 引入了四類 STRIDE 沒有命名的新攻擊面**:攻擊者透過文字輸入改變 LLM 行為(Prompt Injection)、誘導 LLM 執行不該執行的工具(Tool Misuse)、探測 system prompt 或訓練資料(Model Exfiltration)、把 LLM 輸出當可信內容直接執行(Insecure Output Handling)。OWASP LLM Top 10 2024[^CIT-256] 把這四類明確命名,讓它們能被識別、被討論、被緩解。
...
到了 2026,Threat Modeling 多了一個新層。當系統開始呼叫 LLM、開始把工具交給 Agent,STRIDE 仍然是第一層基礎 ⸺ 對 LLM 的 API 端點、工具呼叫介面、輸出 pipeline 跑 STRIDE 六維依然有效。但 LLM 的輸入 / 輸出 / 上下文都是動態文本,引入了四類 STRIDE 六個維度沒有命名、傳統 Threat Model 沒有覆蓋的新攻擊面,需要在 STRIDE 之後額外跑一層 LLM Top 10。

把 OWASP LLM Top 10 2024[^CIT-256] 收斂成四種對 SA/SD 影響最深的威脅類型,作為 STRIDE 的第二層補充:
