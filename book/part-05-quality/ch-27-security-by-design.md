...
> **前置閱讀**:[Ch 13 架構風格](../part-03-design/ch-13-architecture-styles.md)、[Ch 21 微服務拆分判準](../part-04-architecture/ch-21-microservices.md)、[Ch 24 Service Mesh 與 Cell-Based](../part-04-architecture/ch-24-service-mesh-cell-based.md)
> **下游章節**:[Ch 26 可靠性](./ch-27-sre-slo-chaos.md)、[Ch 27 可觀測性](./ch-26-observability-otel.md)、[Ch 33 AI-Native 架構](../part-07-ai-era/ch-33-ai-native-architecture.md)
> **延伸補章**:[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)(緊接本章後)

---

## 25.1 冷觀察 ⸺ KYC 通過,戶口被搬空 NT$280 萬
...
把這個鏈拆開來,失敗點不只一個。**第一個是 OAuth 設計**:refresh token 60 天有效,但發出時沒綁 device fingerprint(IP / device ID / TLS fingerprint 任一個都沒綁),換一台手機不會被發現。**第二個是客服降級流程**:用戶說「換手機登不進去」,客服只驗證了身分證末四碼 + 生日 ⸺ 兩項都是公開資料,在台灣這兩項可以從外洩的物流資料、租屋平台、學校通訊錄拼出來。**第三個是風控規則**:跨境匯款的限額是 USD 5,000 / 日 / 帳戶,攻擊者每筆控制在 USD 4,800,**沒一筆超過閾值**。三個失敗點各自看起來都不嚴重,組合起來就是 NT$280 萬。
...
## 25.2 真問題 ⸺ Security by Design 不是清單,是預設值的選擇
...
### 25.2.3 Threat Modeling 不是檢核表,是設計階段的對話
...
到了 2026,這件事多了一個新戰場:**AI 系統的 Threat Modeling**。當你的系統開始呼叫 LLM、開始有 Agent 自主決策、開始把工具(database / API / 檔案系統)交給 LLM 呼叫,Threat Model 的邊界就從「程式碼周邊」擴展到「Agent 的工具呼叫鏈」。Prompt Injection、Tool Misuse、Model Exfiltration 這些 OWASP LLM Top 10 2024[^CIT-256] 列出的威脅,沒有任何一條能用傳統 STRIDE 完整涵蓋 ⸺ 你需要把 STRIDE 擴張一個維度。這件事 § 25.3.5 會展開。

---

## 25.3 決策框架 ⸺ STRIDE、Zero Trust、Threat Modeling 工作坊
...
### 25.3.3 Zero Trust 五原則(NIST SP 800-207)
...
### 25.3.4 Secrets 管理與供應鏈安全工具對照

Secrets 管理與供應鏈安全是 Security by Design 兩個常被忽視但代價最高的領域。把現場常見工具對照一次:
...
### 25.3.5 AI 系統威脅地圖(OWASP LLM Top 10 2024)

到了 2026,Threat Modeling 多了一個新維度。當系統開始呼叫 LLM、開始把工具交給 Agent,STRIDE 六個維度不夠用 ⸺ 因為 LLM 的輸入 / 輸出 / 上下文都是動態文本,傳統威脅模型沒涵蓋。

把 OWASP LLM Top 10 2024[^CIT-256] 收斂成四種對 SA/SD 影響最深的威脅類型:
...
### 25.3.8 Threat Modeling 工作坊四步

把上述決策框架壓成一場 2 小時的工作坊,按下面四步跑:

1. **Step 1(20 分鐘)畫系統 Context**:用 C4 Level 1/2 的圖,標出每個 trust boundary(用戶 ↔ 系統、系統 ↔ 第三方、服務 ↔ 服務、人類 ↔ Agent)。
2. **Step 2(40 分鐘)跑 STRIDE + LLM Top 10**:每條跨 trust boundary 的線,六個維度走一次;每個 Agent 工具呼叫,LLM Top 10 四種跑一次。寫在便利貼上貼到圖旁邊。
...
## 25.5 交付清單 ⸺ 一頁式 Threat Model Card
...
接下來,**緊接本章的[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)** 會把這套姿態推到下一層 ⸺ 當合規不只是 OWASP / STRIDE 這種「業界共識」,而是 PCI DSS 4.0、SOC 2、ISO 27001、GDPR、台灣個資法、新加坡 PDPA、香港 PDPO 這些「條文等級」要求時,Threat Model 與 Zero Trust 的設計選擇要怎麼對應到稽核項目、怎麼產出可被外部稽核員讀懂的證據。PocketRail 的故事在補章 E 還會繼續,因為他們事故後做完安全重構,接著就要面對新加坡 MAS 的稽核 ⸺ 那是另一個層次的對話。

---

## Cross-References

- **回顧**:[Ch 13 架構風格](../part-03-design/ch-13-architecture-styles.md) ⸺ 安全的責任要落在哪一層架構;[Ch 21 微服務拆分判準](../part-04-architecture/ch-21-microservices.md) ⸺ 服務邊界即是 trust boundary;[Ch 24 Service Mesh](../part-04-architecture/ch-24-service-mesh-cell-based.md) ⸺ mTLS 與 authz policy 落在 Mesh 層
- **下一章**:[Ch 26 可靠性](./ch-27-sre-slo-chaos.md) ⸺ DoS 緩解與 SLO 的接續;[Ch 27 可觀測性](./ch-26-observability-otel.md) ⸺ audit log 與 anomaly detection 的訊號設計
- **AI 時代延伸**:[Ch 33 AI-Native 架構](../part-07-ai-era/ch-33-ai-native-architecture.md) ⸺ Agent 工具呼叫鏈的安全邊界
- **緊接補章**:[補章 E Compliance by Design](../part-05-quality/chE-compliance.md) ⸺ 從 Threat Model 到合規稽核證據
