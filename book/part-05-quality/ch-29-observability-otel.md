...
換句話說,**三大支柱真正的價值不是各自的內容,是它們之間的 correlation**。可觀測性的目標不是「dashboard 多」,是「事故當下能用一個 trace_id 把因果鏈組合起來」。不過,這個「用 trace_id 把因果鏈組合起來」的前提是:事故相關的 trace 必須有被採樣進儲存。採樣策略的缺失會直接影響 correlation 的完整性 ⸺ §29.3.3 會詳細講解如何確保重要事件的 trace 一定進得了儲存。
...
4. **Dashboard 是給人看的,事故響應的第一棒正在轉移到自動化**。部分先進團隊已經在走「SLO burn rate 自動觸發 → AI runbook 第一次診斷 → 人介入」的流程,Dashboard 在這個流程裡的角色被推到很後面。這個趨勢本章不做深入討論,Ch 40(Multi-Agent 系統設計)會帶具體案例;本章只需要知道:可觀測性資料若沒有 correlation,無論是人還是自動化工具都拼不出事故故事。
...
3. **Baseline 採樣保留統計代表性**。`random-baseline: 1%` 的目的不是「保留所有正常請求的故事」,而是「讓 P50 / P95 基準線不會因為全部丟棄而失真」。事故當下你關心的不是 99% 的正常請求細節,而是異常請求的 trace。但 1% 不是固定值:如果事故的症狀是「平均延遲劣化」而非明顯的錯誤(例如 P99 從 80ms 漂移到 200ms,但 5xx 沒上升),error / slow-request 策略就無法捕捉到這些 trace,此時 baseline 採樣率需要提高到 5–10%,成本隨之增加。換言之,**baseline 比例是一個成本與可見性之間的取捨決策,要根據你最怕漏掉的事故類型來調整**。
...
在進入概念之前,先把這三個東西放回 §29.3.4 的拓樸位置:Trace ID 和 Span ID 由 **SDK 層**在每筆請求進來時自動產生和維護;Baggage 由業務應用在 SDK 層顯式設定(如下面程式碼中的 `baggage.set_baggage`)。兩者都透過 W3C `traceparent` header 在服務呼叫間傳遞。當訊號進入 Collector 的 Receiver 後,Trace ID / Span ID 保持原樣,Processor 可以讀取它們來做 tail-sampling 決策(例如「這條 trace 有 ERROR span 就保留」),最終由 Exporter 導出到 Tempo / Jaeger 等後端儲存。了解這個流程可以避免一個常見誤解:**Trace ID 是 SDK 產生的,不是 Collector 產生的;Collector 只負責路由與過濾**。
...
具體的操作流程:在 Grafana Tempo 裡用 TraceQL 下這一條查詢:

```
{ span.error.kind = "third_party_shipping_503" && span.http.status_code >= 500 }
```

Tempo 會列出所有符合條件的 trace;點開其中一條,就能看到完整的跨服務因果鏈 ⸺ 從前端的 `POST /checkout` 一路到 `shipping-quote → 第三方物流 API` 的 503,每個 span 的耗時、parent/child 關係一目了然。如果你用的是 Jaeger,同樣的查詢在「Search」頁面選 `Service: checkout-service`、Tags 填 `error.kind=third_party_shipping_503` 即可。這就是「業務語意 attribute → 事故當下一條查詢定位根因」的完整閉環。
...
> ✅ **修正方向**:所有結構化 log 強制 `trace_id`、`span_id`、`correlation_id` 三個欄位(correlation_id 用於跨非同步邊界的事件鏈)。SDK 層用 OTel logging instrumentation 自動注入,業務碼不需要手動加。Fitness function 在 CI 跑「隨機抽 100 筆 log,trace_id 覆蓋率 < 99% 擋 PR」:實作方式可以是一個 ~50 行的 Go 或 Python 腳本,在 CI 的整合測試跑完後,從測試期間輸出的 log 檔案(JSONL 格式)中隨機取樣 100 行,用 JSON parser 檢查每行是否包含非空的 `trace_id` 欄位,低於 99% 就以非零退出碼中止 PR。需要注意的是:若開發環境的測試只覆蓋部分服務路徑,Log 樣本可能不具代表性 ⸺ 建議只在有完整 e2e 整合測試環境的 pipeline 中啟用這條檢查,而不是在 unit test 階段。具體工具位置可連結到 §29.5 Observability Plane Card 的 Correlation 強制檢查區塊,讓 Owner 欄位指向維護這個腳本的 team。判準:任何事故,5 分鐘內能用 trace_id 把 logs / metrics / traces 三邊串起來;串不起來就是這條沒做。
