...
> **前置閱讀**:[Ch 21 微服務架構](../part-04-architecture/ch-21-microservices.md)、[Ch 23 Kubernetes 實戰](../part-04-architecture/ch-23-cloud-native-kubernetes.md)
> **下游章節**:[Ch 27 SRE 與 SLO](./ch-27-sre-slo-chaos.md)、[Ch 30 ADR](../part-06-engineering/ch-30-adr-architecture-knowledge.md)、[Ch 36 Multi-Agent 系統設計](../part-07-ai-era/ch-36-multi-agent.md)
> **延伸補章**:無

---

## 26.1 冷觀察 ⸺ 12 個 dashboard 全綠,結帳成功率掉 12%
...
HarborPick 那 12 個 dashboard,本質上做的全是監控:**每一張都在回答某位工程師當初想得到的問題**。但黑五那天的問題是「7% 的 checkout 在第三方物流 503 時被吞」,這個問題在 dashboard 設計的時候,沒有人想得到,也沒有任何 alert 設給它。

### 26.2.2 三大支柱不是「多三種資料」,是「同一個事件的三個切片」
...
到 2025 / 2026,業界的共識是:**三大支柱必須收斂為「同一個事件的三個切片」⸺ 用 trace_id 當作主鍵**。這不是「把三個工具放在同一個 UI」(那只是視覺收斂),是「同一筆 checkout 請求在 logs / metrics / traces 三邊都能用同一個 ID 查到」。
...
### 26.2.3 為什麼會出現「Dashboard 牆但沒人看」這個現象
...
## 26.3 決策框架 ⸺ 三大支柱、Sampling、OTel 拓樸、加哪個支柱
...
### 26.3.2 RED vs USE vs Golden Signals 對照
...
Honeycomb[^CIT-264] 在 2024 年的研究指出,**Tail-based Sampling 在大規模系統下是「能不能看到事故根因」的決定性差異**。HarborPick 用的是 head-based 1% 採樣,7% 失敗事件在採樣後幾乎全部丟失。改成 tail-based「100% 保留 status_code >= 500、duration > 2s、特定 partner 的 trace,其餘 1%」之後,事故定位時間從 2 小時 14 分降到 11 分鐘。

現場常用的判準:**日均 trace > 1 億條就該認真考慮 tail-based**。低於這個量級,head-based 高採樣率(5–10%)+ 把錯誤 / 慢請求設 always sample,通常就夠用。

### 26.3.4 OpenTelemetry 架構
...
### 26.3.5 Distributed Tracing:Trace ID / Span ID / Baggage
...
### 26.3.6 OTel Collector Pipeline 配置範例
...
這份配置的精神有三條:**降基數在 Collector 統一做、tail-sampling 規則寫進 yaml 而不是寫進 SDK、三大支柱進不同 pipeline 但共用 receivers**。降基數寫在這裡,而不是寫在每個 service 的 SDK,意味著「拿掉 user_id」是平台決策不是業務決策 ⸺ 這個分工很重要,§26.4 反模式 3 會講。

### 26.3.7 「這次該加哪一個支柱」決策樹
...
### 26.3.8 SLI / SLO / SLA / Error Budget(下游 Ch 27 接續)
...
可觀測性提供 SLI 的原始資料,SLO / Error Budget 是這份資料的政策層。Ch 27 會把這條接完,本章只把可觀測性與 SLI 的關係講清楚:**沒有可觀測性,SLO 沒有可信的 SLI;沒有 correlation,SLI 拿來算 SLO 也只是數字遊戲**。
...
> ✅ **修正方向**:採樣策略分三層:(1)錯誤、慢請求(> P99)、特定關鍵屬性 100% 保留;(2)正常請求 head-based 高採樣率(視成本訂 5–10%);(3)流量極大時用 tail-based 在 Collector 端決策。Tail-based 規則寫在 Collector yaml(§26.3.6 範例),不寫在 SDK。判準:每月做一次「事故 trace 是否被採到」回測,如果有事故的關鍵 trace 沒進儲存,sampling 規則就要加。
...
> ✅ **修正方向**:cardinality 預算是平台級決策,不是業務級決策。原則:(1)高基數 ID(user_id / order_id / trace_id)走 logs / traces 不走 metrics;(2)中基數維度(country / partner / tier)上 metrics,但定期 review;(3)低基數維度(http_method / status_class)隨意。Collector 的 attributes processor 統一執行降基數規則(§26.3.6 範例),禁止應用層自己決定。判準:每季 audit 一次 metrics cardinality,top 10 高基數 metric 都要有人說明為什麼這樣設。
...
- [ ] 用 §26.3.1 三大支柱適用情境表 + §26.3.7 決策樹回答「這個觀測需求該加哪個支柱」⸺ unknown unknown 一律配 traces + 結構化 logs,known + 趨勢才上 metrics
...
四項中先挑一項做完就好,建議從最後那一項 ⸺ **把現有觀測棧的 Plane Card 補一張,寫不出 Cardinality 預算或寫不出 Correlation 檢查的那個區塊,就是這次該優先補的方向**。Ch 27 會把 SRE / SLO / Error Budget 接完,讓這份 SLI 變成可執行的政策;Ch 30 會講 ADR 怎麼把觀測決策制度化;Ch 36 會把 AI Agent observability 接到同一個 OTel 平面 ⸺ 三大支柱的故事在 2026 年又多了 LLM 這條鏈,但底層機制是一樣的。

---

## Cross-References

- **回顧**:[Ch 21 微服務架構](../part-04-architecture/ch-21-microservices.md) ⸺ 微服務的觀測稅金在這裡付;[Ch 23 Kubernetes 實戰](../part-04-architecture/ch-23-cloud-native-kubernetes.md) ⸺ K8s 拓樸下的觀測注入
- **下一章**:[Ch 27 SRE 與 SLO](./ch-27-sre-slo-chaos.md) ⸺ 把本章的 SLI 接成 SLO / Error Budget / 燃燒率告警
- **決策制度化**:[Ch 30 架構決策紀錄(ADR)](../part-06-engineering/ch-30-adr-architecture-knowledge.md) ⸺ 觀測棧選型、Sampling 策略寫成 ADR
- **AI Agent 觀測**:[Ch 36 Multi-Agent 系統設計](../part-07-ai-era/ch-36-multi-agent.md) ⸺ LLM trace 接到同一個 OTel 平面
