...
## 30.1 冷觀察 ⸺ 99.95% 寫在合約上、99.62% 跑在現實裡
...
## 30.2 真問題 ⸺ Error Budget 是上線速度與可靠度的可協商預算
...
### 30.2.1 SRE 的核心不是「值班」,是「可靠度被當成功能管理」
...
### 30.2.2 SLI / SLO / SLA / Error Budget 是同一條鏈的四個位置
...
Google SRE Book 給的起始拇指法則是:**SLO 比 SLA 嚴格一個數量級**。如果 SLA 是 99.95%(允許 21.6 分鐘/月),SLO 就訂 99.9%(允許 43.2 分鐘/月)。目的是**留一條工程緩衝線,在違反客戶承諾之前先觸發內部凍結**。PolicyPilot 把 SLA 跟 SLO 寫成同一個數字,等於把工程的內部紀律跟法律承諾畫等號 ⸺ 沒留任何安全餘量。

但這個「一個數量級」是起始啟發,不是通用規則。**實際間距應依團隊的可觀測性成熟度與事故應對速度校準**:

- **觀測力強 + 自動化補救到位**:0.05% 的間距(99.95% SLA vs. 99.9% SLO)或許已經足夠 ⸺ 因為系統能在燒穿 SLA 之前自動偵測並隔離問題。
- **手動 on-call + 觀測力薄弱**:即使留 0.1% 間距也可能不夠,因為從 soft-freeze 觸發到人工干預,可能已經超出緩衝時間;這時 SLO 必須更保守。
- **新創初期**:訂 99.9% SLO 而實際達成率只有 99.6%,等於 soft-freeze 永遠在觸發,逐漸變成「警報疲勞」⸺ 工程團隊開始無視觸發條件,Error Budget 機制整個失效。

判準:**每季回顧 Error Budget 的觸發頻率**。連續兩季 soft-freeze 從未觸發,間距可能太鬆;連續兩季 Budget 在前六週就燒完,間距可能太緊或服務本身需要更深的工程投資。間距不是一次設好就不動的,是需要季度校準的工程決策。

換句話說,**SLO 不是 SLA**。SLO 是給工程內部的紀律,SLA 是給法務的承諾。兩者方向相反:SLO 用來「自我約束」,SLA 用來「自我曝險」。把這兩個分開,是 Error Budget 機制能運作的前提。

### 30.2.3 Error Budget 是預算,不是道德
...
### 30.2.4 Chaos Engineering 不是「上 prod 搞壞東西」
...
「感覺判斷」為什麼會失效?舉個具體的例子:演練期間刪掉 3 個 Pod,10 分鐘後 K8s 自動恢復。如果沒有 SLO,團隊在覆盤時可能說「恢復挺快的,沒什麼問題」——因為確實「感覺」系統撐住了。但若有 SLO(可用率 ≥ 99.9%,28 天滾動),同樣的 10 分鐘會在 Prometheus 上顯示:**燃燒率 14.4 倍,已消耗本週 Error Budget 的 18 分鐘,超出 fast burn 閾值**。這代表演練方式需要改進——例如先在凌晨低峰期進行、縮小爆破半徑、或補齊 Pod Disruption Budget 設定。「感覺」與「量化」的差距,就是有沒有 SLO 的差距。
...
PolicyPilot 在這張光譜上落在 **Pre-L1**:他們確實有 on-call 輪值、會做事故分流、事後也寫 postmortem ⸺ 但這些都是非正式的、依賴個人判斷的行為,不構成 L1 所要求的「documented runbook」。

L1 的入場券不是「有人會應變」,而是**有三份可查驗的文件**:
1. 事故分級定義(P0/P1/P2 的條件與 SLA 時限)
2. 事故應變決策樹(誰先看什麼、升級條件是什麼)
3. 升級路徑與時間閾值(On-Call → Tech Lead → EM,各層 SLA)

PolicyPilot 的 on-call 工程師是憑經驗決定「這個告警要叫誰」,下一個接班的人不知道判斷依據,整個流程活在幾個人的腦子裡。**有人在值班,不等於有 runbook;有 postmortem,不等於有分級定義**。這正是 Pre-L1 的特徵:應變靠的是英雄主義,而不是可複現的程序。

新創團隊常見的錯誤是聽完 Netflix Chaos Monkey 演講後想直接做 L4,結果第一週就把 production 弄壞,接下來半年再也不敢提這個詞。**Chaos 是培養出來的肌肉,不是買來的**。

即使你的團隊目前在 Pre-L1 或 L1,也不代表無法展開演練規劃——只是優先序不同。下面這張決策樹的設計是:**無論在哪個階段,都能從「本週該排什麼」這個問題找到落腳點**。Pre-L1 / L1 的答案通常是「先補 runbook(對應決策樹的『過去 90 天有事故嗎』分支)」,把事故根因轉換成可重複的演練假設;待 runbook 補齊、SLO 運作穩定之後,再進入 L2 Adversarial 階段。

### 30.3.4 「這次該演練哪個容錯模式」決策樹
...
判準:**長駐 API 用 HPA、事件密集用 KEDA、Stateful 用 VPA,三者不互斥但別讓 HPA / VPA 同時看 CPU**(會抖動)。KEDA 在 2026 年事件驅動場景的採用率持續上升,主因是它能 scale-to-zero,把閒置成本壓到接近 0;HPA 的最低副本數是 1,無法做到同等效果。

面對**混合型負載**時,選型往往不是「只選一個」。以 PolicyPilot 的 underwriting API 為例:日間是高並發 CPU-bound 的 HTTP 核保請求,適合用 HPA 以 CPU utilization 為訊號動態擴展副本;夜間則有跑批次的佣金計算任務,流量突發但可預期,適合用 KEDA 搭配 Cron scaler 在固定時段預熱、任務結束後縮回零。這兩條擴展邏輯應拆開設計:HPA 掛在長駐的 `underwriting-api` Deployment 上,KEDA Cron scaler 另起獨立的 `commission-batch` Job ⸺ 避免兩組 scaler 搶同一個 Deployment 的 replicas 造成互相干擾。

### 30.3.6 程式碼長相:Prometheus SLO Recording Rule

把 § 30.3.1 那條 availability SLO(success_rate ≥ 99.9% / 28d rolling)寫成 Prometheus(2.55+)recording rule,大概長這樣:
...
**觸發場景示例**:假設某天 02:15,underwriting API 的上游核保引擎因資料庫連線池耗盡開始大量回 503。此時 5 分鐘的 `sli:quotes_api:success_rate:5m` 跌到 99.2%,等效錯誤率為 0.8%,是 SLO 預算(0.1%)的 8 倍。YAML 中的 `QuotesApiFastBurn` 條件要求 **5 分鐘視窗與 1 小時視窗同時超過 14.4 倍**——如果這波 503 持續 2 分鐘以上,`for: 2m` 滿足後告警即觸發,PagerDuty 在 02:17 左右發出 `severity: page` 通知。on-call 工程師收到的訊息內容是:`Quotes API fast burn — 2h 內會燒掉一週預算`。典型應對流程:確認是連線池問題 → 手動擴大 pool size 或重啟連線 → 查 Grafana 確認 success_rate 回升 → 15 分鐘內若 fast burn 條件解除則告警自動 resolved,否則升級給 Tech Lead。整個過程中 Error Budget 只消耗了約 2–3 分鐘,遠少於「等到早上才發現」的損耗。

### 30.3.7 Error Budget 燃燒視覺化
...
> ✅ **修正方向**:**先有量測,再訂目標**。判準:能不能在 5 分鐘內回答「上一個 28 天滾動視窗的 success_rate 是多少」⸺ 答不出來就該回到 § 30.3.6 寫 recording rule,而不是繼續訂目標。SLO 訂得越漂亮、跟現實偏差越大,越容易被法務在合約上引用,埋 PolicyPilot 那種坑。一個好用的拇指法則是:**SLO 訂在「目前真實達成率 + 0.05%」**,有改善動力但不到天花板,每季回顧時上調。
...
> ✅ **修正方向**:**嚴守 § 30.3.3 五階順序**。判準:沒做過 L1 Game Day(staging,有人在房間)、沒在 L2 連續跑兩個月看 SLO 沒被破壞,絕對不上 production。上 production 那天,必有三件事:**Kill switch(< 30 秒能停)、客戶分桶(只演練 5% 流量)、CEO / VP 知情(不是事後才知道)**。Chaos Mesh 跟 LitmusChaos 都支援 `manualSelector` + `mode: One` 限制爆破半徑,這是 staging 的最低劑量。
...
## 30.5 交付清單 ⸺ 一頁式 SLO Catalog + Error Budget Card

每一個對外服務 / 每一條核心使用者旅程,**在合約簽下去前都該過一次 SLO Catalog + Error Budget Card**。它不是文件,是可被銷售、產品、工程、法務同時讀的決策化石 ⸺ 寫不滿一頁就是還沒想清楚。

把它存在 `docs/slo/<service>-slo-card.md`,跟 service repo 同 PR 更新,跟 § 30.3.6 那份 Prometheus rule 互鎖。
...
上面的範本包含 8 個 section,**可依團隊成熟度裁剪使用**,不必一次填滿。Pre-L1 團隊建議先填 § 1–3(SLI 定義 / SLO 目標 / SLA 對齊):這三節能讓「可靠度量測管線」先跑起來,把最基本的信號建立好。待 Error Budget 機制實際運作穩定(建議至少跑一個完整的 28 天滾動週期),再補 § 4–7(Error Budget 觸發規則 / Earn-Back / On-Call / Review Cadence)。§ 8 Out of Scope 在任何階段都建議填寫,因為「這份 SLO 不管什麼」跟「管什麼」一樣重要。§30.5.1 是 PolicyPilot 在事故賠款後補寫的完整版本,提供了所有 section 的填寫參考。
...
## 6. Owner & On-Call
<!-- 裁剪說明:此欄在 v1.0 暫以 PagerDuty 排班連結取代完整填寫;待 on-call rotation 穩定後(預計 2026-03-01)補齊。 -->
- 主 Owner:platform-team(見 PagerDuty schedule `slo-underwriting-oncall`)
- Escalation:On-Call → Tech Lead → EM → CTO(P0 < 15 min / P1 < 1h)
...
- [ ] 回答「下一場 Chaos 演練該演哪個容錯模式」⸺ 從最近 90 天事故根因回推,而不是從最帥的演練起跳;沒到 § 30.3.3 對應階段絕不上 production
...
四項中先挑一項做完就好,建議從最後那一項 ⸺ 把目前最痛的服務拉出來填一張卡,**填不出 § 1 SLI 定義的服務,就是下個 sprint 該優先做 Observability 補強的對象**。本書 Ch 35 會把「Error Budget 與 FinOps 預算如何在 ADR 裡同時管理」展開(可靠度與成本是同一個取捨的兩面);Ch 45 會把這套 SLO 思維延伸到 AI Agent 系統 ⸺ 當服務的「行為」本身是非確定性的,SLI 的定義會多一層「品質 SLI」(LLM-as-Judge 的合格率),Error Budget 也會多一個維度。AI Agent 系統的可靠度問題,例如 LLM API rate limit、context window 故障、tool use 重試,本質上仍是這套 SLO + 容錯模式 + Chaos 的延伸,只是訊號更難量測、Chaos 假設更難窮舉 ⸺ 這也是與 Ch 29 OTel + Ch 45 Eval 必須串起來看的原因。

---

## Cross-References

- **回顧**:[Ch 22 微服務拆分判準](../part-04-architecture/ch-22-microservices.md) ⸺ 拆服務的代價之一是可靠度問題會分散,SLO 必須跟著拆;[Ch 24 雲端原生與 Kubernetes](../part-04-architecture/ch-24-cloud-native-kubernetes.md) ⸺ K8s 之上的 HPA/VPA/KEDA 是本章 § 30.3.5 的延伸;[Ch 29 Observability](./ch-29-observability-otel.md) ⸺ SLI 的訊號土壤
