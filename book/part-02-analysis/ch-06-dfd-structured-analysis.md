...
**第二,合規法規開始要求「資料血緣(Data Lineage)」**。GDPR 的 Right to Erasure、PCI DSS 4.0 的卡號流向追蹤、東南亞各地區的個資新法,都要求機構能在被質詢時 30 天內回答「這位用戶的這筆資料,複製過哪幾份、流經哪些系統、目前還活在哪」。OpenLineage [^CIT-064] 與 Marquez 等工具在處理機讀格式,但**人讀格式的入口**仍然是 DFD。

**第三,AI 協作讓「資料約定」變成 prompt context 的剛需**。一個 AI Agent 要修改 `risk` 服務,不知道上游給它什麼欄位、下游期待它輸出什麼,改出來的程式碼通常會壞掉相鄰系統。DFD + 資料字典是 spec.md / CLAUDE.md 裡最值得放的兩個 artifact 之一(另一個是 ADR)。
...
NorthBay 的失誤在於:Context 圖有(放公司簡介簡報用),Level 0 沒有,直接就跳到「每個服務有自己的 Sequence 圖」這種顆粒度。中間缺了「整個系統的資料拓樸」這一層,所以 47 個 topic 才會像他們 Head of Engineering 說的那樣失蹤。
...
> ✅ **修正方向**:資料字典的精髓不在型別(那是 schema 的工作),在**業務語意 + 流向**。模板留四欄至少:**業務規則、來源、流向、對應 ADR / 法規**。如果一個欄位你寫不出業務規則,代表它不該存在,或它的存在沒人為它負責。把這件事在字典裡逼出來,比在事故會議上逼出來便宜。
