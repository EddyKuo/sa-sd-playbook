...
> **前置閱讀**:[Ch 7 物件導向分析](./ch-07-object-oriented-analysis.md)、[Ch 8 資料模型](./ch-08-data-modeling-normalization.md)
> **下游章節**:[Ch 10 NFR 與品質情境](./ch-10-spec-documents.md)、[Ch 23 Event-Driven 與事件溯源](../part-04-architecture/ch-23-event-driven-cqrs-es.md)、[Ch 40 Multi-Agent 系統設計](../part-07-ai-era/ch-40-multi-agent.md)
...
> **時間謂詞 ≠ 執行順序**:時間謂詞是規則的**輸入**(`if upgrade_day < 30`),但規則本身不決定「先算哪個、再算哪個」。執行順序是編排層的責任。舉個具體例子:Proration 與 Tax 的計算順序很重要(必須先算 proration 差額,才能拿正確基數去算稅);但「proration 要套哪條規則(R-03 還是 R-07)」是決策表的事 ⸺ 決策表只看當下的一組快照條件(`billing_cycle`、`upgrade_day`、`discount_carryover`、`cross_region`)就能查表得出答案,它不知道、也不在乎 Tax 計算會在它之後發生。
...
先從最簡版本出發。一個基礎訂閱的生命週期,只有三個狀態、只有最核心的 trigger,沒有 guard、沒有 timer、沒有 action:

```mermaid
stateDiagram-v2
    [*] --> active: signup_and_pay
    active --> past_due: charge_failed
    past_due --> canceled: dunning_exhausted
```

三個狀態、三條 transition,讀者一眼能理解「失敗的扣款會把訂閱推進 past_due,最終催款失敗則 canceled」。**這是最簡單的版本,但它隱藏了所有現實中會踩的坑**:補繳成功怎麼辦?催款可以重試幾次?暫停要怎麼處理?

實務上要回答這些問題,就必須加入 guard(轉移的前置條件)、action(轉移時觸發的動作)、timer(超時自動觸發)。LedgerPilot 的完整訂閱狀態機,寫成 Mealy + Statechart 風格大概像這樣:
...
- **多 Agent 協作流程**:[Ch 40 Multi-Agent 系統設計](../part-07-ai-era/ch-40-multi-agent.md)
- **Multi-Agent 共識與衝突**:[Ch 41 Multi-Agent 共識、狀態與衝突解決](../part-07-ai-era/ch-41-multi-agent-consensus.md) ⸺ Agent 之間的流程編排與狀態同步
