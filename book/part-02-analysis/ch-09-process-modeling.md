...
> **下游章節**:[Ch 10 NFR 與品質情境](./ch-10-spec-documents.md)、[Ch 22 Event-Driven 與事件溯源](../part-04-architecture/ch-22-event-driven-cqrs-es.md)、[Ch 36 Multi-Agent 系統設計](../part-07-ai-era/ch-36-multi-agent.md)
...
| 層次 | 時間性 | 線性 / 週期 | 範例 |
|---|---|---|---|
| 編排層 | 事件時序(順序很重要) | 通常線性、有分支 | 「Subscription 先 emit 事件,Proration 才能算」 |
| 生命週期層 | 狀態遷移(取決於事件,不一定線性) | 可循環、可回溯 | `active` → `past_due` → `active`(補繳後) |
| 規則層 | **無時間性** | 純條件對應 | 「年繳 + 中週期 + carry-over → 套規則 #7」 |

把無時間性的規則層,塞進有時間性的編排層,讀者會以為「規則是流程的一步」⸺ 但規則其實是流程**那一步當下要查的表**。把循環性的狀態機畫進線性的 BPMN,讀者會以為「狀態回到 active 是個 bug」⸺ 但其實是業務正常路徑。
...
- **事件驅動架構**:[Ch 22 Event-Driven 與事件溯源](../part-04-architecture/ch-22-event-driven-cqrs-es.md) ⸺ 編排層落到事件總線時的設計
- **多 Agent 協作流程**:[Ch 36 Multi-Agent 系統設計](../part-07-ai-era/ch-36-multi-agent.md)
- **Multi-Agent 共識與衝突**:[補章 D](../part-07-ai-era/chD-multi-agent.md) ⸺ Agent 之間的流程編排與狀態同步
