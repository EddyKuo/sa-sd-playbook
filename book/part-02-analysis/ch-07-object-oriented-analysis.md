...
**為什麼聚合根的狀態轉移能成為顆粒度判準?** 原因來自 7.2.1 的推導:用例是「行為的合約」,它的後置條件必然是業務世界發生了某個可觀測的改變。在 OOP 的領域模型裡,「業務世界發生改變」幾乎總是表現為「某個聚合根的狀態被更新」。如果一個用例的後置條件只是「回傳一筆計算結果」而沒有改變任何聚合根的狀態(純查詢、無副作用),那麼它在顆粒度上是子功能(海底層),應被併入觸發它的用戶目標層用例的步驟,或移至類別的不變式/方法。同理,涉及無狀態 Value Object(如 `Money`、`Percentage`)的用例,轉移發生的對象是持有這個 VO 的聚合根,判準不變;純 VO 計算本身不構成用例,而是聚合根方法的內部邏輯。
...
OOA 階段的有效範圍是 **Logical + Process + Scenarios** 三個視圖。**State Diagram 在 Kruchten 1995 的原版中屬於 Logical View 的一部分**,視為類別的行為附件,跟靜態類別結構一起更新。然而在 2026 年的高頻迭代工程現場,這個歸屬帶來了一個實際問題:狀態機的更新頻率和決策複雜度與用例高度同步,而不是與靜態類別結構同步。DripCycle 的三條退款流程就是典型案例——每次用例迭代都要改狀態機,但因為狀態機「住在」Logical View 裡,更新時容易被誤認為「架構改動」而走過重審流程,反而降低了更新意願。**2026 的實用做法是把 State Diagram 獨立成第四張**,讓它與 Use Case 同生命週期管理:用例改→狀態機改→序列圖改,三者在同一個 PR 被點名,而不是讓狀態機在 Logical View 的陰影裡被遺忘。Development 與 Physical 留給設計階段(Ch 11 之後)。
...
7.3 的判準表和四圖協同表告訴我們「該怎麼組織思維」——哪個顆粒度的用例值得畫、哪張圖改了要連帶檢查哪張。但現場還需要一個更緊湊、能被機械化檢查的交付物:光有四張圖和一張協同表,PR review 時仍然需要人工回憶「這個用例對應的是狀態機的哪條邊、序列圖有沒有畫例外路徑」。**Use Case Atom 就是把四圖協同關係壓成一張卡片,確保每個用例的四張圖對應不漏檢**。它不取代 Cockburn 模板的敘事深度,而是作為 Cockburn 模板的「對應索引頁」——先填 Atom、確認四圖都有掛上,再展開 Cockburn 的主/例外流程描述。
...
**關於 Linked Sequence 欄位的畫法說明**:7.4 反模式 3 強調例外路徑是序列圖的一等公民。Atom 的 `Linked Sequence` 欄列出了三條路徑,但僅靠文字描述不足以讓工程師知道怎麼在 `seq-uc-07.mmd` 裡畫。正確做法是用 Mermaid 的 `alt` 區塊把 happy path 與例外路徑並列在同一張圖,而不是讓例外路徑只存在於文字。以下是 `seq-uc-07.mmd` 針對 Stripe webhook 例外的最小範例:
...
sequenceDiagram
    ...
    M->>OS: 申請冷靜期退款
    OS->>RS: createRefund(orderId, amount)
    RS->>ST: POST /v1/refunds
    ST-->>RS: 202 { status: pending, refund_id }

    alt Stripe succeeded (webhook 到達)
        ST-->>RS: webhook refund.succeeded
        RS->>OS: markRefundCompleted(orderId)
        OS-->>M: 退款成功通知
    else Stripe pending > 24hr → failed
        ST-->>RS: webhook refund.failed
        RS->>OS: rollbackRefundState(orderId)
        OS-->>M: 退款失敗通知,請聯繫客服
    end
```

這個範例展示兩條分支:happy path(Stripe `succeeded`)與例外(24 小時後 `failed`)。實際專案的 `seq-uc-07.mmd` 應再補充例外路徑 2(倉儲 SLA 逾期→人工審查),並在 Atom 的 `Linked Sequence` 欄對應標注各分支的 `alt` 框名稱,方便 PR review 時對圖校核。
