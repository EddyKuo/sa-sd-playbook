...
> **下游章節**:[Ch 8 資料建模](./ch-08-data-modeling-normalization.md)、[Ch 9 系統規格與 SRS](./ch-10-spec-documents.md)、[Ch 17 DDD 戰術設計](../part-04-architecture/ch-17-ddd-strategic-tactical.md)
> **延伸補章**:無
...
上線兩週後,客服那邊每 100 張單就有 18 張要人工介入。其中最常見的三種出包長這樣:
...
換句話說,OOA 是 DDD 在單一 context 內的工作方法,DDD 是把多個 OOA 模型黏起來的方法。Ch 17 會把這層關係完整展開,本章先把單一 context 內的 OOA 做扎實。
...
如果四項中先挑一項做完就好,建議是最後那一項 ⸺ 把目前 Confluence 上的用例文檔挑五個出來,**用 Atom 模板重寫一次**。寫不滿 Pre-state / Post-state / Linked State Diagram 三欄的,代表用例與狀態機從未對齊過,那五個就是先要回去談的對象。本書 Ch 8 會把這層概念類別圖往資料模型方向收斂,Ch 17 會把它往 DDD Bounded Context 方向展開。
...
- **規格收束**:[Ch 9 系統規格與 SRS](./ch-10-spec-documents.md)
- **DDD 戰術**:[Ch 17 DDD 戰術設計](../part-04-architecture/ch-17-ddd-strategic-tactical.md) ⸺ 把單一 context 的 OOA 擴張到 Bounded Context

## 引用

[^CIT-005]: Eric Evans, *Domain-Driven Design: Tackling Complexity in the Heart of Software* (Addison-Wesley, 2003)。同 Ch 21。本章在 OOA 概念類別圖與 DDD 領域模型對照處再引。
