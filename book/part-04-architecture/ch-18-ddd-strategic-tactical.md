...
Context Map 不是 ER 圖,不是 service dependency 圖,它畫的是**團隊之間的權力關係**。Eric Evans 在原典 [^CIT-170] 裡列了八種模式,在現場使用頻率差異很大。**Step 3 的八種模式見下方 §18.3.2 展開說明;確認 Context Map 後,才進入 §18.3.3 的戰術設計階段。**

### 18.3.2 Context Map 八種模式對照表（Step 3 詳展）
...
| **Shared Kernel(共享核心)** | 兩 context 共用一塊核心模型 | 兩個高度耦合子域需共用不可變資料模型,**雙方共同維護同一份模型**。判準:任何一方變更此模型需對方同意並同步更新。與 Published Language 的差異在於,Shared Kernel 是「兩方共同擁有同一份原始碼」,不是「各自發布自己的語言格式」;若只需單向發布穩定格式,選 OHS + Published Language 即可 | Clinical ↔ Pharmacy 共用 `ICD-10 code list`(只讀,雙方共同維護版本升級) |
...
**反例:如果把 `Encounter`、`DiagnosisList`、`DischargeSummary` 各拆成獨立 Aggregate Root 會怎樣?**

以電商案例類比更直觀:假設把 `Order`、`OrderItem`、`OrderShippingAddress` 全拆成三個獨立 Aggregate Root ⸺ 下單動作變成:

1. `OrderRepository.save(order)` ← 成功
2. `OrderItemRepository.saveAll(items)` ← 成功
3. `OrderShippingAddressRepository.save(address)` ← 網路逾時,失敗

第三步失敗時,資料庫裡有一張沒有收件地址、也沒有商品明細加總金額的訂單。補救方案只有兩條:一是用分散式交易(2PC)把三個 repo 的 save 包成同一個原子操作 ⸺ 代價是引入 XA transaction,在高峰期鎖表時間拉長三倍,效能壓力直接從 OLTP 轉成 OLAP 等級;二是寫一個 Saga,Step 1 失敗回滾 Step 2、Step 2 失敗回滾 Step 1、Step 3 失敗先寫補償 Event 再非同步 retry ⸺ 代碼量從 30 行的 `Order.PlaceOrder()` 膨脹到 200 行的 Saga orchestrator,加上三個補償 handler,任何一段邏輯出錯都可能製造殭屍訂單。

聖維禮做 Clinical context 時遇到過完全相同的問題:早期設計把 `Encounter`(就診事件)和 `DischargeSummary`(出院摘要)拆成兩個 Aggregate Root,結果「出院確認」這個動作需要協調兩個 Aggregate 的 save。Discharge 成功但 DischargeSummary 尚未持久化的那幾百毫秒,Billing 訂閱了 `EncounterDischarged` 事件卻拿不到完整的出院摘要,導致健保申報金額異常。最終修法是把 `DischargeSummary` 降為 `Encounter` 內的 Entity(不是獨立 Aggregate Root),出院邏輯收回 `Encounter.Discharge()` 方法內,一個交易內保持一致。
...
這個結構看起來簡單,但它把過去七年「一筆 update 同時改五張表」的痛點解掉了。

下面是聖維禮 Clinical context 最終落地的 `Encounter` Aggregate 完整代碼架構(C# 13 / .NET 9,2026 .NET LTS)。閱讀時請注意三個設計決策:**第一**,`PatientMrn` 是 Value Object 而非 Entity reference,體現跨 BC 只持 ID 的原則;**第二**,`Discharge()` 是唯一出口,所有出院 invariant 集中於此;**第三**,`EncounterDischarged` Domain Event 是 Billing 的唯一訊號來源,取代 linked server join:
