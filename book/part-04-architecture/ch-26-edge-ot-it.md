...
這張表揭示的不是「OT 與 IT 哪個更重要」,而是**兩個世界的根本假設互相矛盾**——把 IT 假設強加給 OT,就是 §26.1 那三十一秒崩潰的根本原因。以下三條原則直接對應表格中的衝突維度:

- **時間尺度差異** → 必須讓「控制路徑能被切斷」:OT 控制迴路以毫秒計算,任何雲端 REST round-trip 的延遲與可用性都無法滿足這個時序要求。控制決策必須能在本地完成,雲端只能送「建議」。
- **生命週期差異** → 必須「資料流與控制流分離」:設備韌體十年不動,IT 服務一年更新多次。如果控制流與遙測流走同一通道,韌體升級或 IT 服務變更都可能誤觸控制層,後果是設備行為不可預測。
- **失敗代價差異** → 必須「OT 永遠是 Source of Truth」:IT 端的計算結果可能因網路延遲、快取過期、服務重啟而不一致,但設備實際的物理狀態是當下唯一可信的事實。

**邊界的設計原則**(可以寫進每一份合約):

1. **OT 永遠是 Source of Truth**。設備上的值是真實世界的反映,雲端是這個值的「最終一致性副本」。如果雲端與設備的值衝突,以設備為準。**前提條件**:傳感器自身必須通過健康度驗證(設備端上報傳感器異常旗標、資料點附帶有效期 / staleness 標記)。當傳感器失效或資料超過有效期時,本地閘道應進入「安全保護模式」(例如以最後一個已知有效值 + 保守策略繼續運轉),而不是直接接受雲端值——接受雲端值等同於讓 IT 假設覆蓋 OT 現實。
...
**Level 評估檢查清單** — 每個層次有明確的能力門檻,而不是模糊描述:

| Level | 關鍵能力指標 | 典型「撐多久」 |
|---|---|---|
| **0** | 無本地儲存、無本地邏輯;中央斷線即停擺 | 0 秒 |
| **1** | 本地資料緩衝 ≥ 5 分鐘、網路恢復後自動重傳;業務邏輯仍在中央 | 5–30 分鐘(取決於緩衝大小) |
| **2** | 本地資料保留 ≥ 7 天、本地規則引擎響應 ≤ 100 ms、預載策略集 ≥ 5 種(覆蓋主要運轉場景) | 24 小時以上 |
| **3** | 完整本地閉環(本地閘道可自主下達與拒絕命令)、雲端僅收分析與報表 | 無上限(雲端可永久離線) |
| **4** | 多邊緣節點彼此協調(P2P 協議);中央消失後整體系統仍能優化 | 分散式自治 |

**選擇目標 Level 的決策框架**:

| 判斷維度 | 傾向較高 Level | 傾向較低 Level |
|---|---|---|
| **設備類型** | 關鍵基礎設施(發電、配電、醫療) | 非關鍵監測(辦公室環境感測) |
| **故障代價** | 停機 = 安全事故或法規違規 | 停機 = 資料缺口、通知延遲 |
| **網路可靠性** | 4G / 衛星、常斷線、偏遠地點 | 企業有線網路、SLA 99.9% 以上 |
| **設備側硬體成本** | 已有本地計算能力(工控機 / 嵌入式 Linux) | 純感測器節點,無本地計算 |
| **專案預算** | 足夠建本地 DB + 規則引擎 | 僅能做雲端 + 輕量緩衝 |

一般準則:符合「關鍵基礎設施」或「故障代價 = 安全事故」的任一條件,最低應達 **Level 2**;發電、配電、醫療設備建議 **Level 3**。
...
以 §26.1 的 200kW 儲能系統為例,四項內容映射到具體資料欄位如下:

- **讀模型(現在的狀態)**:設備當下的遙測快照。例如 `{id, timestamp, soc_pct, temperature_max_c, pcs_power_kw, grid_freq_hz}`。這份模型供雲端儀表板和決策引擎讀取。
- **寫模型(應該的狀態)**:雲端對設備下達的期望狀態。例如 `{id, desired_soc_pct, desired_power_kw, issued_at, expires_at}`。注意 `expires_at` 是關鍵欄位——若設備端在有效期內沒收到命令,應回退本地策略而非繼續執行過期指令。
- **事件日誌(經歷過的歷程)**:狀態變化和觸發事件的不可變紀錄。例如 `{event_type: 'CHARGE_START', trigger: 'price_signal', soc_before: 31, timestamp}`、`{event_type: 'SAFETY_CUTOFF', reason: 'TEMP_HIGH', temperature_c: 57.2, timestamp}`。
- **行為模型(怎麼回應命令)**:本地閘道的安規邏輯——什麼條件下接受 / 拒絕 / 覆寫雲端命令。這部分對應 §26.5.1 的「安規切斷清單」。

這四份模型需要一個**可靠的傳輸層**保證——讀模型的遙測資料必須能正確同步到雲端,寫模型的命令必須能被設備端確認接收或安全拒絕。這正是下一節要討論的問題。
...
**QoS 的具體失效場景**——先搞清楚問題,再談解法:

| QoS 等級 | 字面承諾 | 實際失效場景 |
|---|---|---|
| **QoS 0** | 最多一次(fire-and-forget) | 任何網路抖動都會丟失訊息,完全沒有重傳 |
| **QoS 1** | 至少一次(有 ACK) | Broker 重啟時,尚未 ACK 的訊息可能丟失;重傳可能導致重複處理,若接收端沒做冪等,業務邏輯會出錯 |
| **QoS 2** | 剛好一次(四次握手) | 協定開銷最高;四次握手並不保證訊息的「業務語意」正確——例如設備重連後,訂閱端仍不知道「這個節點在離線期間狀態有沒有變化」 |

**三個 QoS 等級有一個共同盲點**:它們都是「訊息層」的保證,而不是「狀態層」的保證。設備離線又重連後,訂閱端無從得知設備當前完整狀態——除非重放所有歷史訊息,但誰都不會真的這樣做。

**真正讓 MQTT 「可靠」的不是 QoS,是 Sparkplug B**:

Sparkplug B(Eclipse 基金會規範)是把 MQTT 從「訊息協定」升級為「狀態協定」的關鍵。它正是針對上述盲點設計的:

- **NBIRTH**:每個邊緣節點(EoN)在連線時必須宣告完整狀態。例如 Broker 重啟或設備重連後,訂閱端可以立刻從 NBIRTH 取得設備當前的完整遙測快照(對應數位孿生的「讀模型」),而不是從不完整的歷史訊息流拼湊。
- **NDEATH**:節點意外斷線時,Broker 透過 LWT 發出 NDEATH;訂閱端立刻知道「這個節點死了」,而不是錯誤地解讀為「這個節點剛好沒新資料」——後者在 QoS 1/2 下需要自行實作心跳超時邏輯。
- **NDATA + seq**:之後只發增量(NDATA),訊息序號(seq)強制 0–255 循環。接收端可立即偵測丟失(seq 不連續),觸發重新請求——比 QoS 1 的被動等待更主動、更明確。

**Sparkplug B 的限制**:設備端必須嚴格遵守協定規範——NBIRTH 必須是重連後第一條訊息,seq 必須遞增,NDEATH 必須在 LWT 設定正確。如果設備端韌體實作不規範(例如 NBIRTH 之前先送了幾條 NDATA),Broker 端和訂閱端的狀態重建邏輯會進入混亂,反而增加調試複雜度。導入 Sparkplug B 之前,應先確認設備廠提供的 SDK 版本和測試報告。
...
前三節分別回答了三個問題:「離線時誰說了算」(§26.3.1 離線自治層次)、「設備的資料模型是什麼」(§26.3.2 數位孿生)、「訊息如何可靠傳遞」(§26.3.3 MQTT + Sparkplug B)。下面這張 C4 架構圖是這三個決策的整合呈現——**邊緣層的三個組件正是讓以上決策落地的實體**:

- **本地時序 DB**:提供 Level 2 所需的 7 天本地保留能力,儲存數位孿生的讀模型與事件日誌。
- **本地規則引擎**:執行預載策略(≤ 100 ms 響應),實現數位孿生的行為模型與安規覆寫。
- **Sparkplug B Broker**:承載數位孿生的 NBIRTH/NDATA 狀態同步,確保遙測流的 at-least-once 語意。

三條獨立資料路徑的可靠性等級也對應不同需求:**遙測流**用 Sparkplug B at-least-once(狀態同步必須完整);**控制流**用 REST + 本地安規閘道(每條命令必須被確認或拒絕);**影像流**用 RTSP best-effort(畫面遺失可接受,不阻塞控制路徑)。
...
「邊緣節點」其實只是把雲端應用搬到設備旁邊跑,但沒有根據業務屬性設定明確的離線自治目標——導致設計者誤以為「放了一台邊緣伺服器」就等同於「具備離線自治能力」。

Level 0/1 本身**不一定是錯的**——如果業務屬於非關鍵監測(例如辦公室 CO₂ 感測,斷線只是資料缺口),Level 1 完全可以接受。**問題在於不分場景地套用低 Level**:

| 業務場景 | 最低可接受 Level | 說明 |
|---|---|---|
| 辦公室環境監測 / 非關鍵感測 | Level 1 | 斷線後資料缺口可接受 |
| 工商儲能 / 工廠生產設備 | Level 2 | 斷線後需繼續按策略運轉 |
| 關鍵基礎設施(發電站、配電網) | Level 2–3 | 中央消失後必須完整自主運轉 |
| 生命安全相關(醫療設備、救援系統) | Level 3 | 任何中央依賴都是設計缺陷 |

> ✅ **修正方向**:在需求階段就確認業務場景,把「離線自治目標 Level」寫進合約 SLA。對工商儲能和工廠場景,Level 2 是最低標準——合約應明訂「中央離線 24 h,業務在預載策略下繼續運轉」。每季做網路斷線演練,按 §26.3.1 的能力指標驗收。
