...
> **下游章節**:[Ch 20 C4 模型](./ch-20-c4-model-visualization.md)、[Ch 23 Event-Driven 與事件溯源](./ch-23-event-driven-cqrs-es.md)、[Ch 40 Multi-Agent 系統設計](../part-07-ai-era/ch-40-multi-agent.md)
...
三層之間的順序不是武斷規定,而是**每一層都有明確的出口條件,下一層的入口依賴那個出口**:

- **Big Picture 的出口條件**:一面長牆上識別出 ≥ 2 個 Bounded Context,且每個 Context 的主要事件流已被橘色便利貼覆蓋。**沒達到這個出口,即使貼了藍色 Command 也沒意義**:Command 是在哪個 Context 裡觸發,還不清楚。
- **Process Modelling 的入口**:要求 Big Picture 已識別出 Bounded Context;**出口條件**:挑定的那個流程,每一張橘色便利貼都有藍色 Command 配對(誰觸發)、黃色 Aggregate 配對(發生在誰身上)、紫色 Policy 配對(觸發了什麼)。**「七色齊全」不是說牆上每種顏色都有,而是這個流程裡的每張橘色都有這三種顏色配齊**。
- **Software Design 的入口**:要求 Process Modelling 完成了至少一個流程的七色配對;**出口條件**:單一 Aggregate 的 invariant 寫出 ≥ 5 條,Command 的 payload 欄位定義清楚到 RD 可以直接寫 interface。

為什麼不能「直接跳到 Software Design」?因為 Software Design 聚焦在**單一 Aggregate**,而你要先知道這個 Aggregate 的邊界在哪、它跟哪些 Command / Event 有關——這些是 Process Modelling 定義的。沒有 Process Modelling 的 context,Software Design 很容易把不屬於這個 Aggregate 的邏輯塞進來,或是把一個 Aggregate 拆成兩個——都是常見的設計失誤。

### 19.2.3 Event Modeling 補的是「便利貼到 spec」那段

Event Storming 的三層工作坊本身是探索性的,但 Brandolini 的設計並未覆蓋「從便利貼直接生成 spec 與測試」這個問題——即使 Software Design 層把七色便利貼都補齊了,你仍然缺少「這個事件流對應到哪個 API endpoint、Payload 欄位是什麼、怎麼寫 acceptance test」。這正是 Event Modeling 補充的地方。
...
```gherkin
# 範例 A:RenewLane 的「續約成功扣款」事件流(成功路徑)
...
Event Modeling 的設計目的是覆蓋**所有路徑**,不只有成功路徑。失敗路徑也必須有對應的 Given-When-Then:每個失敗情境都需要明確寫出「emit 了哪個事件、Read Model 顯示什麼」。以下是同一個 Command 的失敗路徑:

```gherkin
# 範例 B:「信用卡過期」失敗路徑

Given Subscription "ACME-Annual-001" 處於 active 狀態
  And 信用卡已過期(expiry < 今天)
When Command RenewSubscription { subscriptionId: "ACME-Annual-001" } 被執行
Then Event PaymentFailed { subscriptionId, failedAt, reason: "card_expired" } 被發出
  And Read Model "Renewal Dashboard" 顯示該筆狀態為 "Payment Failed"
  And Read Model "Dunning Queue" 新增一筆待處理的催收工作
...
Given Subscription "ACME-Annual-001" 處於 active 狀態
  And 信用卡有效但餘額不足
When Command RenewSubscription { subscriptionId: "ACME-Annual-001" } 被執行
Then Event PaymentFailed { subscriptionId, failedAt, reason: "insufficient_funds" } 被發出
  And Policy "dunning" 自動 emit Command SendDunningEmail { subscriptionId, attemptNo: 1 }
```

注意範例 B 與 C 雖然都產出 `PaymentFailed`,但 Read Model 的後續狀態與觸發的 Policy 可能不同——這正是 Event Modeling 逼你把分叉點寫清楚的用意。**沒有失敗路徑的 Event Modeling 是不完整的 spec**,RD 拿到這份圖還是得靠猜。
...
19.3.2 的表格回答了「想回答什麼問題 → 開哪一層」,但表格無法表達兩個現實因素:**團隊規模**與**架構不確定性**。下面的決策樹補上這兩個維度——特別是「C4 模型在哪個時間點介入」的問題。

> **C4 與 Event Storming 的關係**:C4 模型([Ch 20](./ch-20-c4-model-visualization.md))是架構視覺化工具,用來確認「這個系統由哪些 Container 組成、彼此怎麼通訊」。它與 Event Storming 並非同一條鏈上的串聯步驟,而是**平行的架構檢查點**:Big Picture 做完後,如果有人對「這個 Bounded Context 到底跑在哪個 service 上」有疑問,就可以同時畫 C4 System Context / Container 圖來對齊。但 C4 **不是** Process Modelling 或 Software Design 的前置條件——C4 回答「在哪」,Event Storming 回答「發生什麼」,兩者問題層次不同。Software Design 層之前若 Container 邊界仍有爭議,才強制先對齊 C4。

```mermaid
flowchart TD
    Start([「我們要做 Event Storming」]) --> Q1{跨部門是否<br/>對這個領域<br/>有基本共識?}
    Q1 -->|沒有 / 不確定| BP[Big Picture<br/>半天–1 天<br/>橘 + 紅貼為主]
    Q1 -->|有| Q2{要實作的流程<br/>是哪一個?}
    Q2 -->|還不知道| BP
    Q2 -->|知道是哪一個| QTeam{團隊規模<br/>> 30 人 或<br/>跨時區?}
    QTeam -->|是| BP2[先做 Big Picture<br/>拆出 Bounded Context<br/>再分組進 Process Modelling]
    QTeam -->|否| PM[Process Modelling<br/>半天–1 天<br/>七色齊全]
    BP2 --> PM
    PM --> QArch{Container 邊界<br/>有爭議嗎?}
    QArch -->|是,先對齊架構| C4[平行畫 C4<br/>Container 圖<br/>詳見 Ch 20]
    QArch -->|沒有爭議| SD[Software Design<br/>2–4 小時<br/>聚焦單一 Aggregate]
    C4 --> SD
    SD --> Q4{需要對接<br/>AsyncAPI / spec?}
    Q4 -->|是| EM[進 Event Modeling<br/>四象限 + Given-When-Then]
    Q4 -->|否| Code[直接寫 code<br/>+ ADR 記錄決定]

    BP -. 沒設出口條件 .-> Hotspot[反模式:<br/>停在 Big Picture]
    PM -. 紅貼沒有 owner .-> Hotspot

    classDef hot fill:#fee,stroke:#c33
    classDef cold fill:#eef,stroke:#36c
    classDef goal fill:#efe,stroke:#3a3
    class BP,BP2,PM cold
    class SD,EM goal
    class C4 cold
    class Hotspot hot
```

這張圖的關鍵不是分支,**是兩條虛線**。Big Picture 與 Process Modelling 都會掉進「停在這層不走下去」的陷阱 ⸺ 所以每一層離開時都要明確回答「下一層是什麼、何時開」。C4 的位置在 Process Modelling 之後、Software Design 之前——只要 Container 邊界仍有歧義就觸發;若邊界清楚,可以直接進 Software Design。
...
> 版本:v1.0 | 撰寫日期:2026-01-15 | 擁有人:Lead Engineer 周
...
- **Multi-Agent 系統**:[Ch 40 Multi-Agent 系統設計](../part-07-ai-era/ch-40-multi-agent.md) ⸺ 事件流在 Agent 協作中的角色
- **延伸補章**:[Ch 41 共識/狀態/衝突](../part-07-ai-era/ch-41-multi-agent-consensus.md)
