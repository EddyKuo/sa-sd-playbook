...
第一,**Level 2 寫起來最尷尬**。Level 1 容易,畫個方塊代表整個系統就好;Level 4 容易,IDE 自動產出。Level 2 必須誠實面對「這個系統內部到底有哪幾個可獨立部署的東西、它們怎麼說話、各自存什麼」⸺ 而這件事在大部分團隊的腦袋裡是模糊的。這種尷尬感的來源很具體:大多數系統的服務邊界是「在開發過程中自然長出來的」,沒有人正式宣告它。到了必須畫 Level 2 的時候,才會發現「API Gateway 到底算不算一個 container」、「兩個 service 共用同一個 DB 是一個 container 還是兩個」這些問題從來沒被回答過。Level 2 不像 Level 4 有 IDE 自動驗證,它暴露的是組織層面的邊界模糊——而這種模糊,往往比技術本身更難承認。
...
既然 C4 的優勢在於「讀者導向」與「簡潔性」,那在實務上應該如何把這個設計取捨落地——也就是,在具體場景下選對圖層、挑對工具?下面這幾張表跟一張 Mermaid 決策樹,正是這個「從設計哲學到工具選擇」的轉譯。它們的共同前提是:**畫圖前先問「誰會讀」**,而不是「我們要不要畫齊四層」。
...
在上述六個常見場景中,跨越性最強的需求是「合規 / 資安審查需要 Deployment 圖」——這是六個場景裡唯一不能用 Level 1 + Level 2 完全覆蓋的情況。但對大多數以內部協作為主的系統而言,合規場景的 Deployment 圖屬於專項補充,不代表平常也要把四層全畫滿。因此以下的最小劑量法則,是針對**非合規場景**的日常預設基準。
...
除了上節討論的 Structurizr DSL 與 Mermaid C4 plugin 二擇一,市場上還有多種工具值得了解。以下這張速查表按「**系統規模**」與「**團隊使用情境**」整理,幫助在不同場景下快速決策——Structurizr DSL 與 Mermaid 列在首位作為基準,其餘是補充或替代選項。
