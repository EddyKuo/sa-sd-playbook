...
# 第 19 章|C4 Model 與架構視覺化
## ⸺ 給多種讀者看的同一份地圖

> **前置閱讀**:[Ch 5 UML 模型語言全景](../part-01-foundations/ch-05-uml-overview.md)、[Ch 13 架構風格](../part-03-design/ch-13-architecture-styles.md)、[Ch 17 DDD 戰術設計](./ch-17-ddd-strategic-tactical.md)
> **下游章節**:[Ch 20 模組化單體](./ch-20-modular-monolith.md)、[Ch 21 微服務拆分](./ch-21-microservices.md)、[Ch 30 ADR](../part-06-engineering/ch-30-adr-architecture-knowledge.md)
> **延伸補章**:無

---

## 19.1 冷觀察 ⸺ 47 張投影片,沒有一張能回答投資人那句話
...
### 19.2.2 為什麼 Level 2 是最常被跳過、卻最重要的
...
### 19.2.3 C4 與 UML / 4+1 / ArchiMate 的關係
...
### 19.3.2 Structurizr DSL vs Mermaid C4 plugin 取捨

本書其他章節一律用 Mermaid。Ch 19 是唯一例外 ⸺ **C4 主示範鎖定 Structurizr DSL**[^CIT-191],因為它是 Simon Brown 親自設計、為 C4 量身打造的 DSL。Mermaid 的 C4 plugin 仍可作為輕量替代,以下這張表是現場挑工具的判準。
...
### 19.3.5 補充圖種:Deployment / Dynamic / System Landscape / Filtered
...
### 19.3.6 ADR 與 C4 圖的整合節奏

ADR(見 [Ch 30](../part-06-engineering/ch-30-adr-architecture-knowledge.md))是「決定的化石」,C4 是「決定棲息的地形」。兩者整合的節奏可以這樣定:
...
這條 fitness function 的價值在 [Ch 31](../part-06-engineering/ch-31-fitness-functions.md) 會展開,這裡只指出**整合節奏不能靠人工巡檢**,必須機器把關。
...
> ✅ **修正方向**:單張 Container 圖的方塊數**控制在 7±2 個**(對應人類工作記憶上限)。超過時用三招:其一,把若干 container 包進更高層的 Software System(回到 Level 1 / System Landscape 處理);其二,用 Structurizr DSL 的 `view` + `include` / `exclude` 做 Filtered View(例:「只看支付授權路徑的 container」、「只看 KYC 路徑的 container」);其三,承認業務 bounded context 沒切清,先回到 [Ch 17 DDD](./ch-17-ddd-strategic-tactical.md)。
...
> ✅ **修正方向**:架構圖一律走 Diagrams-as-Code(本書其他章 Mermaid,Ch 19 主推 Structurizr DSL),`.dsl` / `.mmd` 進 Git,跟 README 同層,跟程式碼同 PR review。圖形編輯器只用於草圖階段,定稿後必須能匯出純文字進 Git ⸺ 這條紀律跟 Ch 5 的反模式 4(UML 工具鎖定)是同一條,在 C4 場景更嚴重,因為 C4 的讀者更廣。

---

## 19.5 交付清單 ⸺ 一頁式 C4 Diagram Card + workspace.dsl 樣板
...
如果四項中先挑一項做完就好,建議是最後那一項 ⸺ 把現有系統的 Level 1 + Level 2 寫進 Structurizr DSL,**用一張圖去回答下次募資 / 合規審查 / 新人 onboarding 的第一個問題**。本書 Ch 20 模組化單體與 Ch 21 微服務拆分,都會回到這份 Container 圖上做進一步決策;Ch 30 ADR 會教你怎麼把這些決策的「為什麼」寫下來,跟 C4 圖配對成完整的「決定的化石 + 棲息的地形」。

---

## Cross-References

- **回顧**:[Ch 5 UML 模型語言全景](../part-01-foundations/ch-05-uml-overview.md)、[Ch 13 架構風格](../part-03-design/ch-13-architecture-styles.md)、[Ch 17 DDD 戰術設計](./ch-17-ddd-strategic-tactical.md)
- **下一章**:[Ch 20 模組化單體](./ch-20-modular-monolith.md) ⸺ 在 Container 圖上看模組邊界
- **微服務拆分**:[Ch 21 微服務拆分](./ch-21-microservices.md) ⸺ Container 圖如何告訴你拆 / 不拆
- **決策紀錄**:[Ch 30 ADR](../part-06-engineering/ch-30-adr-architecture-knowledge.md) ⸺ 為每個 container 寫一份「為何存在」
- **Fitness Function**:[Ch 31 演進式架構](../part-06-engineering/ch-31-fitness-functions.md) ⸺ 讓 C4 圖跟 repo 自動對齊
