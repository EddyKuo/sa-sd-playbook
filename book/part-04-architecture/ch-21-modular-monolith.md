...
這張表有七個數字,但最能說明「架構選擇的真實代價」的只有兩條:一是「**新人 onboarding 從 12 週降到 3 週**」,二是「**SRE / Platform 工時從 18% 降到 4%**」。前者衡量的是新知識傳遞的摩擦成本 ⸺ 一個工程師從入職到第一個 PR 需要 12 週,代表系統的認知門檻高到整個組織都在持續付出招募與培育的隱性稅。後者衡量的是日常維運的沉沒成本 ⸺ 18% 的 SRE 工時不是在做功能,是在對抗分散式系統的運維複雜度。這兩個指標合在一起說的是同一件事:在這套拓樸下,**光是「讓系統繼續運作」這件事本身,就佔用了組織相當大比例的精力**。雲端帳單的 62% 降幅是可見的成本,工時與 onboarding 時間才是那座冰山在水面下的部分。
...
這張圖從 47 個壓到 6 個,數字本身不是重點。重點是:這場演化花了整整 24 個月,本質上反映的不是微服務的失敗,而是一次「**用拓樸決策替代了設計決策**」的代價結算 ⸺ 先拆了服務,才發現 Bounded Context 根本沒切清楚。
...
## 21.2 真問題 ⸺ Modular Monolith 是中間態,不是反義詞
...
正因為這個二元誤導長期存在,才會有人誤以為 Modular Monolith 是「還沒拆完的微服務」⸺ 只是往「正確方向」走到一半的過渡狀態。但如果二元選項本身是假的,Modular Monolith 就不是中途站,而是一個獨立的、長期可行的架構終點。

### 21.2.2 Modular Monolith 不是「沒拆完的微服務」
...
這張表的關鍵不是行,是**第三欄那個「編譯期強制」**。Modular Monolith 跟 Big Ball of Mud 的差別,不在進程數,在「**模組之間的依賴是不是被工具強制守護**」。

但這裡要把兩件事分開說清楚,否則會掉進一個常見陷阱:

- **Module Boundary Card(§ 21.5)是契約,不是防腐劑。** 它記錄設計意圖、Public API、Allowed Dependencies,讓人與 Agent 都讀得懂這個模組的邊界在哪。但 Card 寫得多漂亮,都只是溝通工具 ⸺ Card 本身沒有任何強制力。
- **CI 裡的 fitness function 才是防腐劑。** ArchUnit / Packwerk / Nx `enforce-module-boundaries` 這些規則在 PR 觸發時跑、違反就擋 merge ⸺ 這是讓「編譯期強制」從表格的文字變成工程現實的那一步。

兩件事缺一件,後果不同:Card 寫了、CI 沒有 → 6 個月後新人從沒看到那張 Card,跨模組 import 12 次沒人攔;CI 有了、Card 沒有 → 規則存在但沒人知道邊界的來龍去脈,下次加規則時沒有依據。**真正能防止 Modular Monolith 腐爛的,是兩者配套**:Card 讓人知道邊界的意圖,fitness function 讓工具在 18 個月後仍然守護那條線。判準很簡單:問兩個問題 ⸺「這個模組有 Card 嗎?」加「這張 Card 的規則有對應的 CI fitness function 嗎?」兩個都是「是」,邊界才算真的存在。有一個是「否」,就是 § 21.4 反模式 3 的起點。

### 21.2.3 為什麼 2026 年特別適合 Modular Monolith

上一節的訊號表說明了 2024–2026 業界在「先做 Modular Monolith」這件事上逐漸形成共識。但這還只是組織層面的理由。同時,2025–2026 年 AI Agent 在軟體工程中的角色變化,為 Modular Monolith 加上了一個過去沒有的新維度。
...
### 21.3.3 「該不該從 Modular Monolith 拆出微服務」決策樹

拆服務的判準不應該是「我想要獨立部署」或「這樣比較現代」這類表面原因。真正驅動拆分的,是五個具體的結構性維度:一、**部署節奏差距**,決定拆出去是否真的省力;二、**SLA 與故障隔離需求**,決定是否需要 out-of-process 邊界;三、**擁有團隊的規模與穩定性**,決定 Conway's Law 是否已對齊;四、**Platform Engineering 能力**,決定組織是否有能力承擔分散式系統的運維稅;五、**節奏差異的持續性**,排除短期高峰誤判為長期需求。這五個維度缺一不可,少了任一個,就只是把問題換成另一種形式。
...
1. **包住(Wrap)**:在現有系統外圍包一層 anti-corruption layer(ACL),把所有外部呼叫先導進 ACL。這一步沒有改業務邏輯,只是讓「替換點」變得明確 ⸺ 以免後續切模組時,不知道邊界該往哪裡畫、哪些呼叫該被攔截。先有 ACL,才有地方定義「模組內部」和「外部世界」之間的那條線。
...
### 21.3.5 程式碼長相:Spring Modulith + ArchUnit 的最小骨架
...
上面是 Spring Modulith 的寫法。如果你的棧是 Ruby / Rails,Shopify Packwerk 提供等效的邊界強制機制 ⸺ 核心問題相同:Public API 是什麼、誰可以依賴誰、違反邊界的 commit 怎麼被擋住。語法不同,但解決的是同一個問題。對照組,如果是 Ruby / Rails 8 + Shopify Packwerk 3.x,大致長這樣:
...
## 21.5 交付清單 ⸺ 一頁式 Module Boundary Card
