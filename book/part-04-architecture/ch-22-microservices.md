...
把 §22.1 改成非同步,故事會變這樣:cart 不會直接呼叫 inventory,而是發 `CartItemAdded` 事件;pricing 訂閱事件異步算價;結帳時 order-service 從本地 read model 直接讀「目前可售量(可能略舊)」做樂觀預訂,真正的扣量在 saga 裡 reserve。inventory 抖,只會卡到 reserve 那一步,不會卡到 cart 跟 pricing。
