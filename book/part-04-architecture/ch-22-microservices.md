...
## 22.1 冷觀察 ⸺ 黑五當天 inventory 服務 latency 上升,六個服務一起掛
...
## 22.2 真問題 ⸺ 微服務不是「規模化的方法」,是「強制購買的稅金清單」
...
### 22.2.1 從 Lewis & Fowler 2014 那篇定義開始
...
差異最致命的一條是**資料**。SOA 留了一個共享資料模型,所以後期常常退化成「分散式資料庫怪獸」;微服務刻意把這條切斷,要求每服務獨立 DB。但這條切斷的代價就是 §22.1 的故事 ⸺ 跨服務一致性,從一個 SQL transaction 的問題,變成一個分散式系統的問題。**微服務拿掉了 ESB 的單點瓶頸,但把交易一致性的責任推回給每一個團隊**。
...
| **網路稅** | 失敗會發生、重試會發生、亂序會發生、延遲會抖 | 等於 §22.1 那七分鐘 |
...
### 22.2.4 與 Modular Monolith 的決策延伸
...
## 22.3 決策框架 ⸺ 拆分四維、Saga、同步非同步、八大謬誤
...
四個維度全部通過,才有資格進「該不該拆」這一步。**任何一個沒通過,留在 Modular Monolith 比拆出去便宜**。TideCart 在 2023 年那一刀,四個維度過了零個半 ⸺ 規模沒到、團隊還在動、變動頻率其實都是月度、平台能力是零。32 份稅金準時開單,§22.1 是 23 個月後的帳單到期通知。
...
這張圖跟 Ch 21 §21.3.3 的決策樹精神一致,但這裡多了最後一格:**第五題是「能不能負擔稅金」**。前四題回答「邊界要不要切出去」;第五題回答「切出去之後,你願不願意付那六項稅」。沒到第五題那一格之前,任何拆分討論都太早。

### 22.3.3 八大謬誤對照表

Peter Deutsch 與 James Gosling 1994 年留下的「分散式系統八大謬誤」[^CIT-213],到 2026 年仍然是微服務團隊每一週都在踩的雷。把每條謬誤對應到 §22.1 的事故場景,大致長這樣:
...
### 22.3.4 跨服務交易:Saga Choreography vs Orchestration
...
- **非同步鏈**:失敗隱藏在 saga 補償與最終一致性視窗裡。inventory 掛,cart 照常接受加入購物車,但 reserve 步驟在 saga 裡靜默失敗,幾分鐘後才觸發補償;使用者以為結帳成功,實際上訂單可能被撤回。代價是可觀測性成本與業務邏輯複雜度大幅上升。

**TideCart 的庫存場景適合走非同步**,原因有三:⑴ 庫存緊張是促銷高峰的可預測現象,不是隨機異常;⑵ 「加入購物車」本就容忍最終一致(多個使用者可以同時把最後一件商品加入車,扣量延遲到 reserve 步驟才強制);⑶ inventory 服務的失敗不需要即時反映到 cart UX。

但**並非所有服務間的呼叫都應該非同步**。以金融支付為例:payment auth SLO 通常是 < 100ms 端到端,且業務上「扣款成功或失敗」必須在單次請求裡給出確定答案 ⸺ 這時走非同步意味著使用者先看到「處理中」,再接非同步通知,中間有一個未確定視窗。這個視窗在快時尚電商可能可以接受,在即時支付場景通常不行。

把 §22.1 改成非同步,TideCart 的故事會變這樣:cart 不會直接呼叫 inventory,而是發 `CartItemAdded` 事件;pricing 訂閱事件異步算價;結帳時 order-service 從本地 read model 直接讀「目前可售量(可能略舊)」做樂觀預訂,真正的扣量在 saga 裡 reserve。inventory 抖,只會卡到 reserve 那一步,不會卡到 cart 跟 pricing。**代價是:使用者有機會拿到「結帳成功」訊息,幾秒後收到「庫存不足,訂單取消」的補償通知** ⸺ 這在 TideCart 的業務設計裡被接受了,但它是業務決策,不是技術優化。

在決定走同步還是非同步之前,先問以下三個問題:

| 問題 | 同步 | 非同步 |
|---|---|---|
| **業務能接受最終一致嗎?** | 否:扣款、帳戶餘額更新、合規記錄 | 是:庫存預算顯示、推薦刷新、通知發送 |
| **使用者需要即時確定答案嗎?** | 是:checkout 的 payment 步驟 | 否:加入購物車、價格預算估算 |
| **失敗後補償成本高嗎?** | 低(fast fail,不產生副作用) | 高(已發出事件需要補償鏈追回) |

現場常用的判準:**讀走同步,寫操作的副作用走非同步**。但「寫操作」本身 ⸺ 特別是帶金融語義的那些 ⸺ 通常需要保留同步確認步驟,再把副作用非同步化。
...
TideCart 後來把 checkout 路徑上 22 次同步呼叫砍到 4 次(都是讀),其餘的副作用全部走 Kafka + Outbox,並在業務層接受「加入購物車的庫存顯示可能略舊」這個最終一致承諾。黑五復盤後一個月,P99 從 47 秒降到 1.2 秒 ⸺ 但配套的是 saga 補償鏈、DLQ 監控、以及向商務團隊解釋「結帳後仍可能取消」的流程溝通。
...
服務拆了 32 個,但每個請求要 hop 過 14 個服務、做 22 次同步呼叫,任何一個服務抖一下整條鏈就斷。表面上是微服務,實質上是「拆過的 monolith」⸺ 失去了 monolith 的單一交易、也沒拿到分散式系統的隔離。§22.1 TideCart 那七分鐘就是這一條。
...
> ✅ **修正方向**:用 §22.3.1 四維 + §22.3.2 決策樹做判斷,**四維全部通過才考慮拆**。團隊規模 < 50 人的階段,預設值是 Modular Monolith(Ch 21),需要獨立部署的就拆出 1–3 個邊緣服務(BFF、Webhook fanout、Search index),不要一次拆 30+ 個。判準:每要拆一個新服務,先過一遍決策樹,把答案寫進 ADR;答不出來的維度,就是這次不該拆的訊號。

---

## 22.5 交付清單 ⸺ 一頁式 Microservice Tax Card
...
**為什麼有「退場條件」?** 因為微服務不是單向決策。Ch 21 的 MeshFirst 跟 §22.1 的 TideCart 都示範了「拆出來之後收回去」這條路徑。在 Tax Card 裡先寫好退場條件,等於把「我們有勇氣承認這個服務不該繼續存在」這件事先制度化 ⸺ 不然兩年後它會變成沒人敢動的歷史包袱。
...
- [ ] 用 §22.3.1 四維 + §22.3.2 決策樹回答「這個模組要不要拆出去」⸺ 任何一維不通過就留在 Modulith,不要被「業界都在用」這種話帶走
