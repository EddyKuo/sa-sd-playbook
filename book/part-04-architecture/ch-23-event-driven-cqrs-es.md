...
## 23.1 冷觀察 ⸺ 授權 P99 從 80ms 變 320ms
...
沒有人答得出來 ⸺ 根本原因不是「Event Sourcing 不能上熱路徑」,而是**ES 上了熱路徑,卻沒有配套足夠的快取設計**。

具體數字如下:團隊把 Event Sourcing 套用在熱路徑(authorization aggregate)上,每一筆授權要先 fold 過去 90 天的事件流(平均 1,200 筆事件/卡)才能算出當前餘額狀態。理論上 snapshot 可以大幅壓縮 fold 成本,但他們只設定了**每 100 筆事件才打一次 snapshot** ⸺ 這意味在 snapshot 與最新事件之間仍可能有 99 筆要 fold。Postgres 的 `events` 表單表已經 47 億 row（合規要求保留 18 個月以上的歷史資料，約 60 萬張活躍卡每張每天產生約 13 筆事件，18 個月累積即達此量），單筆 fetch 從原本走 `cards.balance` 一行(0.8ms)變成 fetch 1,200 row + 應用層 fold(平均 38ms)。若 snapshot 頻率改為每 5 筆打一次,並且引入 pre-computed aggregate cache(授權前先讀 Redis 裡的最新快照,寫入後同步失效),fold 成本本可壓到 3–5ms ⸺ 但沒人做這件事。
...
接下來四週,團隊面前有兩條路。第一條:補足 ES 在熱路徑所需的配套 ⸺ 把 snapshot 頻率改為每 5 筆一次、引入 Redis 層快取最新聚合狀態、把 aggregate cache 的失效邏輯寫進 outbox publisher。這條路技術上可行,但工期估算是六週,且快取一致性錯誤會直接衝擊金融合規。第二條:把 Event Sourcing 拆出熱路徑,移到「對帳審計」與「合規重放」兩個冷子域,熱路徑改回 CRUD + outbox。

面對合約窗壓力,團隊選了第二條。熱路徑的 authorization aggregate 改回 CRUD + outbox + 事件通知(L2 Event-Carried State Transfer),CQRS 也只保留在報表 / 風控 dashboard,授權本身讀寫同庫。授權 P99 回到 75ms,reconciliation 的 read model 延遲也降到 P99 280ms。

這個選擇本身是對的 ⸺ 但理由**不是「ES 天生不適合熱路徑」**,而是「在這個時間壓力與團隊能力下,補足熱路徑 ES 所需的快取設計代價太高」。兩者是不同的判斷。

那位架構長離職前留了一句話在 Slack:**「這三件不是套餐,是三件不同層級的工具。」**

---

## 23.2 真問題 ⸺ EDA / CQRS / ES 是三層獨立工具
...
### 23.2.3 Event Sourcing 的真相:三個正當理由,以及熱路徑的快取代價

Fowler 在 2005 年那篇〈Event Sourcing〉與 Greg Young 後續的演講都強調:Event Sourcing 是一種**極端的工程選擇**[^CIT-222][^CIT-223]。它讓「狀態 = 事件 fold」成為唯一真理來源,代價是**寫入路徑需要 fold 或快取設計、查詢路徑必須重新設計、事件 schema 一旦發布就不能改、運維複雜度倍增**。
...
**ES 用在熱路徑是否可行?** 技術上可行,但有嚴格的配套前提。這是因為熱路徑的 fold 成本會直接衝擊 SLO:每次寫入前都必須重建聚合狀態,而狀態重建的唯一辦法就是大幅縮短 fold 窗口或繞過 fold。因此,ES 在熱路徑的可行性完全取決於以下四項配套設計能否同時到位:

| 配套設計 | 說明 | 若缺少的代價 |
|---|---|---|
| **高頻 snapshot**(每 5–10 筆一次) | 確保 fold 的事件窗口微小 | fold 1,200 筆 → 38ms,超 SLO |
| **pre-computed aggregate cache**(Redis / memcached) | 授權前讀快取,寫入後失效 | 每筆授權都需要 DB fetch + fold |
| **快取一致性協議** | 寫入與快取失效必須在同一 transaction 語義下完成 | 快取舊值導致重複授權或授權拒絕 |
| **ES-specific 運維能力** | 團隊能 debug snapshot lag、projection drift、event store 碎片化 | 上線後問題無人能解 |

CardPilot Asia 的根本錯誤是**上了 ES 熱路徑,卻沒有配齊上述任何一項**:snapshot 每 100 筆才一次、沒有 aggregate cache、團隊沒有 ES 運維經驗。換句話說,事故的精確教訓是:**「ES 用在熱路徑,需要主動 aggressive 的快取設計;如果做不到這個配套,才應該把 ES 移到冷路徑。」** 而不是「ES 天生不能上熱路徑」。

支付授權**不在三個正當理由裡**是另一個獨立判斷,必須與「快取代價太高」分開說清楚。CardPilot Asia 的決定其實是**兩層篩選**:

- **第一層(成本評估)**:評估熱路徑配套後,發現 snapshot 調頻 + Redis aggregate cache + 快取一致性協議的設計代價超出六週工期預算,且快取一致性錯誤會直接衝擊金融合規。
- **第二層(需求合理性)**:深究業務需求後,發現授權根本不需要 ES 的三個正當理由——授權需要的是「當下這一刻餘額是否足以授權」,稽核軌跡用 outbox + 不可變 audit log 就夠了。

**只要任一層 fail,就該把 ES 移出熱路徑。本案同時 fail 兩層,決定就更明確。** 這個分層至關重要:如果只有第一層 fail 但第二層不 fail(例如 CRDT 協作文件的即時衝突解析場景,歷史本身就是價值),那麼正確答案是咬牙補齊熱路徑的快取配套,而不是把 ES 撤出去。反之,如果第二層就 fail(業務根本不需要 ES),即使成本能承受,也沒有理由把 ES 放在熱路徑。
...
## 23.3 決策框架 ⸺ 該採用哪一層、什麼時候採用

### 23.3.1 一張決策樹:這次該採用哪一層

在細看各層的判準之前,先建立一張全局地圖。每次評估「要不要做事件驅動」,從這裡出發:
...
這張圖的關鍵是:**EDA 那條主線跟 CQRS 那條獨立軸是兩條獨立的問句**。把它們混成一個問題,就會回到 CardPilot Asia 那種套餐式採用。以下 §23.3.2–§23.3.5 逐一展開決策樹各分支的細節判準。
...
### 23.3.4 Event Sourcing 引入判準(三個正當理由)
...
### 23.3.6 Outbox Pattern:讓 EDA 與資料庫一致的最小劑量
...
### 23.3.9 2026 視角:AsyncAPI 3.0、AI Agent、OpenLineage
...
### 反模式 2:Event Sourcing 用在熱路徑,卻沒有配套快取設計

把交易主鏈、訂單主鏈、授權主鏈這類 OLTP 熱路徑做成 Event Sourcing,卻沒有配齊高頻 snapshot + pre-computed aggregate cache + 快取一致性協議。結果每筆寫入要 fold 整條事件流,SLO 緊的場景(支付授權、競價、即時計費)直接撞上 SLO 牆。注意:問題不是「ES 用了熱路徑」,而是「ES 用了熱路徑,卻沒補足它所需的配套」。

> ✅ **修正方向**:先問「這個 aggregate 的快取設計能做到 snapshot 每 5–10 筆一次、Redis 層 pre-computed cache、快取失效與寫入事務同步嗎?」如果能,ES 留在熱路徑可行。如果做不到,就把 ES 移到「冷子域」⸺ 對帳、稽核、合規、時間機器查詢 ⸺ 熱路徑改用 CRUD + outbox 廣播事件給冷子域訂閱。一個常用拇指法則:**寫入 SLO < 100ms 的 aggregate,除非已有完整快取配套,否則不做 ES**。CardPilot Asia 的授權正確結論是「不需要 ES(三個正當理由一個都不成立)」,而不只是「配套做不完」。
...
### 23.5.1 範例:CardPilot Asia 把熱路徑救回 75ms 的那張卡

授權 P99 從 80ms 變 320ms 的第八週,CardPilot Asia 沒有立刻動程式碼,而是把套餐拆成三張 Layer Card 重新評估。下面這張是**授權熱路徑(authorization aggregate)**那一張 ⸺ 寫到一半,團隊就發現 ES 與 CQRS 兩格根本勾不下去:

````markdown
# EDA Layer Card — Authorization 熱路徑(ISO 8583 1100)

> 撰寫日期:2026-02-19 | 擁有人:Auth Service Lead 阮
> 對齊:System Charter v1.2、Bounded Context = Authorization
> 狀態:Approved(取代上線時誤套的「三件套」設計)

## 1. EDA 層次選擇
- [ ] 無事件
- [ ] L1 Event Notification
- [x] **L2 Event-Carried State Transfer**(授權結果含金額 / 卡 / 時間,廣播給風控與對帳)
- [ ] L3 Event Sourcing

理由:下游風控、對帳、報表都需要授權結果的完整狀態,不應反向呼叫 Auth(會把 Auth 的 P99 拖下水);
但 ES 在熱路徑會逼每筆 fold 1,200 row,Q1 第八週的 320ms 事故就是這樣來的。

## 2. CQRS 是否必要
<!-- 為什麼這欄:授權的讀寫比接近 1:1,write-then-read 就在同一交易內;
     勾不下去就是勾不下去,不必為了「現代」硬上。 -->
- [ ] 讀寫比 > 10:1(實測 1.1:1)
- [ ] 讀寫 SLO 不對稱(都要 < 100ms)
- [ ] 讀寫擴展軸不同(都按 card_id sharding)
- [ ] 業務允許 stale read(授權必須 read-after-write)
- [ ] 多種讀視圖需求(授權只有一個視圖)

結論:**不引入**(報表 dashboard 那張卡才引入,在另一份 Layer Card)

## 3. Event Sourcing 是否必要
<!-- 為什麼這欄:三個正當理由一個都不成立,卻在 Q4 被誤套上熱路徑;
     寫進來就是要把這個錯誤鎖在 ADR 上不再犯。 -->
- [ ] 法遵 / 審計需要不可變稽核軌跡(用 outbox + immutable audit log 已足)
- [ ] 業務需要時間機器(授權只關心「當下餘額是否足夠」)
- [ ] 業務本身就是事件流(授權是 state machine,不是事件流)

結論:**不引入熱路徑**;ES 限定在 reconciliation 子域(另一份 Layer Card)

## 4. 訊息傳輸選型
| 候選 | 強項 | 弱項 | 選用 |
|---|---|---|---|
| Kafka 3.7 | 高吞吐、replay、Schema Registry | 運維重 | ☒ |
| 其他 | — | — | ☐ |

最終選擇:Kafka 3.7
**運維 Owner**:platform-team SRE 黃(凌晨 4 點 broker GC pause 接電話的人;
Q1 事故後從 Confluent 接受三週 onboarding 訓練)

## 5. Schema 治理
- [x] Schema Registry:Confluent Cloud
- [x] Format:Avro(向下 BACKWARD 相容)
- [x] 相容策略:BACKWARD
- [x] CI 檢查:`schema-compat-check`(PR 不過就紅燈)
- [x] Upcasting:有(`AuthApprovedV1 → V2` upcaster 已寫)

## 6. Idempotency 與一致性
<!-- 為什麼這欄:Q1 事故的第二顆雷就是「以為 exactly-once」;
     寫死「at-least-once + idempotent」才能擋下下次同樣的誤解。 -->
- [x] Producer:idempotent producer 開啟(`enable.idempotence=true`)
- [x] Outbox Pattern:採用 ⸺ 表名 `auth_outbox`(每筆授權同 tx 寫入)
- [x] Consumer 去重:`Idempotency-Key` + Postgres `ON CONFLICT DO NOTHING`
- [x] **接受 at-least-once,不假設 exactly-once**(寫進 runbook 與 onboarding doc)

## 7. Owner
| 區塊 | Owner | 副手 |
|---|---|---|
| Producer 邏輯 | Auth Service Lead 阮 | — |
| Schema 治理 | platform-team 趙 | 阮 |
| Broker 運維 | platform-team SRE 黃 | 趙 |
| Consumer(風控)| Risk Lead 林 | — |
| Read Model | (本卡未引入 CQRS) | — |
| Replay / Audit | Reconciliation Lead 蔡(另一張 Layer Card) | — |
````

把套餐拆成三張卡重評之後,熱路徑這張刻意把 CQRS 與 ES 兩格都勾「不引入」⸺ 不是退守,是承認**讀寫比 1.1:1 的 aggregate 不需要分家**,而 ES 的三個正當理由在這個子域一個都不成立。授權 P99 回到 75ms 那一週,團隊把這張卡釘在會議室牆上,作為下次「我們也來導入 X」的提醒。
