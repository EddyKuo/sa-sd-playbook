...
支付授權**不在三個正當理由裡**是另一個獨立判斷:授權需要的是「當下這一刻餘額是否足以授權」,稽核軌跡用 outbox + 不可變 audit log 就夠了。因此 CardPilot Asia 最終把 ES 移到冷路徑,是**兩個判斷同時成立的結果**:一、熱路徑配套代價太高;二、授權本身就不需要 ES 的三個理由。只有第一個理由成立但第二個不成立的場景(如 CRDT 協作文件的即時衝突解析),才需要咬牙補齊熱路徑的快取配套,而不是把 ES 撤出去。
...
### 23.3.3 Event Sourcing 引入判準(三個正當理由)
