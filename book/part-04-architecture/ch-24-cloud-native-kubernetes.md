...
PaaS(Render / Fly.io / Railway / Cloud Run)解決了 1、2、3,而且解得比 K8s 簡單。**4 是個迷思** ⸺ CNCF 2024 Annual Survey [^CIT-230] 的交叉問卷顯示,在宣稱「使用 K8s 以獲取跨雲可移植性」的組織中,實際執行過跨雲遷移的比例不足 5%。背後的原因不難理解:K8s manifest 只管得了應用的執行層,它管不了資料庫的備份與恢復流程(每個雲的 managed DB 介面、備份格式、PITR 機制差異極大)、IAM 信任邊界(AWS IAM Role / GCP Workload Identity / Azure Managed Identity 的模型本質不同,跨雲要重寫授權邏輯)、VPC peering 與 PrivateLink 的路由規則、以及 CloudWatch / Stackdriver / Azure Monitor 各自的 log sink 格式。舉一個具體例子:TenantForge 的 PostgreSQL 跑在 AWS RDS,要遷到 GCP Cloud SQL,光是 pg_dump / pg_restore 的停機視窗評估、sequence 與 extension 相容性、SSL 模式切換、應用連線字串的 DNS 切換就要寫一份五頁的 runbook ⸺ 這些工作跟 K8s manifest 完全無關。另一個常被忽略的例子是監控管線:Prometheus 的 remote_write 目標、告警規則的 routing、oncall rotation 的 PagerDuty 整合,全都跟 K8s 無關,跨雲時要從頭對接。這就是為什麼「K8s manifest 的可移植性」在實戰中幾乎沒有兌現 ⸺ 真正的跨雲成本 80% 在 manifest 管不到的那幾層。**5 跟 6 是 K8s 的真本事** ⸺ 但需要 Platform team 才用得起來。**7 不是技術理由**,但 2026 年仍在很多會議室裡決策。
...
2. **Platform team 的 ROI 開始為正**:這裡的計算建立在三個明確假設上,列出來方便讀者依自身情況調整:
   - **假設 A(Platform team 規模:4 人)**:50 人規模的 K8s 需要至少一人專管叢集升級與 CNI/CSI 問題、一人管 Helm 抽象與 GitOps 管線、一人管告警與 oncall rotation、一人做安全合規(PSA、RBAC、Secret rotation)。低於 4 人,上述任一面向就會掉入 feature team 手中,重現 TenantForge 的 35% 場景。上調到 5 人,ROI 計算仍為正,只是邊際較薄;在 30–40 人的組織,3 人 Platform team 可能已能覆蓋,但人員餘裕更低。
   - **假設 B(SRE 工時改進:15% → 5%)**:這個幅度來自 CNCF Survey [^CIT-230] 的中位數觀察 ⸺ 沒有統一 Platform 的 50 人組織,各 feature team 的基礎設施瑣事(logging 設定、secrets rotation、告警靜默規則)平均佔工時 12–18%,取 15% 為保守中間值。Platform team 統一做好後,個別 feature team 只需遵循模板與觸發 CI/CD,降至 3–7%,取 5% 為樂觀估計;若 Platform 成熟度中等,合理預估是降到 8–10%。
   - **假設 C(計算本身的敏感性)**:若 SRE 工時只從 15% 降到 8%(保守情境),省下 (15% − 8%) × 46 人 = 3.2 人工,仍能大致覆蓋 4 人 Platform team(3.2 ≈ 4,差距在 1 人工以內,其餘由規模效益補平)。若改進幅度更悲觀(只降到 12%),ROI 轉負 ⸺ 這就是「50 人是軟性拐點,不是硬性門檻」的理由:實際組織的觸發點可能在 45 人或 60 人,取決於產品線數量與基礎設施複雜度。

   把三個假設代入計算:50 人裡組一個 4 人 Platform team(佔 8% 工程人力),讓其餘 46 人的 SRE 工時從 15% 降到 5%,省下的 (15% − 5%) × 46 人 = 4.6 人工,恰好覆蓋那 4 人 Platform team 的成本。換句話說,**Platform team 讓整體 K8s 稅金從分散的 15% 收斂到集中的 8%,並且持續遞減**。對比 PaaS 的 3–5% 平台稅,差距縮到值得換取 K8s 帶來的靈活性。
...
在 50–500 人的組織,跨區域問題通常還在單一叢集的能力邊界內。常見的做法是利用 K8s 的原生機制應對:跨 AZ 的 zone 親和性(topologySpreadConstraints)讓 pod 在三個 AZ 均勻分佈,單一 AZ 掛掉時流量由其餘節點承接;region-level failover 則由 AWS Route 53 / GCP Cloud DNS 的健康檢查加上備援叢集切換處理。這些方案可以撐到組織擁有兩三個區域、十幾個 namespace、幾百個 workload ⸺ 也就是大多數 100–400 人的組織不需要多叢集 federation 的原因。

500 人規模時,上述單叢集方案開始失效,通常由以下幾個具體觸發點啟動,而不是「公司變大了」這種模糊說法:

1. **合規隔離需求**:歐盟 GDPR 要求個資資料落地在特定 region,而北美與歐洲的 workload 不能共用同一個 etcd ⸺ 單叢集的 namespace 隔離在法律層面不夠用,需要獨立 control plane。
2. **事業單位獨立 audit 要求**:兩個以上的事業單位(例如 B2B SaaS 部門與消費者產品部門)需要各自獨立的 Kubernetes audit log、RBAC 政策審計、以及 SOC 2 報告邊界。單叢集的 audit log 是混在一起的,分拆成本遠高於建兩個叢集。
3. **blast radius 難以控制**:500 人、30+ 個 feature team、幾千個 pod 全在一個叢集,一個 misconfigured admission webhook 或 node drain 問題可以波及全部 workload。單叢集的 blast radius 成了公司級風險。

一個可觀察到這個拐點的真實觸發案例形態:一家 300 人的 SaaS 公司在拿到第一個歐洲大客戶(要求資料落在 EU-West-1)後,發現他們的單叢集不符合客戶 DPA(Data Processing Agreement)裡「資料處理 infra 與美國租戶隔離」的條款 ⸺ 那才是他們開始建第二個叢集的觸發點,不是「公司人數到了某個數字」。這就是多叢集 federation 與自建 control plane 的真實觸發條件 ⸺ 合規、稽核隔離、blast radius,不是「我們在多個雲上跑了一些 workload」。
