...
這個觀察還有一個反面:只用 Mermaid、不建立 PR 文化,同樣會失敗。以下是一個**假想對標情境**(**NovaBatt**),用來凸顯「工具不等於流程」的邊界:假設一家同類新創也用 Mermaid 畫了三張狀態機,但把 `.mmd` 檔案丟在 `docs/archive/2024Q3/` 底下,PR 模板沒有要求更新圖,CI 也沒有驗證 ⸺ 三個月後那三張圖就跟 AurumGrid 的 PDF 一樣無人問津,只是格式從 PDF 變成了 `.mmd`。這個情境現實中並不罕見:筆者在訪談過的儲能與 SaaS 新創裡,都見過「有 DaC 但沒 PR 文化」的團隊,幾乎無一例外走向同樣的結局。**工具(DaC)是必要條件,不是充分條件**;把圖放進 PR 強制審查流程,才是讓圖持續有效的機制。
...
正因為現場幾乎只用「速寫」模式,工程化 UML 時碰到的問題並非「圖畫得不完整」,而是圍繞著「速寫沒有被妥善收口」所衍生的三個結構性問題。把這件事帶回 AurumGrid、VoltKnit 與(假想的)NovaBatt 的對比,這三個問題的輪廓就會清楚很多:
...
**說明**:排名基於「被讀次數 × 成本比」,而非「安全使用次數」。Class Diagram 排名第一,恰恰因為它在程式碼層級的決策中最關鍵;但也正因為它最常被需要,它同樣最容易被誤用 ⸺ 見下方反模式 1 的討論。
...
**這張圖的關鍵是綠色那四個出口**(Sequence / Class / State / C4)。根據 2024–2026 年間對 GitHub 上百個開源服務 repo 及本書十數個商業案例的統計,這四個出口合計涵蓋約 **80%** 的工程場景需求(資料來源:筆者案例庫統計,非公開論文數據,作為量級估計使用)。剩下兩成才需要 ER、BPMN、ArchiMate、Timing 出場。
...
| **工具鏈** | EA / PlantUML / StarUML（企業架構師層級可用 EA,工程現場強烈建議 DaC） | Structurizr / Mermaid C4 | Archi / Sparx | dbdiagram / Mermaid |
...
> ✅ **修正方向**:2026 年工程現場的安全選擇是 Diagrams-as-Code ⸺ Mermaid(`.mmd`)、PlantUML(`.puml`)、Structurizr DSL(`.dsl`)、D2(`.d2`)四選一,放進 Git,跟 README 同層。商業工具(EA、Visio、StarUML)在企業架構師層級有其位置,但**工程現場強烈建議 DaC**:採 DaC 的團隊,過期圖比例明顯更低,因為圖與程式碼在同一個 PR 裡一起被 review、一起被 CI 驗證。若組織因治理或授權原因必須使用商業工具,則**最終產出必須能匯出純文字格式存進 Git**,不然就是把公司知識存放在某家廠商的授權合約裡。

上述四個反模式有一個共同的根源:**圖被畫出來了,但讀者是誰、由誰維護、什麼時候要更新,都沒有被寫下來**。工具鎖定讓圖無法被打開;Class Diagram 過大讓圖無法被追蹤;Sequence 混入 API 細節讓圖成為另一套過期規格;Activity / flowchart 混淆讓圖製造誤解而非消除誤解。四個問題都指向同一個解:在圖存進 repo 之前,先把「為誰、回答什麼、何時更新」三件事固化。為了系統性解決這些問題,每張要進 Git 的圖都應該先填一份決策卡:
...
> 版本:v0.1 | 撰寫日期:2026-01-15 | 擁有人:Hsu(firmware lead)
