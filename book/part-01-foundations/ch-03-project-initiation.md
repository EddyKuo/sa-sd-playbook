...
換句話說,可行性研究真正要回答的不只是「我們做不做得出來」,還包括「在這個組織的權力結構裡,這件事走不走得到上線那一天」。後者通常是決定性的,但教科書版的 TELOS 不處理它。本章把 S(Schedule)從並列維度改成**前提約束**、再把 P(Political)補進第五維,得到一個比較貼合現場的版本:**TELOP**。時程不是不重要,而是它的正確位置是「填表之前先確認的輸入條件」,不是跟技術、經濟並列評分的維度(詳見 §3.3.1)。
...
值得深究的是:**護理部不出現在利害關係人清單上,不是個別疏失,而是結構性盲點**。具體而言,有三個相互強化的機制在運作:

1. **組織層級壁壘(Hierarchy Silo)**:MedCanvas 的 IT 專案走 CIO → COO 的匯報鏈;護理部的業務走護理部主任 → CMO 的匯報鏈。兩條線在院長室以下幾乎沒有交叉點。啟動會議的出席邀請沿著 IT 匯報鏈發出去,護理部自然不在名單上 ⸺ 不是刻意排除,而是**組織架構讓他們天然不可見**。
2. **角色模糊帶來的分類錯誤(Role Ambiguity)**:夜班護理長(charge nurse)在人力資源系統裡的職等是「護理師資深」,不是「主任」也不是「科長」。在組織圖上,她是執行層員工,不是決策者。沒有人問這個問題:**一個在人事系統裡是「執行層」的人,有沒有可能握有實質的阻斷能力?** 答案當然是有,但只有到現場的人才知道。
3. **成本偏誤壓低了訪談深度(Cost Bias)**:深夜輪班的護理站不是標準的訪談場景。安排一位顧問或 PM 在凌晨兩點到急診護理站做 shadowing,需要特別申請、保險、院方協助。這個成本夠高,所以被默默跳過了,換成「日班訓練計畫」來代替實際的現場觀察。

這三個因素任何一個單獨存在都可以被克服,但當三個同時作用,護理部就系統性地從可行性研究的視野裡消失了。**Power-Interest Grid 的功能,正是強迫分析者在填表時跨越匯報線**:你不能只填 CIO 匯報鏈上的名字,你得問「醫院裡高關注度的角色還有誰?」光是這一個問題,就會把夜班護理長從盲點中拉出來。
...
技術風險的記錄格式通常是「機率–衝擊–緩解方案」三欄。人風險的觸發機制不同,記錄格式需要另外四個維度:

| 欄位 | 說明 | 範例(護理主任異動) |
|---|---|---|
| **觸發人物/單位** | 誰的狀態改變會引發這個風險 | 護理部主任 |
| **觸發條件** | 什麼事件或時機點觸發 | 人事異動(主任換人)、工會改選 |
| **檢測機制** | 怎麼提早發現風險正在發生 | 每月一次與護理部的 1:1 對話紀錄;HR 異動通報清單 |
| **緩解負責人** | 誰負責在觸發後介入 | PM 負責在 48 小時內重新訪談新主任 |

這四欄的核心邏輯是:**技術風險可以靠測試監控,人風險靠的是人際接觸頻率**。沒有定期對話紀錄,就等於沒有檢測機制;風險一旦觸發時,往往已經來不及介入。
...
TELOP 是在傳統 TELOS 的基礎上,把 S(Schedule,排程)從維度改成**前提約束**,再把 P(Political,政治)補進第五維。這個調整需要解釋清楚,因為它跟多數教科書的寫法不一樣。

**為什麼 Schedule 是約束而不是維度?**

Schedule 的問題性質不同於其他四個維度。技術、經濟、法律、操作、政治這五者問的是「這件事行不行得通」;Schedule 問的是「在這個時間盒(time-box)內行不行得通」。也就是說,Schedule 是**其他所有維度的輸入條件**,不是跟它們並列的判斷維度。

但這不代表時程不重要 ⸺ 恰恰相反。**時程是可行性評估的前提,必須在填表之前先確認**。具體做法是:先定義一個時間約束(例如:12 個月 PoC、上線截止 2025-03-31),再問「在這個時間約束內,T/E/L/O/P 五個維度各自的評分是什麼?」如果某個維度在這個時間約束下評分 ≤ 1,有兩種可能:一是維度本身有問題需要解決,二是時間約束太緊、根本無法在這個期限內到達可行狀態。兩者都需要重新校準,差異在**校準方向**。

反例說明這個邊界:假設同樣是 MedCanvas 案,T/E/L/O/P 五個維度都評分 ≥ 2,但 L 維度的子項目是「取得 HIPAA-like 個資稽核認證」。合規顧問告訴你:從零開始取得那張認證,最快 18 個月。如果合約規定 PoC 要在 6 個月內完成驗收,這個案子不是「L 維度不可行」,而是「在 6 個月的時間盒裡,L 維度的可行性評分自動跌到 0」。因果鏈是:合規認證工期 18 個月 > 時間盒 6 個月 → PoC 階段無法取得合規認證 → PoC 驗收無法通過法遵閘門 → 案子進不了上線評估。這個失敗是時間盒壓出來的,不是技術或法律本身的問題。

**把這種情況誤診為「L 有問題」,指的是誤以為法律要求本身太嚴苛、進而試圖繞過合規要求(例如「先上線後補認證」、「PoC 階段豁免適用」)**。這才是真正的誤判 ⸺ 繞過合規紅線會在後期引發更大的停案風險。正確做法是承認時間盒在當前條件下不可行,重新校準時間盒(例如把 PoC 截止日改為 18 個月後),而不是試圖繞過 L 維度的限制。「改時間盒」與「繞過法律要求」是兩個完全不同的方向;啟動階段最常見的錯誤,就是把前者誤當後者來處理。

| 維度 | 核心提問(在約定的時間約束內) | 不通過的訊號 | 可行的訊號 |
|---|---|---|---|
...
| **L 法律 Legal** | 我們踩到任何紅線嗎?合規準備能在時間盒內完成嗎? | 個資 / 病歷 / 金流的合規答案是「應該還好」;或合規工期超出時間約束 | 法遵單位已書面 sign-off 範圍,且所需認證能在截止日前取得 |
...
> **時間約束(Schedule Constraint)**:在填上表之前先寫在這裡 → `12 個月 PoC / 上線截止日:YYYY-MM-DD`。若任一維度在此約束下評分 ≤ 1,停下來校準,分清楚是「維度本身問題」還是「時間盒太緊」,兩者的解法不同。

每個維度用 0–3 分:0 = 顯然不行,3 = 顯然可行。**任何一個維度 ≤ 1,案子就應該停下來重新校準,不是繼續往下做**。MedCanvas 的案例,在 P 維度其實只有 0,但這個維度當時根本沒被列上。
...
實務上,Power-Interest 適合多數 M 級專案,Salience 適合 L 級(政府、醫療、能源、跨國)。兩者的選擇依據在於組織複雜度:M 級專案的權力結構相對單純,決策者少、匯報鏈清晰,Power-Interest 的二維分類(誰有權力、誰在乎)已足以涵蓋大部分的 stakeholder 動態。L 級組織(醫療、政府機關)的 stakeholder 多且**權力來源多元** ⸺ 除了正式職位帶來的指揮權,還有工會代表資格、醫評委員資格、媒體聲量、道德倡議平台等非正式影響力。這些影響力不能被「低權力」這個格子概括:夜班護理長在人力資源系統裡是「執行層」,但她代表的工會訴求在院長面前具有合法性(Legitimacy)且有持續施壓的急迫性(Urgency)。Salience Model 的三維度(Power / Legitimacy / Urgency)正是為了捕捉這類「無正式頭銜但有道德訴求與媒體影響力」的群體,這是 Power-Interest 二維座標做不到的。

**核心不是用哪個模型,是有沒有真的把名字寫下來、把象限指派下去、把接觸方式排進行事曆**。沒寫下來等於沒做。
