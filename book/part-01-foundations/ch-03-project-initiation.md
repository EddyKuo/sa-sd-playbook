...
````markdown
# Project Initiation Brief — 聖恩醫療體系 EMR 替換案

> 版本:v0.1 | 撰寫日期:2024-02-19 | 撰寫人:Tina(MedCanvas PM)
> 複雜度初判:L  ←(三家分院、1,200 床、HIPAA-like 合規)

## 1. Why Now(為什麼現在做)
<!-- 為什麼這欄:沒有觸發事件就沒有時間壓力,案子很容易被「再評估一年」拖死;
     寫不出觸發事件代表現在做這個案子的理由還沒成熟。 -->
- 觸發事件:現有 EMR 廠商 2026 Q4 停止維護;HL7 v2.3 與健保 2025 新格式不相容
- 不做的代價:2027 起每月健保申報退件率推估 > 18%(NT$2,800 萬/月);
  且 HIPAA-like 個資稽核已連兩年被列改善項目

## 2. TELOP 五維初判
<!-- 為什麼這欄:任一維度 ≤ 1 就該停下來;
     P 放最後是逼自己不能空著交,空著就是還沒做完啟動評估。 -->
| 維度 | 評分 (0–3) | 紅旗 | 主要 Owner |
|---|---|---|---|
| T 技術 | 3 | FHIR R4 團隊有兩位資深可用 | CIO |
| E 經濟 | 2 | ROI 假設靠效率提升 12%,需保守情境再驗 | CFO |
| L 法律 | 2 | 個資法施行細則 2024Q3 修法草案待觀察 | 法遵長 |
| O 操作 | 1 | **訓練計畫尚未做過 shadowing,動線改變未量化** | UX Lead |
| P 政治 | **0** | **護理部、夜班護理長、護理工會皆未訪談** | PM |

> P=0 與 O=1 即停下重新校準,Charter 不續推。

## 3. 利害關係人初步盤點
<!-- 為什麼這欄:組織圖上的人只是第一層,
     沒寫出隱性否決者代表 stakeholder 分析還停在 sponsor / IT 三人組。 -->
- 已識別的高權力 stakeholder 共 9 位,其中已實質接觸 4 位
- 隱性否決者清單:護理工會、感染管制委員會、醫院評鑑準備小組、院長室秘書處
- 預期最大政治阻力來源:護理部(三班交班動線改變,未事先 shadowing)

## 4. 風險清冊起手(Top 5)
| # | 風險描述 | 機率 | 衝擊 | 類別 | 緩解策略 | Owner |
|---|---|---|---|---|---|---|
| 1 | 三班交班動線在新系統需點擊 ≥ 5 下,夜班集體反彈 | H | H | 組織 | 第 1 月內完成 3 場 shadowing | UX Lead |
| 2 | 個資法施行細則修法,FHIR 資源範圍需重切 | M | H | 法規 | 法遵每月閘門檢核 | 法遵長 |
| 3 | 護理工會理事改選,新理事長立場未定 | M | H | 政治 | 雙週 1:1 + 工會 workshop | PM |
| 4 | HL7 v2.5 與舊系統 v2.3 訊息映射缺漏 | M | M | 技術 | PoC 第 3 月完成全訊息 diff | RD Lead |
| 5 | 醫評時程 2025Q2,系統穩定度未達標 | L | H | 組織 | 將醫評日期納入 release plan | CIO |

## 5. 下一個 Gate 的判斷標準
- [ ] P 維度補完訪談、評分 ≥ 2,且 O 維度完成 shadowing,評分 ≥ 2
- 不通過則退回:預算重劃 + Sponsor 重新評估是否啟案
- 決策者:王院長 + CIO 共同決定
- 決策日期:2024-03-15

## 6. Out of Scope(明確不在這次啟動範圍)
- **不做**門診與檢驗科系統替換,本期僅急診 + 住院兩科
- **不做**舊系統資料庫遷移,先以雙寫並行 6 個月再評估
````

把這頁交給院長前,P=0 那一格已經夠擋下 12 個月的 PoC ⸺ 啟動階段最有用的不是漂亮的可行性表,是**敢於在第一頁寫 0 分**那種誠實。
