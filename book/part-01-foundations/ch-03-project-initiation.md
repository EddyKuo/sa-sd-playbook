...
反例說明這個邊界:假設同樣是 MedCanvas 案,T/E/L/O/P 五個維度都評分 ≥ 2,但 L 維度的子項目是「取得 HIPAA-like 個資稽核認證」。合規顧問告訴你:從零開始取得那張認證,最快 18 個月。如果合約規定 PoC 要在 6 個月內完成驗收,這個案子不是「L 維度不可行」,而是「在 6 個月的時間盒裡,L 維度的可行性評分自動跌到 0」。這個失敗是時間盒壓出來的,不是技術或法律本身的問題。**把這種情況誤診為「L 有問題」而去改法律方案,是啟動階段常見的誤判**。
