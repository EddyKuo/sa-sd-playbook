...
> **前置閱讀**:無(本書起點)
> **下游章節**:[Ch 2 SDLC 演進](./ch-02-sdlc-evolution.md)、[Ch 30 ADR](../part-06-engineering/ch-30-adr-architecture-knowledge.md)
> **延伸補章**:[補章 C 遺留現代化](../part-07-ai-era/chC-legacy-ai.md)
...
第 18 天,他們收第一筆 50 美元。第 60 天,月流水破百萬美元。第 180 天,新加坡端發生一次 SGD 流動性事件,他們的對帳對不上,差額 8,400 美元,全公司沒人能在六小時內回答這個問題:

> 「這筆交易在我們系統裡走了哪幾步?哪一步寫了哪張表?哪張表是 source of truth?」

不是因為他們笨,而是因為**這個答案從來沒有從 AI 的會話裡出來過**。當時寫程式的那位工程師早就離職了,他在 Cursor 裡的 chat history 沒有 export,他做過的決定也沒有寫進任何一份文件。三個月後接手的人,只看得到 git diff,看不到 *為什麼這樣 diff*。
...
這不是 AI 的鍋,是**工程實踐的鍋**。AI 沒有失憶,是「決定發生時的脈絡」從來沒有被持久化。
...
人類大腦的工作記憶大約是 7±2 個 chunk。Claude 4.7 的 1M context 看起來很大,但**有效注意力範圍**遠小於 1M token,長 context 下的精度衰減已經有大量公開研究在記錄。所以當一個系統的日常決策都需要「全部上下文同時在腦中」,人會吃力、AI 會幻覺,兩者都會出錯。

SA/SD 的核心技藝就是**把上下文壓縮到可被局部推理**:Bounded Context、模組邊界、API 合約、ADR ⸺ 這些都是同一個目的的不同手段:讓你不需要看全部,就能放心改一部分。
...
| **必交付 artifact** | System Charter(本章末尾)+ ADR | + 精簡 PRD + C4 Container 圖 | 全套(SRS / C4 全層 / Threat Model / 合規對照) |
...
3. **一張 Container 級 C4 圖**(`Ch 19` 詳述)
...
> ✅ **修正方向**:C4 圖跟 repo 一起版本控制(`docs/architecture/*.dsl`),CI 中加一條 Fitness Function 驗證真實依賴是否還符合圖。具體做法在 [Ch 31](../part-06-engineering/ch-31-fitness-functions.md) 會詳細展開。
...
- **深入決策紀錄**:[Ch 30 架構決策紀錄(ADR)](../part-06-engineering/ch-30-adr-architecture-knowledge.md)
- **AI 時代的脈絡工程**:[Ch 34 Context-Driven Engineering](../part-07-ai-era/ch-34-context-driven-engineering.md)
- **遺留系統如何補做 SA/SD**:[補章 C](../part-07-ai-era/chC-legacy-ai.md)
