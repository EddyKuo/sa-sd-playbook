...
這週的重寫成本,根本上源於 backlog 卡沒有驗收標準。工程師在 code review 或上線前的效能測試階段,面對的只有「完成匯出 PPT」這個描述,**無法做出「慢到不符合需求」的客觀判斷** ⸺ 因為「多慢算慢」從來沒有被寫下來。這不是工程疏失,也不是 PM 溝通不足,是文件缺乏可驗證的數字。假如 NFR 當初寫成 `Response Measure: P95 ≤ 30 秒`,工程師在上線前跑一次效能測試,就能直接發現問題,不需要等客戶驗收後才回頭重做。
...
```mermaid
flowchart TD
    Start([有人提需求]) --> Q1{"能寫成 User Story /<br/>Job Story / Use Case 嗎?<br/>(工程師能寫出驗收測試?)"}
...
這裡的數字代表**產品成熟度**,而非任何單一指標 ⸺ 以 DAU 為參考座標:0–1 指探索 PMF 階段(DAU < 100),使用者輪廓尚未收斂,場景比角色更重要,此時 Job Story 的「When 情境」能捕捉到角色不確定的需求；1–10 指已找到商業模式但用戶仍在快速流動(DAU 100–10K),使用者輪廓已知、迭代以週為單位,User Story 能直接拆進 sprint；10–100 指規模化、多角色、涉及第三方整合或法規簽署(DAU > 10K 或有合約義務),此時 Use Case 的完整流程與例外情境描述對交付與合規都至關重要。如果組織同時混合 Agile sprint 與合約交付(例如同時服務內部產品團隊與外部客戶),**User Story 進 sprint board、Use Case 進合約附件** ⸺ 兩份文件服務不同讀者,不是彼此替代。
...
| REQ-0088 | 客製化 SKU 編碼引擎 | 客戶 B | 屬於單一客戶 Indifferent 需求,不擴大產品邊界 | 2026-04-20 | 不再評估 |
...
- 任何來自單一客戶且 Kano = Indifferent 的需求,默認進 No 欄,「重新評估時點」標「不再評估」。
- 與既有 ADR 抵觸的需求,默認進 No 欄,「重新評估時點」標「不再評估」,並回連結到衝突的 ADR。
- 估點 > 5 SP 但 Reach 覆蓋 < 10% 用戶的需求,默認進 Defer 欄,標具體季度或觸發條件。

> **「重新評估時點」三類規則**:① `不再評估` — 基於核心產品原則的永久拒絕（如 Indifferent、ADR 鎖死路線）；② 具體季度（如 `2026-Q3`）— 依賴外部條件尚未滿足，條件一旦改變即重評；③ 迴圈時點（如 `每月初`）— 定期檢視，無特定觸發。Defer 欄的需求通常標 ② 或 ③；No 欄的需求通常標 ①，除非是「暫時衝突但未來可能解除」的情況。
...
| REQ-0088 | 客製化 SKU 編碼引擎 | 客戶 B(年費 $58K) | 單一客戶 Indifferent,不擴大產品邊界 | 2025-09-09 | 不再評估 |
...
- 任何來自單一客戶且 Kano = Indifferent 的需求 → 默認進 No,標「不再評估」,引用 REQ-0088
- 與既有 ADR 抵觸的需求 → 默認進 No,標「不再評估」,並回連結到衝突的 ADR
- 月費 > $30K 的客戶提出但 Reach < 5% 用戶的需求 → 默認進 Defer,並由 CSM 用配置/服務交付,不進產品
- 任何「客製化」字樣的需求 → 走配置或 Marketplace,不進核心 backlog

<!-- 「重新評估時點」的三類邏輯：
     ① 「不再評估」— 基於核心產品原則的拒絕，永久不重評（如單一 Indifferent、ADR 鎖死路線）；
     ② 具體季度/日期（如 2026-Q1）— 拒絕依賴外部條件（ADR 尚未定案、流量未達門檻），條件滿足即重評；
     ③ 迴圈時點（每月初）— 定期檢視，但無特定觸發。
     依這個邏輯，REQ-0088 屬於「基於原則」的拒絕，應標「不再評估」；
     REQ-0118 依賴 ADR-0007 是否鬆綁，標具體季度。 -->
