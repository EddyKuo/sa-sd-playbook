...
- **Deployment Tempo(部署節奏)**:ShelfMart 的真正硬傷。Magento 2 繼承自 2007 年設計理念的架構 + 自製 OMS 緊耦合,任何可見的消費者功能都要碰到三到四個模組,缺乏獨立部署能力。就算 Q1 第一天就拍板做掃碼結帳,也不可能在六週內安全上線 ⸺ 因為架構上跑不通,不是流程上拖太久。這裡的因果機制值得拆開說清楚:耦合意味著掃碼結帳這個「單一消費者功能」,實際上是一個橫跨 Magento 前台、自製 OMS、倉儲 API、推播通知四個模組的「全站變更」。全站變更帶來的不是開發難度倍增,而是**風險等級跳升** ⸺ 任何一個模組的測試未過,整個 release 就得卡住。測試矩陣從「N 個情境」變成「N × 4 個整合情境」,QA 時間成倍拉長;跨模組的 change review 要各模組 owner 分別確認,審批路徑從一條變四條。結果就是:即使決定做了、開發完成了,「安全上線」這道門仍然需要半年的大版本視窗才能通過。這就是為什麼 Deployment Tempo 受限的不是流程速度,而是架構耦合所決定的最小安全發布單位。
...
三個瓶頸的性質不同 ⸺ Business Tempo 是市場問題、Decision Tempo 是流程問題、Deployment Tempo 是架構問題 ⸺ 因此治法也必然不同。要理解這三種治法的邏輯從哪裡來,最直接的辦法是回頭問:在不同的歷史時期,當工程組織面對的節奏失配形態各異時,他們分別發明了什麼來應對?這個問題的答案,就是 SDLC 演進史。看這段歷史的正確視角不是「哪個方法論比較先進」,而是**「它是在什麼時代的什麼瓶頸下被擠出來的」** ⸺ 把這個脈絡補進去,每個方法論的適用範圍和侷限都會變得清晰,而不是一堆名詞在競爭「誰才是對的」。
...
- **DDD** 解的是「業務語意」⸺ Bounded Context、Ubiquitous Language。它處理的是**業務節奏**維度:讓系統邊界與業務變化的粒度一致,使不同部門的功能可以獨立演化,而不互相干擾。它跟瀑布、敏捷、CDE 都相容,因為它處理的是「拆問題」這層,與執行工具無關。
- **Cloud-Native** 解的是「執行節奏」⸺ 容器、K8s、Service Mesh,讓部署從週縮到分鐘。它處理的是**部署節奏**維度:讓已經拆對的服務能夠獨立、快速、安全地上線。
- **CDE** 解的是「協作介面」⸺ 把脈絡寫成 AI 可讀的格式,讓決定不會隨人或會話消失[^CIT-025]。它處理的是**決策節奏**維度:讓架構意圖能跨時間、跨 Agent、跨人員持續傳遞,而不需要每次重建脈絡。

三層分別對應三個不同的節奏瓶頸 ⸺ 這也是它們互相依賴、缺一不可的根本原因。

ShelfMart 的真正困境,不是「沒做敏捷」,是這三層都還停留在 1990 年代。**只升一層通常救不了**,因為每層的缺口都會擋住其他層的效果:DDD 缺失時,Bounded Context 拆錯,即使有 Cloud-Native,也只是把邊界錯誤的服務容器化 ⸺ 部署更快了,但每次部署的範圍仍然是全站,Deployment Tempo 沒有改善。反過來,只上 Cloud-Native 但 DDD 不到位,K8s 就變成貴的擺飾。只上 CDE 但組織決策模式不變,spec.md 缺少對應的 ADR 流程支撐,很快就淪為另一份過期文件,而不是真正的協作介面。三個缺口要同時補,才能讓升級的效果真正累積。
