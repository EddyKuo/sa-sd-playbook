...
你可能注意到 § 36.2.3 定義的是「四個機制」，但這裡展開成了「五個層次」。對應關係如下：§ 36.2.3 的 Policy-as-Code 對應層次一（機制/政策分離）；黃金路徑是層次一的**組織形式**——它定義了政策層「什麼時候走標準流程、什麼時候允許偏離」（層次二）；複雜度預算是 Policy-as-Code 在量化管控上的延伸（層次三）；決策稽核軌跡獨立成層次四；分層審批對應層次五。換句話說，**這四個機制在實施時會層疊組成五個層次，每層都有獨立的檢查點，但彼此相互支撐**。

接下來逐層拆。

### 36.3.2 層次一：機制與政策分離（Mechanism-Policy Separation）

第一層的基礎是架構分層。機制層（通用、穩定）與政策層（可變、可稽核）的分離決定了上面四層能否有效運作——沒有這層分離，後續的複雜度預算、稽核軌跡都會污染到程式碼邏輯本身，讓你在追蹤「誰改了什麼規則」時陷入「規則和程式碼混在一起、根本分不清邊界」的困境。這就是它必須是層次一的理由。
...
黃金路徑（層次二）與機制/政策分離（層次一）的差別在於**粒度不同**：層次一回答的是「規則應該存在哪裡（政策層，不是資料庫）」，層次二回答的是「規則要怎麼進來（走哪條流程）」。黃金路徑是政策層的治理入口，不是政策層本身。
...
具體說明「偏離路徑」長什麼樣：假設某個企業客戶因特殊業務協議需要臨時提高台灣單筆匯款上限至 NTD 80,000（高於標準閾值 50,000）。這個例外**不能**直接改 Rego 檔後馬上合併——那樣繞過了正常 PR 流程，等於走暗門。正確做法是：在偏離登記表建立一筆記錄，填入例外原因（企業協議編號）、生效期限（e.g. 三個月）、exception owner（合規 lead Lin）、以及對應的客戶 ID 範圍。之後才提 PR、走中級審批。**黃金路徑的價值就在這裡：80% 常態場景走簡單流程（無審批成本），20% 例外場景走有紀錄的複雜流程（審批留軌跡）**——讓彈性點被使用的每一次都是可查的，而不是消失在 `UPDATE` 的漩渦裡。
...
1. **退場一個舊 flag（同 PR 提交）**：找出一個已確認無作用的舊 flag（e.g. 對應的 A/B test 已結束、某地區法規已穩定），在同一個 PR 裡把它標記為 deprecated 並移除對應的 Rego policy。CI 重新計數，通常可以讓新 flag 進得去。這條路是正路——強迫你在加法之前先做減法。
2. **向 governance owner 提出 exception request**：說明為什麼這個新 flag 無法透過退場舊 flag 來換，例如「所有現存 flag 都處於活躍業務場景中，無法在這個時間點退場」。exception 需要說明期限（e.g. 下季 flag 重整前的臨時豁免）與後續退場計畫。

針對**系統當前已混亂**（flag 數已超過任何合理預算）的情況，建議第一個季度不要立刻設定目標值，而是先做一次盤點：把所有 flag 過一遍，標出「可立即退場」「有退場計畫」「長期維持」三類。第一個季度的預算上限設成**當前實測數量**（不阻斷任何現有 flag，但阻斷凈增長），然後每月遞減 5–10%，逐步改善到目標狀態。這避免了「一刀切設嚴格預算 → CI 全紅 → 沒人理睬」的失敗模式。
...
分層審批的關鍵是**正確判定風險等級**。§ 36.5 會提到一份「Governance Card」模板，裡面的第 4 節包含了針對每個彈性點的風險判定清單（合規 / 安全 / 架構 / 行為路徑四個維度）。現在先理解這層的原理與等級劃分，等讀完全章後，你可以用那份清單來判定你自己系統裡的彈性點應該落在哪個等級。
...
> ✅ **修正方向**：從版本控制下的 YAML / JSON 政策檔開始，一條規則對一個檔案，跑 schema validate 就好。等到規則總數超過 30 條、跨服務共用、需要型別推論時，才升級到 OPA / Cedar。**治理機制的複雜度上限永遠要低於被治理的系統**，不然就是用更大的洞補小的洞。NovaPay 整改第一階段只用 Rego 的最基本 deny 規則，沒碰 partial evaluation、external data 這些進階功能，半年後再評估升級。複雜治理工具之所以是反模式，根源在它**違反了「機制層應永遠簡單穩定」這個原則**（§ 36.3.2）。當治理工具本身變成下一個禁忌神器時，代表機制層已經被污染——你需要一套工具來治理你的治理工具，這是遞迴陷阱。回到 § 36.3.2 的定義，重新用簡單的 YAML + schema validate 清理治理工具本身，才是正確的出口。
