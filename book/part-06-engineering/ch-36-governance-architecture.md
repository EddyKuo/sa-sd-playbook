...
sequenceDiagram
    ...
    Note over Code,DB: Day 0 上線：彈性極高
...
「治理（Governance）」這個詞在中文世界容易被誤讀成「管理委員會」「審批表單」這類管理學概念。把它放回工程語境，它要回答的其實只有一個問題：

> 「**當系統允許被改變的部分被改變時，行為仍然可預測、可追蹤、可復原嗎？**」

這個問題分開來看是三件事：

- **可預測（Predictable）**：在 PR review 階段，能不能告訴開發者「你這個改動會影響哪些行為路徑」？
- **可追蹤（Traceable）**：六個月後，能不能回答「這個值在 2025-09-15 是多少、誰改的、依據哪份 ticket / ADR」？
- **可復原（Recoverable）**：發生問題時，能不能在 5 分鐘內回到上一個已知好的狀態？

NovaPay 的稽核問題其實是「可追蹤」失守。但如果再深問，它的「可預測」也沒有 ⸺ 改 threshold 之前，沒有人能告訴你「這會影響哪些已上線的合約規則」。可復原更不用提，UPDATE 蓋掉就回不去了。

這一章接下來要做的，是把這三個問題的工程答案攤開來，組成一套五層的治理架構。
