...
她週一上午約了當時的後端 lead(在公司第三年),下午約了 SRE lead(在公司第二年),週二約了風控組長(剛加入半年)、KYC 組長、清算組長、平台組長,週三約了 CTO 與最早的兩位創始工程師。一週下來,**每一道題問到的版本都不完整,彼此之間空白處還對不上**。

她把答案攤在 Notion 上做交叉比對。光是「為什麼風控引擎是自研」這一題就收到四種不同說法,而且彼此只有部分重疊:
...
四個答案沒有彼此矛盾,但**也沒有一個能完整還原當時的決策脈絡**。原始的權衡 ⸺ 延遲、成本、模型準確度、合規 ⸺ 散在四個人的腦袋裡,湊起來才是完整的;而其中兩位早期關鍵決策者,一位已經離職、一位記憶模糊。其他五道題也一樣:每題都有過討論的痕跡,但沒有任何一份文件能對六個月或三年後的人交代「**當時的完整理由**」。
...
> 「OrbitPay 不是沒做過架構決策,也不是沒有討論過 ⸺ 是**沒有任何一份文件被設計成『三年後還能挖出來當作 source of truth』**。」

這句話刺耳的不是「沒寫下來」這件事,而是 ⸺ 工程團隊一直以為自己有寫。Slack 的 thread、Confluence 的 RFC、Notion 的草稿、git commit、白板照片,**他們以為這些加總起來等於「有紀錄」**。但盡職調查那一週證明了:**散落的紀錄,等於沒有可被信賴的 source of truth**。OrbitPay 的 `docs/adr/` 裡確實有 11 份 ADR ⸺ 全部時間戳是 2022 年 Q1 公司剛成立那兩週。公司成立後做的上百個重要決策,只有那頭兩週的氣候被保存下來,其餘全部蒸發在對話流和人的記憶裡。
...
### 33.2.2 判準很簡單:合規逃生門 + 改回來成本 + 外部介面 + 不可逆

換句話說,寫不寫 ADR 不該靠「感覺重不重要」,而該靠結構性條件。Ch 1 §1.4 已經提過一個簡化版,本章把它寫成可直接抄走的完整判準,分兩層:

**第一層 ⸺ 合規逃生門(必寫,不論以下三條是否成立)**

> 只要決策**涉及合規、安全、隱私、或錢**,不管改回來多容易、是否影響外部介面,一律寫 ADR。

理由很直接:稽核員要看的是「決策當時有沒有被記錄」,不是「決策事後容不容易改」。PCI DSS、SOC 2、GDPR 場景下,「我們選了 AWS KMS 因為感覺比較好」在合規文件裡沒有任何效力;「我們選了 AWS KMS,理由記錄在 ADR-0042」才有。§33.3.5 的決策樹 Q0 對應這一層。

**第二層 ⸺ 三條結構性條件(可選條件，任兩條成立則寫；適用於不涉及合規的決策)**

> **條件 A:改回來成本高**(reversibility cost)── 對應決策樹 Q1  
> **條件 B:影響至少一個外部介面**(external surface)── 對應決策樹 Q2  
> **條件 C:不可逆或半不可逆**(irreversibility)── 對應決策樹 Q4 的「接手者會問為什麼」作為代理指標

三條中**任兩條成立,就值得寫 ADR**。

OrbitPay 那六題對照這個判準:

- 「為什麼 KMS 用 AWS KMS」⸺ 先過合規逃生門(PCI DSS),**直接寫 ADR**,不需要再看三條。
- 「為什麼 Kafka 不用 Pulsar」⸺ 條件 A(改回來要重寫所有 producer/consumer)+ 條件 B(影響服務間契約)+ 條件 C(半不可逆)。**三條都中,該寫 ADR**。
- 「為什麼 commit message 用英文」⸺ 條件 A 不中(零成本)、條件 B 不中、條件 C 不中。**零條中,不該寫 ADR**(寫進 CONTRIBUTING.md 即可)。

這個判準的好處是它不依賴主觀「感覺重不重要」,而是四個可驗證的結構性問題。團隊可以在 PR 模板裡放這四題,作為 ADR triage 的第一道閘門。
...
反例說明問題的嚴重性。若 OrbitPay 的 ADR 只寫「我們選了 PostgreSQL 因為它很好用」,當 Agent 受命評估是否遷移到 MongoDB 時,它在 context window 裡找不到任何關於 partial index 效能或 multi-tenant P95 的約束,會合理地推論「PostgreSQL 和 MongoDB 在這個場景沒有本質差異」,進而建議遷移是低風險選項。而實際上 ADR-0036 隱藏的關鍵資訊是:「partial index 讓 multi-tenant 熱路徑 P95 比 MySQL 8 快 3.2x,MongoDB 沒有等效機制」— 這句話被藏在人的腦袋裡,沒進 ADR,Agent 就推不出「遷走會讓 P95 惡化」的結論。**模糊 ADR 不只讓人類讀不懂,還會讓 Agent 做出和原始設計意圖相違背的推薦**。
...
> **關於 Q1「1 週工程時間」門檻的背景**:這個數字是業界經驗法則,對應「決策改變後 restart 成本 ≥ 一個 sprint 週期」— 也就是說,如果把決策改回來需要佔掉整個 sprint,那這個決策就值得留下化石。你的團隊若跑雙週 sprint,可把門檻調整為 2 週;若跑一週 sprint,1 週就夠嚴格。**估算「改回來成本」的簡單方法**:問「如果現在要把這個選擇換成另一個,需要幾位工程師花幾天?把人天數乘以日薪」— 不需要精確,數量級夠了。

```mermaid
flowchart TD
    Start([有人說『這個決定該不該寫 ADR?』]) --> Q0{Q0 合規逃生門<br/>涉及合規 / 安全 / 隱私 / 錢?}
    Q0 -->|是| Write[✅ 寫 ADR<br/>Nygard 或 MADR<br/>⚑ 合規必寫,無條件]
    Q0 -->|否| Q1{Q1 改回來成本<br/>> 1 週工程時間?}
    Q1 -->|是| Q2{Q2 影響至少一個<br/>外部介面?}
    Q1 -->|否| Q4{Q4 半年後接手者<br/>會問為什麼?}
    Q2 -->|是| Write
    Q2 -->|否| Q4
    ...
    class Skip cold
    class Q0,Q1,Q2,Q4 cold
```

**這張圖的兩個出口各有意義**。Q0 是合規逃生門:凡是涉及 PCI DSS、SOC 2、GDPR、PII、資安邊界的決策,不管可逆性如何,直接寫 ADR ⸺ 稽核員看的是「有沒有決策紀錄」,不是「決策改不改得回來」。Q4 是右下角的 `Skip` 出口:大部分日常決定走 Skip 是健康的。真正的紀律是**留住那些「三年後還會被問」的決定**,其他都讓位給 PR description 或 CONTRIBUTING.md。
