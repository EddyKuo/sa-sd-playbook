...
> **下游章節**:[Ch 35 平台工程 / Internal Developer Platform](./ch-35-platform-engineering-idp.md)
> **延伸補章**:無
...
受邀進場的第一週,我拉下 HelixOps 那批 core module commit,跑了一次 ArchUnit 反向掃描。報告長這樣:
...
換句話說,Fitness Function 不是「另外一種文件」,**它是把 ADR 從希望升級為合約的那條 CI line**。Neal Ford 與 Rebecca Parsons 在 *Building Evolutionary Architectures* 2nd ed. (2023) [^CIT-310] 裡把這件事寫得很直接:「架構特徵(architectural characteristics)若沒有對應的 fitness function,就無法在演進中被守護」。HelixOps(§ 34.1)就是這個道理的反面教材 — ADR-0007 從 2023-09 通過到 2026-01 才第一次被量測,中間 28 個月沒有任何機器在驗「合約是否被遵守」。
...
換言之,Fitness Function 的核心價值是**偵測自動化 + 偵測即時性**。

這兩個屬性為什麼重要?先說**偵測自動化**:違反不再依賴「有人記得安排季度架構審查」,CI 在每個 commit 自動執行,不需要人工跑掃描工具。再說**偵測即時性**:違反發生在 commit 層,被擋也在 commit 層,不需要等到下一次季度 review。兩者合在一起,讓腐爛的**發現延遲從「月」降到「分鐘」**,清理成本從「重構衝刺」降到「修這一個 PR」。Fitness Function 不是守護架構的唯一方法 ⸺ 季度架構審查也能做到 ⸺ 但它是讓這個可觀測性**自動化、持續化**的最高效機制。
...
這只是 fitness function 的**第一層 ⸺ Code 層**。實際上,fitness function 在架構中有五個不同的層次,每層守護不同的演進方向。每一層守護不同的架構特徵:

| 層級 | 守護什麼 | 沒守護的後果 |
|---|---|---|
| **Code** | 依賴方向、模組邊界、命名規範 | 程式碼腐爛(HelixOps § 34.1) |
...
這張圖最關鍵的是右下那條虛線:**Observe 層燒爆,反向回去 block PR**。SLO error budget 不只是 SRE 的事,它是 fitness function 的最後一層 ⸺ 如果近 7 天 error budget 燒掉超過 50%,新功能 PR 就應該被擋,這是 Ch 30 的延伸,也是 § 34.4 第四條反模式要處理的。
...
選工具不是這節的重點,**選哪一層該用哪個工具,才是**。下面這張表把 § 34.3.1 五層的常用選項並排,版本標到 2026 Q1 還在維護的版本:
...
這段規則檢查三件事:(1)`domain` package 裡的類不能依賴 `infrastructure`、`web` 或任何第三方 JDBC/HTTP 框架;(2)`application` 層只能依賴 `application`、`domain`、`port` 三個包,杜絕它直接呼叫外部 SDK;(3)Spring Modulith 的模組依賴宣告必須與實際 import 一致,若有未宣告的跨模組依賴會在測試期報錯。
...
`.spectral.yaml` 定義了兩條自訂規則:(1)所有 `POST/PUT/PATCH` 路徑必須在 `parameters` 裡宣告 `Idempotency-Key` header,確保寫操作符合 ADR-0014 的冪等性要求;(2)不允許直接刪除既有 operation,必須先走棄用流程。`buf breaking` 那一行則是在 CI 中把當前 OpenAPI 與 main branch 對比,若有破壞性欄位刪除或型別變更就報錯中止。
...
這段 Rego 政策由 Conftest 在 CD 階段執行,對象是 Kubernetes `Deployment` manifest。第一個 `deny` 規則:從 Trivy 的掃描結果(`data.trivy`)裡找出所有 `CRITICAL` 等級的 CVE,只要存在就拒絕部署並輸出含 CVE ID 的錯誤訊息。第二個 `deny` 規則:若 Deployment 缺少 `helixops.io/tenant-isolation` annotation,同樣拒絕部署,確保每個租戶的工作負載都被正確標記隔離。對不熟悉 Rego 語法的工程師來說,把 `deny[msg] { ... }` 理解成「當大括號裡的條件全為真,就輸出 msg 並阻擋本次部署」即可。
...
下面這四個常見地雷,在 saas、fintech、ecommerce 都看得到。它們的共同點是「**形式上採用了 fitness function,但實質上沒有產生選擇性壓力**」⸺ 也就是 § 34.2.2 講的那個「沒有守護就只是腐爛」的退化路徑。每一個都附修正方向,下次遇到可以這樣處理。
...
> ✅ **修正方向**:用 § 34.3.1 的五層全景圖盤點,**每一層至少要有一條 fitness function 在 CI 跑**,即使是最低劑量也要有。最小組合可以這樣定:Code 層 1 條 ArchUnit/Packwerk、API 層 1 條 Spectral lint + 1 條 Buf breaking、Data 層 1 條 Schema Registry 相容性、Deploy 層 1 條 Trivy CVE-Critical、Observe 層 1 條 multi-burn-rate alert。五條合計工程成本約 3 人週,比起腐爛的清理成本(HelixOps 那 18 個月)便宜很多。
...
> ✅ **修正方向**:Pact 是雙邊機制,必須有 consumer 端產出 pact 檔案丟到 Pact Broker,provider 端才有東西可以 verify。判準:每個被 partner 或內部服務呼叫的 API,**必須有至少一份 consumer pact 在 Broker**;沒有的 API 視為「未被消費」(那它存在的意義也要重新檢查)。這裡涉及三個不同職責的工具,各自守護 § 34.3.2 三維分類裡不同的「Dynamic」契約面向:Spectral 守 OpenAPI 自身合規(格式與規範),Pact 守「對消費者不造成破壞」(雙邊消費者驅動契約),Buf breaking 守「跨版本相容」(schema 演進約束)— 三者各管一件事,缺一不可。
...
到此為止,fitness function 的五層框架、三維分類、工具選擇都已說清。最後一步是把這些規則與對應的 ADR 綁定,用一張 Card 跟 CI 共同執行。Card 的作用是:把「這條規則在守什麼、誰負責、違反了怎麼辦、何時退場」這四個問題的答案固化成一份可查、可審、可更新的合約。沒有 Card,規則會在三個月後變成沒人記得「為什麼當初加這條」的黑盒。
...
- [ ] 用 § 34.3.4 的決策樹,為一份新 ADR 判斷該寫哪一層 fitness function;寫不出對應規則的 ADR,要重新檢查它是否屬於風格指南而非架構約束
...
- **回顧**:[Ch 21 Modular Monolith § 21.4 反模式 3](../part-04-architecture/ch-21-modular-monolith.md) ⸺ 沒有 fitness function 的模組邊界會在 18 個月內腐爛
...
- **下一章**:[Ch 35 平台工程](./ch-35-platform-engineering-idp.md) ⸺ 把 fitness function 包成 paved road

## 引用

[^CIT-310]: Neal Ford, Rebecca Parsons, Patrick Kua, *Building Evolutionary Architectures*, 2nd Edition (O'Reilly, 2023)。第一版 2017,二版加入 GenAI 章節與更新案例。書中三維分類(Atomic/Holistic、Triggered/Continuous、Static/Dynamic)為本章 § 34.3.2 的依據。
