...
為什麼?我在那場會議後,親自走了一遍 IDP 的 onboarding path,並且拿碼表計時:

> 第 1 步:進 Backstage 首頁,catalog 頁面 loading **3 分 10 秒**(連到內部 plugin API 的 cold start)。第 2 步:建立 Backstage 帳號,需要走 LDAP SSO;SSO 跳轉後要求另外開一個 AWS SSO 帳號。第 3 步:申請 namespace,連到 access request portal ⸺ 那是另一個服務、另一次登入。第 4 步:填完表單後「等待 PEG approve」⸺ 平均 SLA **8 小時**。第 5 步:拿到 namespace 後,回到 Backstage 寫 ArgoCD application YAML。第 6 步:跑 `hp-cli deploy`,CLI 版本跟 portal 版本不一致報錯,需要查 Confluence。我走完這段,計時 **4 小時 12 分**。
>
> 對照組:我問旁邊一個 checkout team 的 mid-level,他打開自己的終端,複製貼上同事給的 Terraform module,改了 5 個變數,`terraform apply` ⸺ **1 小時 5 分**,結束。

這就是那 41/47 的機制。工程師不是懶,也不是壞:他們計算了路徑成本,選了快的那條。**IDP 的四倍慢,不是感覺,是可以量到的摩擦累積:7 個入口、7 次登入、1 個 8 小時 SLA**。診斷如果只停在「採用率低」,就是跳過了這個機制。
...
把 HarborPick 那場失敗拆成根因,實際上有**三條不同的鏈**,解法也不一樣:

| 根因 | 症狀 | 解法 | HarborPick 命中? |
|---|---|---|---|
| **採用度量缺失**(missing PM) | 沒有 NPS、retention、paved road 命中率;做了東西不知道有沒有人用 | 補 PM 角色,建 adoption 儀表板 | ✅ 是(17 個工具全無 DAU 追蹤) |
| **Paved road UX 摩擦**(path slower than dirt road) | 工程師量過,走 IDP 比手寫慢 4 倍;自然選擇土路 | 重設計 paved road:砍步驟、並行化 approve、消除多帳號 | ✅ 是(4h12m vs 1h05m,細節見 § 32.1) |
| **認知負荷反向增加**(platform adds load instead of removing it) | Stream team 需要學 7 個獨立工具的 mental model;越用越累 | 工具整合:選 1 個 portal 當入口,其他工具變 plugin | ✅ 是(7 個 dashboard、7 個帳號、3 套 CLI) |

這三條鏈在 HarborPick 同時存在,但**治療順序不同**:先修 UX 摩擦(不然 PM 加入後也是在管一個沒人想走的路),再補 PM(讓 adoption 指標有人負責),再整合工具(讓認知負荷有結構性下降)。把它們攪在一起一起修,通常三件都修不好。下面三節各自展開。
...
Gregor Hohpe 在 *The Software Architect Elevator* [^CIT-299] 寫過一句話:「Platform 真正的考驗是你能不能 charge 內部 team 真實的 cost ⸺ 即使他們有選擇權,他們仍然選你。」這就是 Platform-as-a-Product 的核心。也就是說,Platform 要能像真正的商業產品一樣經受市場檢驗——這意味著要有明確的客戶(Persona)、清晰的策略與 PM 負責、能量測效果(採用度量)、以及敢於淘汰失敗的功能(退場機制)。把它拆成五件事:
...
在 2025–2026 年的組織演進中,有越來越多公司在補齊了 Platform-as-a-Product 的基本功之後,嘗試用 AI Agent 來進一步改善 IDP 的開發者體驗。但這引發一個常見誤區:把 Agent 當成 IDP 設計缺陷的修補器,而不是完善 platform 之後的加分項。
...
State of DevOps Report 系列 [^CIT-292] 從 2014 跑到 2024,核心結論一直是:**DORA 高 ≠ 工程師快樂**,兩件事必須分別量。HarborPick 的 PEG 在 2025 Q3 OKR 達標(deploy frequency +30%),但 stream team 內部問卷的 satisfaction 同期跌 14 分 ⸺ 那 30% 的 deploy 多數是 PEG 自己 deploy 自己的 17 個工具。這 30% 增長的背後是 PEG 在 2025 Q3 大量推出新功能(7 個新工具),stream team 工程師在新工具與舊工具之間不斷切換(7 個 dashboard、7 個帳號、3 套 CLI),實際工作反而變複雜了——速度在表上,痛苦在心裡。
...
這張圖的關鍵不是分支,**是 Q4 跟 Q5 之前還有一個隱藏問題**。許多 platform team 進到 Q4(有 PM?)就停了 ⸺ 補了 PM、建了 roadmap,但 paved road 的 UX 摩擦沒修,PM 加入之後管的仍然是一條沒人想走的路。

> **決策樹的問法 vs 實際修復優先順序**:決策樹的 Q4(有 PM?)在 Q5(有採用率度量?)之前,這是「成立 Platform team 的資格檢查」順序——先問人、再問工具。但實際上一旦發現問題、開始修復時,優先級是不同的:最緊急的是先修 paved road 本身的 UX 摩擦(Q3 檢查的那個問題),其次才是補 PM(Q4),最後才是建齊度量系統(Q5)。別把決策樹的問法當成修復計畫的排序。

正確的修復次序是:

1. **先確認 paved road 本身是否比土路快**(≥ 3×)⸺ 如果不是,最緊急的工作是 path redesign,而不是補 PM。
2. **再補 PM**⸺ 讓人對 adoption 指標負責,並且把 path 的改進有系統地量測。
3. **最後整合工具**⸺ 單一入口、統一 SSO、降低 mental model 數量。

Q4「有人能擔任 Platform PM 角色?」是必要條件,但不是充分條件。沒有 PM、沒有 paved road 採用率度量就成立 Platform team,結果幾乎一定是 HarborPick 那 17 個沒人用的工具。如果 PM 已到位但 paved road adoption 仍然 < 30%,先回頭量 UX 摩擦(§ 32.1 那個碼表走法),再決定下一步。
