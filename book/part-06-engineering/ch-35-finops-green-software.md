...
## 35.1 冷觀察 ⸺ 一張 23 萬美元的雲帳單,後面跟著一封寫不出來的碳排報告
...
## 35.2 真問題 ⸺ 帳單與碳排底層是同一個 Workload Profile
...
這個觀察在 2024 年就有人提,但到了 2026 年才變成不能迴避的問題,原因有三——LLM 成本爆炸把 token 帳單推上財務報表第一頁、法規把碳排揭露從「自願」變成「合約義務」、SCI 從業界方法論升格為可被引用的 ISO 標準。換句話說,「FinOps 與 Green 一起治理」從可選項變成法務、財務、客戶都可能來敲門的必答題。
...
## 35.3 決策框架 ⸺ FinOps + Green 在同一張地圖上怎麼走

### 35.3.1 FinOps Foundation 三階段循環
...
注意上表中每款工具的「限制」欄——從 Cost Explorer 的「tag 沒打就看不到」到 Kubecost 的「只看 K8s」,再到 AWS CCFT 的「region 粒度、無法切 workload」,所有限制的共同根源都是**切片能力依賴 tag**。沒有強制 tag 紀律,買十款工具也只能看到帳號級粗粒度;有了 tag 紀律,即使只用 Cost Explorer 一款也能切到 service 與環境級別。
...
### 35.3.3 SCI Specification 的量測公式(可抄)
...
### 35.3.6 Operate 階段:把它寫進 CI、寫進 ADR
...
> ✅ **修正方向**:Spot 限定在「可中斷 + checkpointable」的 workload(batch、CI、非即時 inference);即時 API 用 Savings Plan / Reserved 為主、spot 為副(< 30%);上 spot 之前先寫 chaos drill 驗證中斷處理(對照 Ch 30 §30.3 Chaos)。**省下的成本不該以 SLO 為代價**。
...
> ✅ **修正方向**:SCI 量測能力要自己有 ⸺ 至少 §35.3.3 那段腳本級別的能力,跑進 CI、每日輸出。顧問可以幫忙審計、幫忙對齊 GHG Protocol / ISSB,但**核心量測管道要在自己手上**,因為這條管道接下來會跟 cost optimization 共用同一個 workload profile,外包出去等於把最佳化能力一起外包。

---

## 35.5 交付清單 ⸺ 一頁式 Workload Cost & Carbon Card
...
### 35.5.1 範例：AmpereLoom 那條 71K 美元的告警摘要 workload

AmpereLoom(`CASE-ENR-006`)Q3 帳單裡最戲劇性的單項,是「案場異常告警自然語言摘要」這條 LLM workload 從 6,000 美元一個月變成 71,000 美元。下面是 FinOps + SRE + ESG 三個 lead 第一次坐在同一張桌前共同填的那張卡:

````markdown
# Workload Cost & Carbon Card — alert-summary-llm

> 版本:v0.1 | 撰寫日期:2026-04-04 | Owner:Wei(平台)+ Lin(ESG)
> 對應服務:`alert-summary-svc` / namespace `ops` | prod | aws us-east-1

## 1. Workload Profile
<!-- 為什麼這欄:R 寫不出來,後面的 SCI 與 $/R 全部沒地方落 ⸺ 寫不出 R 的 LLM workload 通常就是「整包丟 prompt」的訊號。 -->
- 功能單元 R:每 1,000 次告警摘要呼叫
- 流量:約 4,200 R/day(平均一天告警 4.2 萬筆,每 10 筆觸發一次摘要)
- 即時:否(SLO 容忍 5 分鐘延遲)
- 可中斷:是(checkpointable,可走 Batch API)
- LLM workload:是(目前全用 Claude Sonnet 4.5)

## 2. 月成本基線
| 項目 | $/月 | 來源 |
|---|---|---|
| Compute(EKS)| 1,800 | Kubecost |
| LLM API token | **71,000** | Anthropic usage(input 92%、output 8%)|
| 其他 | 600 | CUR |
| **合計** | **73,400** | |

## 3. 月碳排基線
- LLM 推論估算:約 1,240 kgCO2e/月(基於 model card + token 量)
- SCI:**0.30 gCO2e / R**(每千次摘要)

## 4. 與 SLO 的互動
- 對應 SLO:`alert-summary-availability ≥ 99.5%` / `latency P95 < 5min`
- 任何最佳化不得讓 P95 退化 > 10%

## 5. Top 3 最佳化動作(2026 Q2)
<!-- 為什麼這欄:cost 與 carbon 寫在同一格,FinOps 與 ESG 第一次發現 80% 動作重疊。 -->
| # | 動作 | $/月 變化 | kgCO2e/月 變化 | SLO | Owner |
|---|---|---|---|---|---|
| 1 | Prompt Caching:30 天運轉日誌只 cache 一次,後續 incremental | -42,000 | -700 | 無影響 | Wei,2026-05 |
| 2 | Model Routing:90% 走 Haiku、10% 走 Sonnet | -16,000 | -260 | P95 +8%(可接受)| Wei,2026-06 |
| 3 | Batch API(非緊急部分占 60%) | -7,000 | -120 | 延遲 +3min(SLO 容忍) | Lin,2026-06 |

## 6. LLM 專屬指標
<!-- 為什麼這欄:沒這四格,LLM workload 就是黑洞,只有帳單會說話。 -->
- Model routing:Haiku 0% / Sonnet 100% / Opus 0%(目標 90/10/0)
- Cache hit ratio:0%(目標 ≥ 75%)
- Batch API 使用率:0%(目標 60%)
- 平均 input token / R:約 18,400 | output:約 320

## 7. 治理
- FinOps Lead:Mei | SRE Lead:Wei | ESG Lead:Lin | Service Owner:Wei
- 季度 review:2026-Q2 / Q3 / Q4
````

AmpereLoom 在 Q4 把這張卡上 §5 三條動作做完,LLM 月支出回到 12,000 美元、SCI 從 0.30 降到 0.11 ⸺ **同一份 workload profile 寫進同一張卡,FinOps 跟 ESG 才第一次在同一個句子裡**。
...
[^CIT-320]: FinOps Foundation, *FinOps Framework* (v3, 2024) — finops.org/framework/。三階段循環(Inform / Optimize / Operate)、Six Principles、Personas 為本章 §35.3.1 主要參照。
