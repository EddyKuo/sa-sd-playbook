...
兩年前的 PayLoop 之所以崩潰,不是因為他們沒做 SA/SD,而是因為他們**做的 SA/SD 沒留下「可被傳遞的理解」**。兩年後的 PayLoop 2.0 之所以能站住,不是因為他們做了「最佳實踐」,而是因為他們把這本書 38 主章 + 6 補章每一章交付的那張 Card,都老老實實寫成了現場的 artifact。
...
把全書 38 主章 + 6 補章拉開來看,內容像是覆蓋了所有 SA/SD 的領域:從 UML 到 DDD、從 SOLID 到 K8s、從 Threat Model 到 LLM Eval。但若把每一章交付的那張 Card 攤開、把每一章開頭的「真問題」段落攤開,會發現全書反覆在講的,其實只有六件事。這六件事是同一張地圖的六個「定錨點」⸺ 任何一個 SA/SD 問題,最後都會落回其中一個。
...
| **⑤ AI 時代的 SA/SD = 脈絡工程** | LLM 沒失憶,是脈絡沒持久化;CLAUDE.md / Skill / RAG / Memory 都在補這件事 | Ch 33 / 34 / 35 / 36 / 37 / 38 / 補章 B / D | CDE Setup、Agent System Card、AI Quality Card |
...
45 份 artifact,每份一頁,加總約 60–80 頁 ⸺ **這就是 PayLoop 2.0 在 MAS 抽查當天投影到牆上的那棵 `docs/` 樹**。

把 45 份 artifact 跟對應章節畫成一張結構圖,可以更清楚看到「同一張地圖的不同 zoom level」這句話的形狀:
...
這張圖的關鍵不是六個盒子的順序,是**那三條虛線**:Charter 一直在約束底層架構與 AI 層、BC 邊界一直延伸到 AI Agent 的職責劃分。少了這三條虛線,45 份 artifact 就會退化成 45 份各自獨立的文件 ⸺ 這就是 §39.4 反模式 1 要避免的事。
...
最常見的讀者反應是「我把每章的 Card 都寫一遍就行了」,然後在月底交出 45 份各 1 頁、彼此互不相干的 Card。Charter 寫了一份、ADR 寫了 84 份,但 ADR 沒回頭引 Charter,Charter 沒有約束 ADR。結果是有 artifact、沒有理解。
...
讀完這 39 章 + 6 補章,你可以帶走的不是 39 份心得,而是**一整個整合 Pack**。它的形狀就是 PayLoop 2.0 那棵 `docs/` 樹,規模 45 份 artifact,加總 60–80 頁。
...
這張圖的關鍵不是「八個 Part」,是**那三條虛線**:Charter 一直在約束最底層的 capstone、BC 是中段的脊椎、CDE 是 2026 才補的最後一塊。讀者把這三條虛線記住,本書 38 章 + 6 補章就有了座標。
...
5. **AI 時代的 SA/SD = 脈絡工程**。LLM 沒失憶,是脈絡沒持久化。CLAUDE.md / Skill / RAG / Memory / Multi-Agent / Eval-Drift-RedTeam 都在處理這件事。([Ch 33–38](../part-07-ai-era/ch-33-ai-native-architecture.md))
...
如果你是剛接手新專案的工程師,請從 Charter 開始,一頁。如果你是接到 brownfield 任務的架構師,請從補章 C 開始,Strangler Fig。如果你是把 AI 放進系統的 PM / RD,請從 Ch 38 + Ch 33 開始,三軸並行。其餘 38 章是地圖,等你需要某個 zoom level 的時候再回來查。
