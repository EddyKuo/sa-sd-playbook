...
> **前置閱讀**:全書 38 主章 + 補章 A–I(本章是收束章,不重講細節,只接劇本)
> **下游章節**:附錄 1 全書術語 / 附錄 2 全書案例 / 附錄 3 全書引用 / 附錄 4 一頁式 artifact 速查 / 附錄 5 全書 ADR 範例庫
> **延伸補章**:[補章 A 雲原生選擇](../part-04-architecture/chA-edge-ot-it.md)、[補章 B Agentic QA](../part-07-ai-era/chB-agentic-qa.md)、[補章 C 遺留系統 AI 現代化](../part-07-ai-era/chC-legacy-ai.md)、[補章 D Multi-Agent Consensus](../part-07-ai-era/chD-multi-agent.md)、[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)、[補章 F CUX 對話式體驗](../part-03-design/chF-cux.md)、[補章 G 工程直覺保護手冊](../part-09-human-engineer/chG-engineering-intuition.md)、[補章 H Agent 設定語言](../part-07-ai-era/chH-agent-spec.md)、[補章 I Agent Harness 工程](../part-07-ai-era/chI-harness-engineering.md)

---

## 39.1 冷觀察 ⸺ 兩年後,同一個 CTO,被問同一句話
...
│   ├── c4/                    # C4 四層 .dsl(Ch 19)
│   ├── adr/                   # 84 份 ADR(Ch 30)
│   └── fitness/               # 23 條 Fitness Function(Ch 31)
├── domain/
│   ├── bounded-contexts.md    # DDD BC 圖(Ch 17)
│   └── event-storming.md      # ES 結論(Ch 18)
├── api/
...
├── agents/
│   ├── claude.md              # 給 Claude Code 看的 root(Ch 34)
│   ├── system-card.md         # Agent System Card(Ch 35)
│   ├── multi-agent-card.md    # Multi-Agent System Card(Ch 36)
│   ├── quality-card.md        # AI Quality Card(Ch 38)
│   ├── agent-spec.md          # Agent Spec Card(補章 H)
│   └── harness-config.md      # Harness Engineering Card(補章 I)
└── compliance/
    └── design-pack.md         # Compliance by Design Pack(補章 E)
```
...
timeline
    ...
    section 月 0(事故覆盤)
      ...
      畫 Bounded Context 圖 : Ch 17
    section 月 1–3(補課)
      Cadence Charter 把四個 Tempo 對上 : Ch 2
      TELOP 評估 + 找出隱性否決者 : Ch 3
      Class/Sequence/State 三張圖補上 : Ch 5
    section 月 4–9(架構重構)
      DDD 重劃 BC + Event Storming : Ch 17 / 18
      C4 四層 + Module Boundary Card : Ch 19 / 20
      Coarse-grained API + Storage 重選 : Ch 14 / 15
    section 月 10–14(品質與工程)
      SLO + Threat Model + Data Architecture : Ch 25–28
      Platform / ADR / Fitness Function / FinOps : Ch 29–32
    section 月 15–22(AI 起手)
      AI-Native L1–L7 + CDE + RAG : Ch 33–35
      Multi-Agent + Coding Agent + Eval : Ch 36–38
      Agentic QA + Compliance Pack : 補章 B / E
    section 月 23(MAS 抽查)
...
|---|---|---|---|
| **① SA/SD = 製造可被傳遞的理解** | 不是儀式、不是文件,而是讓三年後的人(和 AI)能接手 | Ch 1 / 4 / 10 / 30 / 34 / 補章 C | System Charter、Requirements Decision Log、ADR、CLAUDE.md |
| **② System Charter 是入口** | 比 PRD 更早、比 ADR 更總,是專案的「定錨」 | Ch 1 / 2 / 3 / 11 | Charter、Cadence Charter、TELOP |
| **③ Cadence 讓四個 Tempo 對上** | Business / Product / Engineering / Compliance 不是同一個鼓點 | Ch 2 / 27 / 32 / 補章 E | Cadence Charter、SLO Catalog、FinOps Card |
| **④ Bounded Context 是抗變動的核心** | 從 DDD 到 Modular Monolith 到 Cell-based,都在處理同一件事:邊界 | Ch 17 / 18 / 19 / 20 / 21 / 24 | BC Card、ES Outcome、C4、Module Boundary Card |
| **⑤ AI 時代的 SA/SD = 脈絡工程** | LLM 沒失憶,是脈絡沒持久化;CLAUDE.md / Skill / RAG / Memory / Harness 都在補這件事 | Ch 33 / 34 / 35 / 36 / 37 / 38 / 補章 B / D / G / H / I | CDE Setup、Agent System Card、AI Quality Card、Agent Spec Card、Harness Engineering Card |
...
### 39.3.2 月 1–3:把上一版的「會跑的東西」拆解(對應 Ch 5 / 6 / 7 / 8 / 9 / 10)
...
### 39.3.3 月 4–9:架構級重構(對應 Ch 11–24,本書最長的一段)
...
| Audit Log | 寫入即不可變 | append-only、合規 7 年 | S3 + Iceberg(via 補章 E) |

關鍵決策是「**不引入 MongoDB / Cassandra**」⸺ Workload Profile 表明所有需求 PG 17 + pgvector + Citus 都能處理,引入 NoSQL 只會增加運維成本。這條 ADR 編號 `ADR-0023`,後面 §39.5 會展示完整內容。

#### UI/UX([Ch 16](../part-03-design/ch-16-uiux-system-view.md) + 補章 F)

banker 工作站的錯誤訊息在 PayLoop 1.0 是「Transaction failed.」這種等於沒講的話。重構引入 [Ch 16](../part-03-design/ch-16-uiux-system-view.md) 的 **DACR 錯誤訊息框架**(Diagnose / Action / Context / Recovery),把每個錯誤改寫成四欄。同時為將要上線的 AI banker 助手預備 [補章 F](../part-03-design/chF-cux.md) 的 **CUX(Conversational UX)Card**,定義 AI 何時主動發言、何時等待、何時 handoff 給真人。

#### DDD / Event Storming / C4([Ch 17 / 18 / 19](../part-04-architecture/ch-17-ddd-strategic-tactical.md))
...
[Ch 19](../part-04-architecture/ch-19-c4-model-visualization.md) 的 C4 四層在月 7 用 Structurizr DSL 寫出來,放 `docs/architecture/c4/*.dsl`。CI 加入一條 Fitness Function:每次 PR 自動 parse C4 .dsl 與真實 Maven 依賴,如果發現「C4 圖上沒寫的依賴在 code 裡」就阻擋 merge ⸺ 這就是 [Ch 1.4 反模式 3](../part-01-foundations/ch-01-why-sa-sd.md) 講的 C4 圖過期問題的根治法。

#### Modular Monolith / 微服務 / EDA / Mesh / Cell([Ch 20 / 21 / 22 / 23 / 24](../part-04-architecture/ch-20-modular-monolith.md))
...
這條決策對應 Module Boundary Card 與 Microservice Tax Card([Ch 20 / 21](../part-04-architecture/ch-20-modular-monolith.md))。Microservice Tax Card 列出「如果現在拆成 6 個 service 要付的稅」:6 個 K8s deployment、6 條 CI pipeline、6 份 observability stack、跨 service transaction 改成 saga 的 4 週改動成本 ⸺ 加總後團隊的反應是「不值得」。

EDA / CQRS / ES([Ch 22](../part-04-architecture/ch-22-event-driven-cqrs-es.md))**三件不套餐**:只引入 EDA(Kafka 3.7 處理 BC 之間的 domain event),CQRS 與 ES 暫時不做。理由是 ES 對 audit / replay 雖好,但 PayLoop 2.0 的合規要求用 append-only audit log 已經滿足,不需要 ES 的全套機械成本。這個「只挑一件」的決策,對應 EDA Layer Card。

K8s / Mesh / Cell([Ch 23 / 24](../part-04-architecture/ch-23-cloud-native-kubernetes.md) + [補章 A](../part-04-architecture/chA-edge-ot-it.md))在月 8 引入。Cell-based 的關鍵切割是「按地理 + 合規邊界」:新加坡 cell、香港 cell、菲律賓 cell 各自獨立,符合 MAS 的資料境內要求。Mesh 用 Istio 1.22 處理 cell 之間的流量(主要是 Reporting 跨 cell 聚合)。Edge 處理 banker 工作站本地端的合規預檢(避免每次按鈕都飛到 cloud)。

對應交付:Cloud-Native Choice Card、Traffic Governance Card、OT/IT Edge Design Pack(補章 A)。

### 39.3.4 月 10–14:四層品質與工程實踐(對應 Ch 25–32 + 補章 E)

#### Security / Observability / SRE / Data([Ch 25 / 26 / 27 / 28](../part-05-quality/ch-25-security-by-design.md))
...
合規這一條走 [補章 E Compliance by Design](../part-05-quality/chE-compliance.md) 的 **Compliance Design Pack**:把 MAS / FATF / EU AI Act Annex III 的條文逐條對應到系統元件,輸出 Compliance Trace Matrix。月 12 MAS 來做小範圍 pre-audit,這份 matrix 直接拿去用,過。

#### Platform / ADR / Fitness / FinOps([Ch 29 / 30 / 31 / 32](../part-06-engineering/ch-29-platform-engineering-idp.md))
...
### 39.3.5 月 15–22:AI 起手(對應 Ch 33–38 + 補章 B / C / D)

到這個階段,前面 14 個月的 SA/SD 基礎做穩了,PayLoop 才開始把 AI 放進系統。**順序很重要** ⸺ 把 AI 放在沒做好 SA/SD 的系統上,等於把第二個 PayLoop 1.0 蓋在第一個之上。

#### AI-Native L1–L7([Ch 33](../part-07-ai-era/ch-33-ai-native-architecture.md))
...
#### CDE 脈絡工程([Ch 34](../part-07-ai-era/ch-34-context-driven-engineering.md))

CLAUDE.md root 寫在 monorepo 根目錄,SKILL.md 散落在每個 BC 之下。Skill 的設計原則是「每個 Skill 對應一條業務能力,且必須註冊到至少一份 ADR」⸺ 這是 [Ch 30 / 34](../part-06-engineering/ch-30-adr-architecture-knowledge.md) 強調的「Skill 與 ADR 連動」,避免 Skill 漂移。

對應交付:CDE Setup Card + SKILL.md(每個 BC 一份)。

#### RAG / Memory / Tool([Ch 35](../part-07-ai-era/ch-35-rag-memory-tool.md))
...
#### Multi-Agent / Coding Agent / Eval([Ch 36 / 37 / 38](../part-07-ai-era/ch-36-multi-agent.md) + [補章 B](../part-07-ai-era/chB-agentic-qa.md))

風控做成 Multi-Agent(LangGraph 0.2):Screener Agent 負責規則匹配、Investigator Agent 負責拉相關歷史交易、Reporter Agent 負責出 STR 草稿。三個 Agent 透過 [補章 D Multi-Agent Consensus](../part-07-ai-era/chD-multi-agent.md) 的 consensus pattern 達成共識,任一 Agent 不同意時走 HITL。對應 Multi-Agent System Card。

Coding Agent([Ch 37](../part-07-ai-era/ch-37-coding-agent.md))在 22 人團隊中作為 pair programmer,但寫死兩條規則:(1)所有 commit 必須由人類 approve;(2)涉及 ADR 變動的 PR 必須先寫 / 改 ADR 再改 code。對應 Agent-Friendly Codebase Card。

[Ch 38](../part-07-ai-era/ch-38-ai-eval-drift-redteam.md) 的 Eval / Drift / Red Team **三軸並行**全做。Eval set 480 題、Drift PSI 接 PagerDuty、Red Team CI 攻擊集 240 題。對應 AI Quality Card,並走 [補章 B Agentic QA](../part-07-ai-era/chB-agentic-qa.md) 的雙 Agent 對抗評估補強 Judge 校準。

### 39.3.6 全書 artifact ↔ 章節對應索引(可帶走)
...
| 18 | Bounded Context Card | Ch 17 | `docs/domain/bc-*.md` |
| 19 | Event Storming Outcome Card | Ch 18 | `docs/domain/event-storming.md` |
| 20 | C4 Diagram Card | Ch 19 | `docs/architecture/c4/*.dsl` |
| 21 | Module Boundary Card | Ch 20 | `docs/architecture/modules.md` |
| 22 | Microservice Tax Card | Ch 21 | `docs/architecture/microservice-tax.md` |
| 23 | EDA Layer Card | Ch 22 | `docs/architecture/eda.md` |
| 24 | Cloud-Native Choice Card | Ch 23 | `docs/cloud/choice.md` |
| 25 | Traffic Governance Card | Ch 24 | `docs/cloud/traffic.md` |
| 26 | Threat Model Card | Ch 25 | `docs/security/threat-model.md` |
| 27 | Observability Plane Card | Ch 26 | `docs/ops/observability.md` |
| 28 | SLO Catalog + EB Card | Ch 27 | `docs/ops/slo.md` |
| 29 | Data Architecture Decision Card | Ch 28 | `docs/data/architecture.md` |
| 30 | Platform Product Card | Ch 29 | `docs/platform/product.md` |
| 31 | ADR Template + Index Card | Ch 30 | `docs/architecture/adr/INDEX.md` |
| 32 | Fitness Function Card | Ch 31 | `docs/architecture/fitness/*.yaml` |
| 33 | Workload Cost & Carbon Card | Ch 32 | `docs/finops/cost-carbon.md` |
| 34 | AI-Native System Vision Card | Ch 33 | `docs/agents/vision.md` |
| 35 | CDE Setup Card + SKILL.md | Ch 34 | `CLAUDE.md` + `**/SKILL.md` |
| 36 | Agent System Card | Ch 35 | `docs/agents/system-card.md` |
| 37 | Multi-Agent System Card | Ch 36 | `docs/agents/multi-agent-card.md` |
| 38 | Agent-Friendly Codebase Card | Ch 37 | `docs/agents/codebase-card.md` |
| 39 | AI Quality Card | Ch 38 | `docs/agents/quality-card.md` |
| 40 | OT/IT Edge Design Pack | 補章 A | `docs/edge/design-pack.md` |
| 41 | Agentic QA Pack | 補章 B | `docs/agents/qa-pack.md` |
| 42 | Brownfield Modernization Pack | 補章 C | `docs/legacy/modernization.md` |
| 43 | Multi-Agent Consensus Pack | 補章 D | `docs/agents/consensus-pack.md` |
| 44 | Compliance Design Pack | 補章 E | `docs/compliance/design-pack.md` |
| 45 | CUX Design Pack | 補章 F | `docs/ui/cux-pack.md` |
| 46 | Engineering Intuition Card | 補章 G | `docs/human/intuition-card.md` |
| 47 | Agent Spec Card | 補章 H | `docs/agents/agent-spec.md` |
| 48 | Harness Engineering Card | 補章 I | `docs/agents/harness-config.md` |
...
這張圖的關鍵不是六個盒子的順序,是**那三條虛線**:Charter 一直在約束底層架構與 AI 層、BC 邊界一直延伸到 AI Agent 的職責劃分。少了這三條虛線,48 份 artifact 就會退化成 48 份各自獨立的文件 ⸺ 這就是 §39.4 反模式 1 要避免的事。
...
Linked: System Charter v3.2 §2 (Constraints),
        ...
        Module Boundary Card (Ch 20),
        Workload Cost & Carbon Card (Ch 32),
        Compliance Design Pack §4.2 (補章 E)
...
## Consequences
+ 運維面:1 個 RDBMS 比 3 個 RDBMS 少 60% 運維成本(對應 Ch 32 FinOps)
+ 合規面:單一 source of truth 對 MAS audit trail 友善(對應補章 E)
+ 邊界面:Module Boundary Card 限定每個 BC 一個 schema,單庫多 schema
  即可滿足(對應 Ch 20)
- 規模面:若未來月流水 > USD 5 億且 OLTP 寫入 > 5,000 TPS,需重評估
  (觸發條件寫入 Fitness Function FF-0014,對應 Ch 31)
...
> ✅ **修正方向**:每份 Card 強制標註「上游(我從哪來)」與「下游(我影響誰)」兩欄,跟 git commit 一樣建立 DAG。本章 §39.3.7 那份 ADR-0023 就是示範:它在 Linked 區塊明確列出 6 章 + 1 補章的連動。把全書當「彼此引用的圖」而不是「依序讀完的書」⸺ 這也是 [Ch 1.2](../part-01-foundations/ch-01-why-sa-sd.md) 一開始強調的「製造可被傳遞的理解」最具體的形狀。

### 反模式 2:只做 Greenfield,不做 Brownfield(承接補章 C)

PayLoop 1.0 → 2.0 是 brownfield,不是 greenfield。團隊一開始有過一陣子的衝動,想「砍掉重寫」,因為 1.0 的 70% AI 寫的 code 看起來像個爛攤子。如果走 greenfield,根據 [補章 C](../part-07-ai-era/chC-legacy-ai.md) 的經驗,大概率會在月 6 出現「新版做不完、舊版沒人改 bug」的雙線崩潰。

> ✅ **修正方向**:走 [補章 C](../part-07-ai-era/chC-legacy-ai.md) 的 Brownfield Modernization Pack。先做 **Strangler Fig** ⸺ 新功能寫在新模組,舊功能逐步從舊系統剝離,中間 6–9 個月雙線並存。AI 幫忙的地方不是「寫新程式」,是「讀舊 code 寫 ADR、補測試、生 type stubs」。PayLoop 2.0 month 4–9 的架構重構,就是 Strangler Fig 在跑。

### 反模式 3:只做 Eval,不做 Drift / Red Team(承接補章 B)

[Ch 38](../part-07-ai-era/ch-38-ai-eval-drift-redteam.md) 的 AMLNavigator 教訓在 PayLoop 2.0 內部被當作經典案例反覆複述。但實際做的時候,還是有人想偷懶,理由是「我們先做 Eval,Drift / Red Team 等 Q4 再做」。這個拖延一旦發生,通常第三季就會吃到事故,因為輸入分布飄移與 prompt injection 攻擊不會等你做好準備才來。

> ✅ **修正方向**:Eval / Drift / Red Team **三軸並行,同月上線**。任何一軸晚兩個月做,等於三軸都白做。具體做法去 [補章 B Agentic QA](../part-07-ai-era/chB-agentic-qa.md):用雙 Agent 對抗評估自動產生 Drift 警報、用自動 Red Team Agent 持續產生新攻擊向量。PayLoop 2.0 ComplianceCopilot 上線當月,三軸都接好 PagerDuty。

### 反模式 4:Skill 沒接 ADR(承接 Ch 30 / 34)

[Ch 34](../part-07-ai-era/ch-34-context-driven-engineering.md) 的 SKILL.md 設計初期容易出現一種偏差:工程師寫 Skill 是為了「讓 Claude 寫 code 更順」,而不是「讓決策被持久化」。結果是 Skill 變成個人偏好的集合,半年後跟 ADR 互相打架,Claude 拿到衝突的脈絡時會幻覺,人類開 PR 時會踩雷。

> ✅ **修正方向**:每份 SKILL.md 必須在 frontmatter 列出 `linked_adrs: [ADR-0023, ADR-0041]`,且每次 Skill 改動都要在 commit message 引用 ADR 編號。CI 加一條 Fitness Function:Skill 引用的 ADR 不存在或已 deprecated 時阻擋 merge。這就是 [Ch 30](../part-06-engineering/ch-30-adr-architecture-knowledge.md) 與 [Ch 34](../part-07-ai-era/ch-34-context-driven-engineering.md) 的連動 ⸺ Skill 是 ADR 在 AI 協作層的可執行延伸,不是另起爐灶。

---

## 39.5 交付清單 ⸺ 全書 SA/SD 整合 Pack
...
### 39.5.2 AI 時代與 SA/SD 整合表(可貼上會議室白板)
...
| **DDD BC** | 學術概念 | Modular Monolith 切割線 | BC Card + Module Card(Ch 17 / 20) |
| **Code Review** | 人對人 | 人 + AI + Fitness Function | ADR + FF Card(Ch 30 / 31) |
| **Test** | unit / integration / e2e | + Eval / Drift / Red Team | AI Quality Card(Ch 38) |
| **Documentation** | Word / Confluence | git-tracked Markdown + CLAUDE.md | CDE Setup Card(Ch 34) |
...
S 模式適合「丟掉重寫不會痛」的內部工具;L 模式留給「弄錯會被監理罰款 / 上新聞」的場景;**預設值是 M**。這跟 [Ch 1.3.1](../part-01-foundations/ch-01-why-sa-sd.md) 的三維 Triage 表一致 ⸺ 全書從 Ch 1 到 Ch 39,預設值始終沒變。

---

## 39.6 Recap ⸺ 全書六大核心觀念與一封給讀者的短信
...
4. **Bounded Context 是抗變動的核心**。從 DDD 到 Modular Monolith 到 Cell-based,本質上都在處理同一件事:邊界。([Ch 17 / 20 / 24](../part-04-architecture/ch-17-ddd-strategic-tactical.md))
5. **AI 時代的 SA/SD = 脈絡工程**。LLM 沒失憶,是脈絡沒持久化。CLAUDE.md / Skill / RAG / Memory / Multi-Agent / Eval-Drift-RedTeam / Harness 都在處理這件事。([Ch 33–38](../part-07-ai-era/ch-33-ai-native-architecture.md) + [補章 H](../part-07-ai-era/chH-agent-spec.md) / [補章 I](../part-07-ai-era/chI-harness-engineering.md))
...
我看著 PayLoop 從 18 天上線、180 天崩潰,走到 22 個月後在 MAS 稽核員面前打開 `docs/` 那棵樹。中間沒有奇蹟,沒有銀彈,只有一張一張被認真寫出來的 Card。每一張 Card 一頁,寫不滿就是寫得不對 ⸺ 這句話從 Ch 1 講到 Ch 39,沒變過。

如果你是剛接手新專案的工程師,請從 Charter 開始,一頁。如果你是接到 brownfield 任務的架構師,請從補章 C 開始,Strangler Fig。如果你是把 AI 放進系統的 PM / RD,請從 Ch 38 + Ch 33 開始,三軸並行。如果你是要寫 Agent 設定規格的 SA/RD,請從補章 H 的 SKILL.md 語言開始。如果你是要讓 Agent 穩定跑在生產環境的平台工程師,請從補章 I 的 Harness Engineering 五元件開始。如果你擔心自己的工程直覺在 AI 輔助下逐漸退化,請從補章 G 的「不可外包的五種判斷」開始。其餘 38 章是地圖,等你需要某個 zoom level 的時候再回來查。
...
- **Bounded Context 三章組**:[Ch 17 DDD](../part-04-architecture/ch-17-ddd-strategic-tactical.md) / [Ch 18 Event Storming](../part-04-architecture/ch-18-event-storming-modeling.md) / [Ch 19 C4](../part-04-architecture/ch-19-c4-model-visualization.md)
- **Modular Monolith 路線**:[Ch 20](../part-04-architecture/ch-20-modular-monolith.md) / [Ch 21 微服務稅](../part-04-architecture/ch-21-microservices.md)
- **AI 系統工程紀律**:[Ch 33–38](../part-07-ai-era/ch-33-ai-native-architecture.md) + [補章 B](../part-07-ai-era/chB-agentic-qa.md) / [補章 D](../part-07-ai-era/chD-multi-agent.md)
- **Brownfield 路徑**:[補章 C 遺留系統 AI 現代化](../part-07-ai-era/chC-legacy-ai.md)
- **合規對齊**:[補章 E Compliance by Design](../part-05-quality/chE-compliance.md)
- **CUX 預備**:[補章 F 對話式體驗](../part-03-design/chF-cux.md)
- **工程直覺保護**:[補章 G 工程直覺保護手冊](../part-09-human-engineer/chG-engineering-intuition.md)
- **Agent 設定語言**:[補章 H Agent 設定語言](../part-07-ai-era/chH-agent-spec.md)
- **Agent Harness 工程**:[補章 I Agent Harness 工程](../part-07-ai-era/chI-harness-engineering.md)
...
[^CIT-392]: Eric Evans, *Domain-Driven Design: Tackling Complexity in the Heart of Software* (Addison-Wesley, 2003)。Bounded Context、Ubiquitous Language、Anti-Corruption Layer 的奠基。本書 Ch 17 / 20 / 39 三章共同的概念源頭。
[^CIT-393]: Sam Newman, *Monolith to Microservices* (O'Reilly, 2019) + GOTO 2020 / 2024 重訪。Strangler Fig 在 brownfield 的應用,對應 Ch 21 / 補章 C / 本章 §39.4。
[^CIT-394]: ThoughtWorks Technology Radar Vol. 30 (2024)。Modular Monolith → Adopt;Microservices Envy → Hold。對應 Ch 1 / 20 / 21 / 本章 §39.3.3。
[^CIT-395]: Michael Nygard, "Documenting Architecture Decisions" (2011)。ADR 概念奠基。本章 §39.3.7 的 ADR-0023 範例直接套用 Nygard template。
[^CIT-396]: Neal Ford, Rebecca Parsons, Patrick Kua, *Building Evolutionary Architectures* (O'Reilly, 2nd ed. 2023)。Fitness Function 的奠基。對應 Ch 31 / 本章 §39.3.4。
[^CIT-397]: CNCF 2026 Q1 Microservices Regression Report。對應 Ch 1 / 本章 §39.3.3 微服務反思的數據來源。
[^CIT-398]: Shopify Engineering Blog, "How we scale our modular monolith to 30TB/min" (2025)。對應 Ch 1 / 20 / 本章 §39.3.3。
[^CIT-399]: EU AI Act, Annex III "High-Risk AI Systems" (2024 通過,2026 起分階段生效)。對應 Ch 38 / 補章 E / 本章 §39.3.4 的高風險分類。
