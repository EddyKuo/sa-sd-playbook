...
PayLoop 在 Ch 1 是一個 18 天上線、180 天崩潰的新創,CTO 在事故覆盤會上問:「我們有什麼?除了一個會跑的東西之外,我們到底擁有什麼?」Ch 47 的 PayLoop 2.0,月流水從一百萬成長到 4,200 萬美元,當 MAS 再次問同一句話時,CTO 打開 IDE,把 `docs/` 的整棵樹投影到牆上。那棵樹上的每一個檔案,對應全書的一個 artifact。
...
flowchart LR
    subgraph VII[第 VII 篇 AI 時代]
        CH38[Ch 44<br/>AI Eval]
        CHB[Ch 45<br/>Agentic QA]
        CHD[Ch 40<br/>Multi-Agent 共識]
    end

    VII --> CH39

    subgraph CH39[Ch 47 Capstone]
        direction TB
...
Ch 47 按**時間軸**走了一遍 PayLoop 2.0 的生命週期:
...
- **前置**:全書 38 主章 + Ch 26–F(本章不重複理論,只接劇本)
