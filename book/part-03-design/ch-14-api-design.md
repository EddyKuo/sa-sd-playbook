...
### 14.5.1 範例:HarborGate 第三週補的第一張 contract card

HarborGate(`CASE-ECM-004`)第三週開始補卡,第一條補的是 12 萬賠款事故的元凶 ⸺ `orders.create`(REST + Webhook 雙協定)。下面這張就是後來放進 v3 開放平台公告的版本:

````markdown
# API Contract Card — orders.create.v3

> 版本:v0.3 | 撰寫日期:2026-02-14 | Owner:Platform Team
> 對應 schema:`api/openapi.yaml#/paths/~1orders` + `api/asyncapi.yaml#/channels/order.created.v3`
> 對應 ADR:`docs/adr/0042-order-status-canonical-enum.md`

## 1. Trust Boundary(信任邊界)
<!-- 為什麼這欄:對方是誰、要做什麼,直接決定了 schema 顆粒度與 SLA;
     寫不出這一欄代表這條 API 還沒準備好對外。 -->
- 對方是誰:已認證 partner(200+ 商戶系統,以 OAuth 2.0 client_credentials 認證)
- 對方拿這個介面做什麼:在自家後台下單後,推到 HarborGate 履約倉
- 我們對對方的承諾:JSON 200 + `Order` 物件、p95 < 400ms、棄用通知期 18 個月
- 我們不對對方承諾的事:`internal_routing_hint` 欄位變動、retry queue 內部行為

## 2. Granularity(顆粒度層級)
<!-- 為什麼這欄:粗顆粒對 partner 與 Agent 都好;
     細顆粒對外開放等於把內部步驟綁死,演化空間會被吃光。 -->
- [x] 粗顆粒(business intent:`createOrder`,一次完成預留庫存 + 建單 + 開立發票草稿)
- [ ] 中顆粒
- [ ] 細顆粒
- 對 AI Agent 開放:粗顆粒 + `_actions` HATEOAS(回傳 cancelOrder / addItem 連結)

## 3. Protocol(協定)
<!-- 為什麼這欄:每種協定處理不同的信任邊界,混用沒問題,
     但每種都要寫清楚對方是誰、SLA 是什麼。 -->
- [x] REST(Richardson 級別:**3**,含 HATEOAS `_actions`)
- [ ] GraphQL(僅內部 BFF 使用,不對 partner)
- [ ] gRPC
- [x] Webhook(事件:`order.created.v3`,5 次重試:1m → 5m → 25m → 2h → 12h)
- [x] AsyncAPI(broker:Kafka 3.7,replay 視窗:7 天)

## 4. Auth Model(認證模型)
- 認證方式:OAuth 2.0 client_credentials(partner 系統對系統)
- Scope:`orders:write`、`orders:read`
- Token 壽命:access 60 min,refresh 30 days
- 速率限制:200 req/min/client,burst 400

## 5. Retry & Idempotency(重試策略)
<!-- 為什麼這欄:HarborGate 12 萬賠款的根因就在這欄沒寫;
     沒 idempotency-key 等於授權 partner 重複出貨。 -->
- Idempotency-Key:**required**(partner 自產 UUID v7,server 保存 24h)
- 客戶端可重試的狀態:408、429、500、502、503、504
- 客戶端不可重試:400、401、403、404、**409 衝突**、422
- Webhook 重試曲線:1m → 5m → 25m → 2h → 12h(共 5 次,失敗進 DLQ 7 天)
- DLQ 行為:partner console 可見 + email 通知 owner + 7 天保留 + 提供 replay endpoint

## 6. Deprecation Cadence(棄用節奏)
<!-- 為什麼這欄:API 死掉的方式跟 API 出生同樣重要、且更難談;
     沒寫等於沒承諾,出事只能道歉。 -->
- Minor 版本:相容期 12 個月
- Major 版本:相容期 **18 個月**(高於業界 12 個月,因 partner 多為傳統零售)
- 棄用通知管道:`Sunset` header + monthly changelog email + console banner(180 天前開始)
- breaking change 定義(Spectral rule 鎖死):欄位移除 / 型別變更 / enum 縮窄 / 必填新增

## 7. Observability(觀測)
- Trace context:W3C Trace Context(`traceparent`)
- 必發 metrics:`api_orders_create_total{status,partner_id}`、`api_orders_create_latency_seconds`
- Audit log 欄位:`actor_client_id` / `scope` / `order_id` / `outcome` / `idempotency_key`
...
HarborGate 用這份卡的格式產出 47 條,佔他們宣稱 200 條 API 的 23%。剩下 153 條被合併、廢棄或證明根本沒人在用。**寫不出 contract card 的 API,通常是不需要存在的 API。**
