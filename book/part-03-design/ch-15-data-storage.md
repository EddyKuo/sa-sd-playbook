...
> **下游章節**:[Ch 23 事件驅動架構](../part-04-architecture/ch-23-event-driven-cqrs-es.md)、[Ch 31 資料治理與合規](../part-05-quality/ch-31-data-architecture.md)、[Ch 37 AI-Native 架構](../part-07-ai-era/ch-37-ai-native-architecture.md)
...
|---|---|---|---|
| **讀寫比** | reads/sec : writes/sec | 320 : 1,920(讀:寫 ≈ 1:6)⸺寫密集進料,但 **讀的代價由聚合主導** | 量吞吐量看寫入,量查詢成本看聚合複雜度;兩件事要分開量 |
| **查詢模式** | 點查 / 範圍掃描 / 聚合 / 全文 / 向量 / 圖 占比 | 點查 8%、範圍 12%、**聚合 78%**、向量 2% | 78% 聚合代表查詢成本集中在 pre-aggregation 能力,不是寫入吞吐量 |
| **規模** | row 數、storage、年增量 | 7 億 row、原始 ~1.4TB(壓縮後 ~480 GB)、年增 40% | 壓縮前後數字需分開標示;1.4TB 是規劃容量基準,480 GB 是 TSDB 壓縮後的計費基礎 |
| **一致性需求** | 強一致 / 讀己之寫 / 最終一致 | 最終一致(telemetry monitoring 場景,讀己之寫不必要) | PACELC 軸 EL 可接受,PC/EC 是過度付費 |
| **延遲 SLO** | P50 / P95 / P99 by 查詢類別 | 寫入 P95 < 20ms ✅、月報 P95 < 30s ❌(實際 90s) | 月報 SLO 是「可接受上限」,不是理想目標;超標 3× 才觸發問題 |

把這五維填完,選型對話會從「Cassandra 是不是比較快」變成「我們的 78% 聚合查詢在哪個引擎上 P95 能進 30 秒」。後者才是可被測試的問題。注意:讀寫比(1:6 寫密集進料)和查詢成本(78% 聚合佔 DB 工時主體)是**兩個獨立的量化維度** ⸺ SunLedger 的教訓正是因為只看到「有很多寫入」就推斷「需要寫入擴展方案」,跳過了「聚合查詢成本才是瓶頸」這個推論步驟。
...
> **關於 SQL 的版本說明**:事故覆盤白板上的完整查詢有「四個 join、兩個時間視窗、一個 PR 計算」——四張表(`inverter_telemetry`、`site_weather_15min`、`sites`、`site_inverters`)跨兩個窗口(當日 vs 前 30 天滾動)。下面的 SQL 是**簡化示意版本**,保留三個 join + PR 計算,省略了 `site_inverters` 與第二個時間視窗分支,目的是展示 hypertable + continuous aggregate 的核心原理——使用相同量級資料集、相同 P95 改善幅度(90 秒 → 7.4 秒)可以在此基礎上重現。完整四 join 版本的查詢欄位定義與測試資料規格,詳見本書 `book/_refs/case-registry.yaml` 的 `CASE-ENR-003` 條目。
...
-- ↑ 這一層是「提前做好 pre-aggregation」的關鍵:月報查詢直接查這張視圖,
--   不再掃 7 億 row 的 raw telemetry;這才是 90s → 7.4s 改善的主因。
...
-- 月報查詢(簡化示意版 — 3 join):
-- 過去 30 天每案場每 15 分鐘發電,對齊氣象站日射,計算 PR
...
JOIN site_weather_15min w          -- join 1:發電量 對齊 日射量
  ON s.site_id = w.site_id AND s.bucket = w.bucket
JOIN sites                         -- join 2:案場面積 + 模組效率(算 PR 分母)
  ON sites.site_id = s.site_id
WHERE s.bucket >= now() - INTERVAL '30 days'
ORDER BY s.site_id, s.bucket;
-- 完整版另有 join 3(site_inverters 用於過濾停機逆變器)
-- 及 join 4(前一年同期 bucket 用於 YoY 時間視窗比較)
```

這份 schema 在 SunLedger 的真實量級(7 億 row,原始 1.4TB)上跑同樣的月報查詢,P95 約 6–8 秒,TSDB 壓縮後儲存 ~480GB(壓縮比約 3×;若再開 columnar 壓縮可進一步壓到 ~120GB)⸺ 跟 Cassandra 那條路相比,沒換引擎、沒新增運維、查詢甚至更快。性能改善的根本原因不是「PG 變快了」,而是 continuous aggregate 把二次聚合的計算從查詢時挪到了寫入後——這是 Cassandra 應用層 join 路線永遠無法複製的設計。

### 15.3.5 決策樹 ⸺ 這次該換引擎嗎?

決策樹的入口(Q0)是「workload profile 是否完整」——這呼應 §15.2.1 的三層模型:沒有 profile 就沒有選型依據。進入 Q1 之後,邏輯順序是:**先按 PACELC 軸的一致性強度確認方向**,再按寫入量級細分路徑。具體而言,如果 §15.2.2 的 PACELC 分析顯示系統需要 PC/EC(強一致、低延遲聚合),就應該先排除 PA/EL 類引擎(Cassandra、DynamoDB 預設)——因為 EL 延遲特性在聚合密集場景下會產生 §15.1 描述的「應用層拼裝」問題。只有在 PACELC 軸確認可接受 PA/EL 且寫入吞吐量確實超出單機上限時,Wide Column 路徑才成立。把 workload 寫入量作為決策樹的第一個具體問題(Q1),是因為寫入量可直接量化、易於從監控取得——但它的前提是 PACELC 軸已通過。
...
### 反模式 2:為了「擴展性」用 Cassandra,但瓶頸其實是聚合查詢

Cassandra 的設計重點是「線性寫入擴展 + partition key 點查」。**即使寫入量確實大於讀取量(如 SunLedger 的 1:6),只要查詢模式以聚合為主(78%),Cassandra 仍然是錯誤答案** ⸺ 它的 partition key 設計會逼你把資料先撈到應用層算,網路來回 + 應用層聚合的代價常常比 PG 一條 SQL 含 pre-aggregation 慢數倍。SunLedger 的錯誤不是「讀寫比判斷錯誤」,而是「把寫密集(進料量)與聚合查詢成本(DB 工時)混為一談」。

> ✅ **修正方向**:查詢模式中聚合占比 > 50%,Cassandra 通常不是答案,**不論讀寫比如何**。先看 PG 的 BRIN / partition / continuous aggregate / materialized view 能不能解,再看 ClickHouse / DuckDB 這類 analytical replica。Cassandra 真正適合的是「寫遠多於讀、查詢以 partition key 單一 row 點查為主、幾乎不做跨 partition 聚合」的場景(典型如:訊息存放、event log、IoT 原始 telemetry 入口且只查最近 N 筆)。
...
| 維度 | 量化值 | 來源 |
|---|---|---|
| 讀寫比 | reads:writes ≈ **1 : 6**(寫密集進料)⸺但讀側因月報聚合每次掃 7 億 row,**查詢耗費時間佔 97% DB 工時** | Grafana 30 天 P50 |
| 查詢模式 | 點查 8% / 範圍 12% / **聚合 78%** / 全文 0% / 向量 2% | pg_stat_statements |
| 規模 | 7 億 row、壓縮後 ~480 GB(原始 ~1.4 TB)、年增 40% | pg_database_size + TSDB chunk stats |
...
- 主要理由:瓶頸是**聚合 78% 的查詢成本**(每次月報掃 7 億 raw row),而非寫入吞吐量(1:6 寫密集進料但 P95 < 20ms ✅);continuous aggregate 把計算挪到寫入後,正面解決二次聚合問題
...
- **AI 時代**:[Ch 37 AI-Native 架構](../part-07-ai-era/ch-37-ai-native-architecture.md) ⸺ embedding 第一公民與向量索引在系統設計中的位置
