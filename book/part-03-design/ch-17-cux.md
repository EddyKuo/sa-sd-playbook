...
BPMN 的決策菱形有一個根本假設:輸入是確定的布林值(是/否、成功/失敗)。但上述每個情況都帶著機率與不確定性 ⸺ 攝影機的判斷是「跌倒信心 0.61」,不是「跌倒 = true」;語音的回應是「顫抖程度 0.7」,不是「否認 = true」。把這些連續機率訊號硬塞進「是/否」菱形,結果要麼資訊被截斷,要麼閾值被寫死在圖上卻實際無法維護。

因此,BPMN 的活動框與決策菱形,完全表達不出**「機率性訊號」「持續推論」「跨模態融合」「不確定下的決策」** 這四件事。語意太離散、太線性、太確定。
...
與其他實時決策框架相比,SDC 之所以適合 SA 階段,是因為它將複雜的機率推論封裝成可審查的五層結構,便於跨職能團隊溝通。Bayesian Networks 和 Markov Decision Processes 雖然理論更嚴謹,但需要完整的機率分佈與轉移矩陣才能落地,SA 階段往往缺乏這些數據基礎;更重要的是,這兩者沒有「不可逆動作」的概念,無法直接回答「信心 0.82 時該撥 119 嗎?」這種工程問題。SDC 的五層結構(訊號目錄、特徵抽取、融合規則、信心決策、可逆性評估)對應到五份具體的分析工件,讓 PM、SA、工程師第一次坐下來就能對著同一張圖討論,而不是各自拿著黑盒子。
...
上面的三層是**縱向**的對話元件(Intent → Slots → Confirmation),描述的是「一次對話請求」在系統內部被如何分解與填充。下面的五條路徑則是**橫向**的使用者行為模式,描述的是「使用者在不同意願與能力狀態下,會如何走完(或走不完)一次請求」。每條路徑都會以不同方式組合上面這三層元件:Happy Path 三層全部順序走完;Slot Filling 卡在 Layer 2 反覆補槽;Bailout 可能連 Layer 1 都沒確認就中斷。
...
圖中的 `Decision` 節點對應 SDC Layer 4(信心度決策):它的輸出不是直接操作,而是帶信心度的結構化事件(例如 `{event: "fall_confirmed", confidence: 0.95, source: "temporal_3frame"}`)。這個結構化事件才是 §17.2 所述 BPMN 層的觸發條件 ⸺ 視覺採樣圖在 Edge 完成特徵抽取與 Temporal Aggregation 後,把判斷結果「翻譯」成業務層能讀懂的事件,BPMN 接管後續的跨人/跨系統協作。
