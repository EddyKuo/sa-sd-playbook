...
換句話說,**架構風格的成本是真實的**:抽象層越多,讀懂越慢、改動越貴、新人上手越久。Clean 的代價在 8 人 SaaS 後台收不回來;分層的代價在 200 人 fintech 系統會以**義大利麵式耦合 + 跨模組一致性維護**的形式付出 ⸺ 每個支付閘道的 SDK 升版都逼你翻遍 Business Layer,每次合規規則更新都不確定哪個 Service 偷偷直接呼叫了外部 SDK。下一段給你一張對照表來判斷。
...
flowchart TD
    ...
    Q1 -->|是| Q2{業務規則是否跨越<br/>多個獨立決策領域?<br/>即 DDD Aggregate 各自有<br/>獨立不變量,彼此獨立演變;<br/>例如:定價 × 合規 × 履約}
    Q1 -->|否| Q3{壽命 > 5 年?}
...
這個視角改變了 §13.3.1 對照表的讀法:如果你的系統需要呼叫 LLM,它在**邊界變動**列應該被視為第 N+1 個外部系統 ⸺ 因此會強化你對 Hexagonal 或 Clean 的選擇、削弱 Layered 的評分。回到 §13.3.3 的決策樹:AI Agent 的存在會讓 Q1「有 ≥ 2 個外部系統要對接」更快答「是」;若 Agent 同時帶入跨領域決策(如 LLM 輸出影響定價、合規、履約三個 Aggregate),Q2 也可能從「否」翻成「是」。
